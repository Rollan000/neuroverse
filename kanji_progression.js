...
#knjModeInner { min-height: 400px; box-sizing: border-box; position: relative; z-index: 1; }
...
`;
  ...
  function _renderCatalogTab(container) {
    ...
    trackHdr.innerHTML = '<div class="knj-track-pill">N5</div><div class="knj-track-name">N5 Track</div><div class="knj-track-count">' + introduced + ' / ' + total + '</div>';
    container.appendChild(trackHdr);

    const grid = document.createElement('div'); grid.className = 'knj-kanji-grid';
    n5Data.forEach(k => grid.appendChild(_renderKjTile(k, kd => {
    ...
    if (candidates.length > 0) {
      const gachaWrap = document.createElement('div'); gachaWrap.style.cssText = 'text-align:center;margin-top:16px;margin-bottom:8px';
      gachaWrap.innerHTML = '<button class="btn ' + (canLearn ? 'btn-primary' : 'btn-secondary') + '" id="knjGachaCTA" style="font-size:12px;padding:9px 22px;' + (!canLearn ? 'opacity:.45;cursor:default' : '') + '">' +
        (canLearn ? '✦ Learn a new kanji — ' + (MAX_NEW_PER_DAY - todayCount) + ' left today' : 'Daily limit reached (5/5)') + '</button>';
      container.appendChild(gachaWrap);
      if (canLearn) gachaWrap.querySelector('#knjGachaCTA').addEventListener('click', () => _openGachaModal(container));
    }
  }

  // ── _renderPathTab() — per-kanji grouped nodes ──────────────────
  function _renderPathTab(container) {
    ...
    const wrap = document.createElement('div'); wrap.style.cssText = 'padding:4px 0 20px';
    const colors = { intro:'#8B7CF6', drill:'#185FA5', reading:'#EF9F27', reinforce:'#1D9E75', rank_check:'#D85A30' };
    const labels = { intro:'Introduction', drill:'Drill', reading:'Reading', reinforce:'Reinforce', rank_check:'Rank Check' };

    allIntroduced.forEach((k, ki) => {
      const p = progress[k.char]; if (!p) return;

      // Per-kanji section header
      const secHdr = document.createElement('div');
      secHdr.style.cssText = 'display:flex;align-items:center;gap:8px;margin:' + (ki === 0 ? '0' : '18px') + ' 0 8px';
      secHdr.innerHTML =
        '<span style="font-family:\'Noto Sans JP\',sans-serif;font-size:22px;font-weight:900;color:var(--text)">' + k.char + '</span>' +
        '<span style="font-size:13px;font-weight:700;color:var(--text)">' + k.meaning + '</span>' +
        '<span style="flex:1;height:1px;background:var(--border);display:block;margin-left:4px"></span>';
      wrap.appendChild(secHdr);

      // Build nodes for this kanji
      const nodes = [];
      ...
      nodes.forEach((node, idx) => {
        ...
        const isLocked    = curIdx !== -1 && idx > curIdx && !isCompleted;
        ...
        const statusTag   = isCompleted
          ? '<span style="font-family:\'Space Mono\',monospace;font-size:8px;color:var(--correct);background:rgba(0,255,157,0.1);border:1px solid rgba(0,255,157,0.25);border-radius:4px;padding:1px 5px;margin-left:6px">done</span>'
          : isCurrent
            ? '<span style="font-family:\'Space Mono\',monospace;font-size:8px;color:' + color + ';background:' + color + '15;border:1px solid ' + color + '33;border-radius:4px;padding:1px 5px;margin-left:6px">tap to start</span>'
            : '';
        const lineDiv = idx < nodes.length - 1
          ? '<div style="position:absolute;left:11px;top:26px;bottom:0;width:2px;background:' + (isCompleted ? color : 'var(--border)') + ';opacity:0.4"></div>' : '';
        const dotDiv  = '<div style="position:relative;z-index:1;width:24px;height:24px;border-radius:50%;flex-shrink:0;margin-top:2px;display:flex;align-items:center;justify-content:center;' + dotStyle + '">' +
          (isCompleted ? '<div style="width:10px;height:10px;border-radius:50%;background:#fff;opacity:0.9"></div>' : '') + '</div>';
        const contentDiv = '<div style="flex:1;padding-top:2px;opacity:' + (isLocked ? '0.3' : '1') + '">' +
          '<div style="display:flex;align-items:center;margin-bottom:2px">' +
            '<span style="font-size:13px;font-weight:700;color:' + (isCurrent ? color : 'var(--text)') + '">' + (labels[node.type] || node.type) + rankLabel + '</span>' + statusTag +
          '</div>' +
        '</div>';

        const nodeEl = document.createElement('div');
        nodeEl.style.cssText = 'display:flex;align-items:flex-start;gap:12px;padding:0 0 10px;position:relative';
        nodeEl.innerHTML = lineDiv + dotDiv + contentDiv;

        if (isCurrent) {
          nodeEl.style.cursor = 'pointer';
          const fullNode = Object.assign({ char: k.char, meaning: k.meaning }, node);
          nodeEl.addEventListener('click', () => _startPathNode(fullNode, document.getElementById('knjModeInner')));
  ...
  function _renderSentenceBlock(s, opts) {
    ...
    const furiVisible = stored ? stored === 'show' : (opts.showFuri !== false);
  ...
  function _dotsHTML() {
    ...
    return '<div class="knj-dot-row">' +
      ds.sequence.map((s, i) => {
        let cls = 'knj-dot';
        if (i < ds.idx)      cls += ' knj-dot-ok';
        else if (i === ds.idx) cls += ' knj-dot-cur';
        return '<div class="' + cls + '"></div>';
      }).join('') +
    '</div>';
  }

  // P3/P5: 4-grade row with interval subs
  function _gradeRowHTML(iv) {
    iv = iv || { again:'<1d', hard:'~3d', good:'~7d', easy:'~14d' };
    return '<div class="knj-grade-row">' +
      '<button class="knj-grade-btn knj-grade-again" data-grade="0">Again<span class="knj-grade-sub">' + iv.again + '</span></button>' +
      '<button class="knj-grade-btn knj-grade-hard"  data-grade="1">Hard<span class="knj-grade-sub">'  + iv.hard  + '</span></button>' +
      '<button class="knj-grade-btn knj-grade-good"  data-grade="2">Good<span class="knj-grade-sub">'  + iv.good  + '</span></button>' +
      '<button class="knj-grade-btn knj-grade-easy"  data-grade="3">Easy<span class="knj-grade-sub">'  + iv.easy  + '</span></button>' +
    '</div>';
  }
  ...
  function _runDrillStep(container) {
    ...
    // advance(correct): correct=true moves forward; correct=false stays on same step
    // For MCQ/reading/context: wrong answer stays on same step after delay (re-show same prompt)
    // Write step always moves forward (self-graded, handled inside _renderActiveWrite)
    const advance = (correct) => {
      if (correct) { ds.idx++; ds.totalCorrect++; }
      const delay = correct ? 800 : 1600;
      setTimeout(() => _runDrillStep(container), delay);
    };
    ...
    else if (step.type === 'context')     _renderActiveContext(ds.char, container, advance, ds._usedSentenceIdx);
  ...
  function _renderActiveMCQ(char, container, isReverse, onResult) {
    ...
    grid.querySelectorAll('.knj-mcq-opt').forEach(opt => {
      opt.addEventListener('click', () => {
        ...
        const mnemSnip = k.mnemonic ? k.mnemonic.slice(0, 80) + (k.mnemonic.length > 80 ? '…' : '') : k.meaning;
        feedback.innerHTML = isCorrect
          ? '<div class="knj-exp-box knj-exp-box-correct"><strong>✓ Correct!</strong> ' + mnemSnip + '</div>'
          : '<div class="knj-exp-box knj-exp-box-wrong"><strong>✗ ' + k.char + ' = ' + k.meaning + '</strong><br>' + mnemSnip + '</div>';
        onResult(isCorrect);
      });
    });
  }
  ...
  function _renderActiveContext(char, container, onResult, usedIdx) {
    const k = n5Data.find(d => d.char === char); if (!k) return;
    const sentences = k.sentences || [];
    if (!sentences.length) { onResult(true); return; }

    // Pick a sentence, preferring one not used in the previous context step
    let sIdx = Math.floor(Math.random() * sentences.length);
    if (sentences.length > 1 && usedIdx !== undefined && sIdx === usedIdx) {
      sIdx = (sIdx + 1) % sentences.length;
    }
    const s = sentences[sIdx];
    // Store which sentence we used so next context step can avoid it
    if (_drillState) _drillState._usedSentenceIdx = sIdx;
  ...
  function _renderActiveWrite(char, container, onComplete) {
    ...
    container.innerHTML =
      _dotsHTML() +
      _eyebrowHTML('Stage 4 · Writing Recall', 'Self-grade', 'knj-eyebrow-badge-warn') +
      '<div class="card" style="padding:20px;text-align:center">' +
        ...
        '<div style="font-size:30px;font-weight:800;margin-bottom:8px;color:var(--text)">' + (k ? k.meaning : '') + '</div>' +
        ...
        '</div>' +
        '<div class="knj-write-hint">Ghost model at 10% · draw over it or alongside</div>' +
        ...
        '</div>' +
        '<div class="knj-self-grade-note">Did you get the shape right?<br><span style="font-size:10px;opacity:.7">Reading accuracy doesn\'t count here</span></div>' +
        '<div class="knj-grade-row">' +
          '<button class="knj-grade-btn knj-grade-again" id="knjWriteAgain">Again<span class="knj-grade-sub"><1d</span></button>' +
          '<button class="knj-grade-btn knj-grade-hard"  id="knjWriteHard">Hard<span class="knj-grade-sub">~3d</span></button>' +
          '<button class="knj-grade-btn knj-grade-good"  id="knjWriteGot">Got it ✓<span class="knj-grade-sub">~7d</span></button>' +
        '</div>' +
      '</div>';
    container.querySelector('#knjBackBtn').addEventListener('click', () => { _drillState = null; renderHome(container); });
    ...
    container.querySelector('#knjWriteAgain').addEventListener('click', () => {
      const ds = _drillState;
      if (ds && ds.mode === 'learn') {
        ds.sequence.splice(ds.idx + 1, 0, { type:'mcq', reverse:false }, { type:'mcq', reverse:true }, { type:'write' });
      }
      onComplete(false);
    });
    container.querySelector('#knjWriteHard').addEventListener('click', () => onComplete(true));
