...
// Flash known
(function _patchFlashForQuests() {
  const _orig = typeof fcKnown !== 'undefined' ? true : false;
  // Hook rewardStudy call path — track flash_known via flashcards known button
})();
...
function generateShopMain() {
  ...
  if (activeCat === 'all' || activeCat === 'consumable') {
    ...
    vialEl.innerHTML = \`
      ...
      \${vialCanAfford
        ? '<button class="btn btn-secondary" style="font-size:12px;padding:6px 12px;white-space:nowrap;margin-left:10px">Buy 200 🎰</button>'
        : '<button class="btn" style="font-size:12px;padding:6px 12px;white-space:nowrap;margin-left:10px;opacity:0.4;cursor:not-allowed" disabled>Need 200 🎰</button>'
      }
    \`;
    if (vialCanAfford) {
