...
function _invItemIcon(item) {
  // Returns an icon filename from R2 icons/ based on item type and id
  const id = item.id || '';
  const type = item.type || '';
  if (id.includes('void') || id.includes('skull') || id.includes('crimson') || id.includes('remove') || id.includes('none')) return 'skull.png';
  if (id.includes('overdrive')) return 'trophy.png';
  if (id.includes('ember') || id.includes('vfx_basic') || id.includes('fire')) return 'fire.png';
  if (id.includes('star') || id.includes('nebula') || id.includes('aurora')) return 'star.png';
  if (id.includes('matrix') || id.includes('steel')) return 'settings_button.png';
  if (id.includes('rain') || id.includes('synth')) return 'quill.png';
  if (id.includes('arcane') || id.includes('sakura')) return 'important_loooking_book.png';
  if (id.includes('naruto') || id.includes('slayer') || id.includes('ocean') || id.includes('forest') || id.includes('dusk')) return 'open_book.png';
  if (id.includes('gold') || id.includes('neongreen')) return 'gold_coin_stack.png';
  if (id.includes('aot') || id.includes('tsundere')) return 'paper_with_writing.png';
  if (id.includes('dlg') || id.includes('genki') || id.includes('kuudere') || id.includes('chuu')) return 'dictionary_book.png';
  if (id.includes('title')) return 'trophy.png';
  if (type === 'theme') return 'closed_book.png';
  if (type === 'effect') return 'quill.png';
  if (type === 'vfx') return 'fire.png';
  if (type === 'bg') return 'open_book.png';
  if (type === 'dialogue') return 'dictionary_book.png';
  if (type === 'cursor') return 'quill.png';
  return 'star.png';
}
          ...
          <div class="inv-section">
            ...
            <div class="bag-items-grid">
              ${items.map(item => {
                ...
                const iconFile = _invItemIcon(item);
                return `<div class="bag-inv-item ${isEquipped ? 'equipped' : ''}" title="${_invItemTooltip(item)}">
                  ...
                  ${iconFile ? `<img src="${R2_BASE}/icons/${iconFile}" class="bag-inv-item-icon" style="width:22px;height:22px;image-rendering:pixelated;flex-shrink:0" alt="">` : ''}
