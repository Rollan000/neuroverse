...
function setMcqFilter(axis, val, el) {
  ...
  if (el && el.parentElement) {
    el.parentElement.querySelectorAll('.mcq-filter-btn').forEach(b => b.classList.remove('active'));
    el.classList.add('active');
  }
...
const CURSOR_ITEMS = [
  { id: 'cursor_sword',      name: '⚔️ Sword Cursor',     cursor: 'sword',      cost: 300,  rarity: 'uncommon' },
  { id: 'cursor_wand',       name: '🪄 Magic Wand',        cursor: 'wand',       cost: 450,  rarity: 'rare'     },
  { id: 'cursor_star',       name: '⭐ Star Cursor',        cursor: 'star',       cost: 250,  rarity: 'common'   },
  { id: 'cursor_sakura',     name: '🌸 Sakura Cursor',     cursor: 'sakura',     cost: 380,  rarity: 'uncommon' },
  { id: 'cursor_kunai',      name: '🗡️ Kunai Cursor',      cursor: 'kunai',      cost: 500,  rarity: 'rare'     },
  { id: 'cursor_pencil',     name: '✏️ Scholar Cursor',    cursor: 'pencil',     cost: 200,  rarity: 'common'   },
  { id: 'cursor_gem',        name: '💎 Diamond Cursor',    cursor: 'gem',        cost: 600,  rarity: 'epic'     },
  { id: 'cursor_fire',       name: '🔥 Fire Cursor',       cursor: 'fire',       cost: 700,  rarity: 'epic'     },
  { id: 'cursor_crosshair',  name: '🎯 Crosshair',         cursor: 'crosshair',  cost: 350,  rarity: 'uncommon' },
];

let equippedCursor = localStorage.getItem('equippedCursor') || 'none';

function applyCursorCosmetic(cursorId) {
  // Remove all cursor classes
  document.body.className = document.body.className.replace(/cursor-\S+/g, '').trim();
  if (cursorId && cursorId !== 'none') {
    document.body.classList.add('cursor-' + cursorId);
  }
...
(function _cursorInit() {
  if (equippedCursor && equippedCursor !== 'none') applyCursorCosmetic(equippedCursor);
})();

// Patch equipItem to handle cursor type
const _s16_origEquipItem = typeof equipItem !== 'undefined' ? equipItem : null;
if (typeof equipItem !== 'undefined') {
  const _s16_base = equipItem;
  equipItem = function(itemId) {
    const item = inventory.find(i => i.id === itemId);
    if (item && item.type === 'cursor') {
      applyCursorCosmetic(item.data);
  ...
  if (typeof shopPool !== 'undefined') {
    CURSOR_ITEMS.forEach(ci => {
      if (!shopPool.find(s => s.id === ci.id)) {
        shopPool.push({ id: ci.id, name: ci.name, type: 'cursor', data: ci.cursor,
          cost: ci.cost, rarity: ci.rarity, desc: 'Custom cursor cosmetic.' });
      }
    });
  }
})();
...
function renderEquippedPreviewPanel() {
  ...
  const equipped = [
    ...
    { type: 'cursor',   icon: '🖱️', label: 'Cursor',   value: (equippedCursor && equippedCursor !== 'none') ? equippedCursor : 'Default',
...
if (typeof renderInventory !== 'undefined') {
  ...
  renderInventory = function() {
    ...
    if (eqCursor) {
      eqCursor.textContent = (equippedCursor && equippedCursor !== 'none') ? equippedCursor : 'Default';
    }
  };
}
