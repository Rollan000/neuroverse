...
const ACHIEVEMENTS_DEF = [
  { id: 'ach_100_mcq',    name: 'Century',      desc: 'Answer 100 MCQ correctly',         target: 100,  track: 'mcq_correct_total',   reward: 200, icon: 'trophy.png'  },
  { id: 'ach_50_trace',   name: 'Pensmith',     desc: 'Trace 50 kanji correctly',          target: 50,   track: 'trace_correct_total',  reward: 150, icon: 'quill.png'   },
  { id: 'ach_500_mcq',    name: 'Veteran',      desc: 'Answer 500 MCQ correctly',          target: 500,  track: 'mcq_correct_total',   reward: 500, icon: 'trophy.png'  },
  { id: 'ach_streak_7',   name: 'Consistent',   desc: '7-day study streak',                target: 7,    track: 'day_streak',          reward: 150, icon: 'fire.png'    },
  { id: 'ach_streak_30',  name: 'Devoted',      desc: '30-day study streak',               target: 30,   track: 'day_streak',          reward: 500, icon: 'fire.png'    },
  { id: 'ach_vault_50',   name: 'Scholar',      desc: '50 items in learned vault',         target: 50,   track: 'vault_count',         reward: 200, icon: 'chest.png'   },
  { id: 'ach_vault_100',  name: 'Archivist',    desc: '100 items in learned vault',        target: 100,  track: 'vault_count',         reward: 300, icon: 'chest.png'   },
  { id: 'ach_review_100', name: 'Reviewer',     desc: 'Review 100 cards',                  target: 100,  track: 'review_graded_total', reward: 200, icon: 'open_book.png'},
  { id: 'ach_dq_10',      name: 'On a Mission', desc: 'Complete 10 daily quests',          target: 10,   track: 'dq_completed_total',  reward: 300, icon: 'star.png'    },
  { id: 'ach_sac_1000',   name: 'Devoted Soul', desc: 'Earn 1000 sacraments total',        target: 1000, track: 'sac_earned_total',    reward: 400, icon: 'moon_tear.png'},
  { id: 'ach_flash_50',   name: 'Card Shepherd', desc: 'Mark 50 flashcards as known',        target: 50,   track: 'flash_known_total',   reward: 150, icon: 'open_book.png'},
];
...
function answerMCQ(choice) {
  ...
  if(correct){
    mcqScore.c++; session.mcqC++; rewardStudy(1);
    // Phase 2.2 — boost meaning dimension on vault item for the answered word
    (function() {
      try {
        const vault = JSON.parse(localStorage.getItem('learnedVault') || '[]');
        const term = q.q;
        const idx = vault.findIndex(v => v.term === term || v.meaning === term);
        if (idx === -1) return;
        if (!vault[idx].mastery) vault[idx].mastery = { meaning: 0, reading: 0, writing: 0 };
        vault[idx].mastery.meaning = Math.min(100, (vault[idx].mastery.meaning || 0) + 5);
...
function renderBagNew() {
  ...
  topBar.innerHTML = `
    <div style="font-family:'Space Mono',monospace;font-size:10px;text-transform:uppercase;letter-spacing:2px;color:var(--accent)">Cosmetics</div>
  ...
  titleHeader.className = 'bag-col-header';
  titleHeader.innerHTML = `Titles <span class="bag-col-count">${inventory.filter(i=>i.type==='title').length}</span>`;
  leftCol.appendChild(titleHeader);

  const titleItems = inventory.filter(i => i.type === 'title');
  if (titleItems.length === 0) {
    const empty = document.createElement('div');
    empty.className = 'bag-empty-note';
    empty.textContent = 'No titles yet. Buy from the Shop.';
    ...
    titleItems.forEach(item => {
      ...
      const displayName = item.name.replace(/^Title:\s*/,'');
      div.innerHTML = `
        <div class="bag-title-left">
          ${isEq ? '<span class="bag-title-eq-dot"></span>' : '<span class="bag-title-eq-dot hidden"></span>'}
          <span class="bag-title-name">${displayName}</span>
        </div>
        <div class="bag-title-actions">
  ...
  if (_bagPackDetailOpen) {
    ...
    const bgPacks = COSMETIC_PACKS.filter(p => p.category === 'bg');
    const csPacks = COSMETIC_PACKS.filter(p => p.category === 'colorset');

    // Background packs section
    const bgHeader = document.createElement('div');
    bgHeader.className = 'bag-col-header';
    bgHeader.textContent = 'Backgrounds';
    rightCol.appendChild(bgHeader);

    const bgGrid = document.createElement('div');
    bgGrid.className = 'bag-pack-grid2';
    rightCol.appendChild(bgGrid);

    bgPacks.forEach(pack => _renderBagPackCard2(bgGrid, pack));

    // Color sets section
    const csHeader = document.createElement('div');
    csHeader.className = 'bag-col-header';
    csHeader.style.marginTop = '14px';
    csHeader.textContent = 'Color Sets';
    rightCol.appendChild(csHeader);

    const csGrid = document.createElement('div');
    csGrid.className = 'bag-pack-grid2';
    rightCol.appendChild(csGrid);

    csPacks.forEach(pack => _renderBagPackCard2(csGrid, pack));
  }
...
// Helper: render a single redesigned pack card into a grid container
function _renderBagPackCard2(container, pack) {
  ...
  card.className = 'bag-pack-card2' + (owned ? ' owned' : ' not-owned') + (isComplete ? ' complete' : '');
  card.onclick = () => { _bagPackDetailOpen = pack.id; renderBagNew(); };

  // Cover gradient area
  const cover = document.createElement('div');
  cover.className = 'bag-pack-cover';
  cover.style.background = pack.coverColor;

  // Color swatch dots — first 4 items colours pulled from coverColor shades (visual hint)
  const swatches = document.createElement('div');
  swatches.className = 'bag-pack-swatches';
  // Generate subtle swatch colors from the items array (use their index to vary)
  const swatchColors = _bagPackSwatchColors(pack);
  swatchColors.forEach(col => {
    const dot = document.createElement('span');
    dot.className = 'bag-pack-swatch-dot';
  ...
  catPill.className = 'bag-pack-cat-pill';
  catPill.textContent = pack.category === 'colorset' ? 'Color Set' : 'Background';
  cover.appendChild(catPill);

  card.appendChild(cover);

  // Info section below cover
  const info = document.createElement('div');
  info.className = 'bag-pack-info';

  const nameEl = document.createElement('div');
  nameEl.className = 'bag-pack-name2';
  ...
  if (owned) {
    ...
    prog.className = 'bag-pack-prog2';
    const pct = Math.round((ownedCount / totalCount) * 100);
    prog.innerHTML = `
      <div class="bag-pack-progbar">
        <div class="bag-pack-progfill" style="width:${pct}%"></div>
      </div>
      <span class="bag-pack-prog-label">${ownedCount}/${totalCount} ${isComplete?'· Complete':''}</span>`;
    info.appendChild(prog);
  } else {
    // Buy prompt
    const buyLine = document.createElement('div');
    buyLine.className = 'bag-pack-buy-line';
    buyLine.innerHTML = `<span style="font-size:11px;color:var(--muted)">${pack.packCost}</span>
      <img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/moon_tear.png" style="width:10px;height:10px;image-rendering:pixelated;vertical-align:-1px" alt="">
      <button class="bag-pack-buy-btn2" onclick="event.stopPropagation();buyCosmPack('${pack.id}')">Unlock</button>`;
    info.appendChild(buyLine);
  }
...
// Generate representative swatch colors for a pack based on its coverColor
function _bagPackSwatchColors(pack) {
  // Colorset packs: show accent-ish hues; bg packs: show lighter/darker variants of cover
  const colorMap = {
    pack_melancholic: ['#3a3a50','#2e2e42','#505060','#1e1e2e'],
    pack_cool:        ['#0d2040','#1a0f3a','#163050','#243060'],
    pack_warm:        ['#3a1800','#501a00','#3a2808','#602000'],
    pack_crimson:     ['#2a0000','#4a0808','#380010','#5a0505'],
    pack_monotone:    ['#1a1a1a','#2e2e2e','#444','#555'],
    pack_frost_cs:    ['#d0d8e8','#7060a8','#a0c8d8','#e8e4dc'],
    pack_ember_cs:    ['#c87840','#e87830','#b85030','#e0a060'],
  };
  return colorMap[pack.id] || ['#333','#444','#555','#666'];
}
