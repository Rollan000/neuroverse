...
function equipItem(itemId) {
  ...
  // Check if already equipped — if so, unequip (toggle off)
...
function _invItemTooltip(item) {
  const isEquipped = (item.type === 'dialogue' && equippedDialogue === item.data) ||
                     ...
                     (item.type === 'rain' && rainOverlayVariant === item.data);
...
// RAF-throttled so it only updates once per frame — no layout thrashing.
...
function applyCursorCosmetic(cursorId, variantIdx) {
  ...
  // .cur files work as static CSS cursors; everything else uses fake cursor so animation plays
...
if (typeof equipItem !== 'undefined') {
  ...
  equipItem = function(itemId) {
    ...
    if (item && item.type === 'rain') {
      if (rainOverlayVariant === item.data) {
        // Already equipped — unequip
