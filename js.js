...
async function _anLoadSubtitles(ep) {
  ...
  if (ep.subtitleUrl) {
    ...
    } catch(e) { console.warn('[anime] remote subtitle fetch failed', e); }
  }

  // Fallback: local episode1.json served alongside the page
  if (!data) {
    try {
      const r = await fetch('./episode1.json');
      if (r.ok) { data = await r.json(); }
    } catch(e) { console.warn('[anime] local subtitle fetch failed', e); }
  }

  if (data && Array.isArray(data.segments) && data.segments.length > 0) {
    // filter out high-no-speech-prob segments (music / silence)
    _AN.segments = data.segments.filter(s => (s.no_speech_prob ?? 0) < 0.5);
    _anRenderSubtitles();
    console.log('[anime] loaded', _AN.segments.length, 'subtitle segments');
  } else {
    _AN.segments = [];
    ...
    console.warn('[anime] no subtitle data found');
  }
}
