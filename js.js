...
// ════════════════════════════════════════════════════════════════
// PHASE 5 — ANIME TAB ENGINE  (v2)
// ════════════════════════════════════════════════════════════════

// ── State ────────────────────────────────────────────────────────
let _animeCurrentAnimeId = 'yofukashi';
let _animeCurrentEpId    = null;
let _animeSegments       = [];
let _animeActiveSegIdx   = -1;
let _animeLookupCache    = new Map();
let _animeSynthVoice     = null;
let _animeLoopActive     = false;
let _animeLastCoinTick   = 0;
let _animeSubHistory     = [];   // all lines seen this session
let _animePopupEntry     = null;

// QS state
let _animeQSQueue  = [];   // words queued via subtitle clicks
let _animeQSMode   = 'flash';
let _animeQSDeck   = [];   // working deck for current session
let _animeQSIdx    = 0;
let _animeQSWrong  = [];
let _animeQSScore  = 0;
...
const ANIME_CATALOG = [
  {
    ...
    shopItems: [
      { id: 'yoru_title1',  name: '夜遊び',          type: 'Title',      cost: 80  },
      { id: 'yoru_title2',  name: 'Call of the Night',type: 'Title',      cost: 120 },
      { id: 'yoru_bg1',     name: 'Midnight Street',  type: 'Background', cost: 200 },
      { id: 'yoru_color1',  name: 'Night Palette',    type: 'Colour Set', cost: 150 },
    ],
  },
];
...
const _aWatched  = () => _aLS.get('animeWatched',  {});
const _aProgress = () => _aLS.get('animeProgress', {});
const _aCoins    = () => _aLS.get('animeCoins',    {});

function _aSaveWatched(d)  { _aLS.set('animeWatched',  d); }
function _aSaveProgress(d) { _aLS.set('animeProgress', d); }
function _aSaveCoins(d)    { _aLS.set('animeCoins',    d); }

function animeCoinsGet(animeId)    { return _aCoins()[animeId] || 0; }
function animeCoinsAdd(animeId, n) {
  const d = _aCoins();
  d[animeId] = (d[animeId] || 0) + n;
  _aSaveCoins(d);
  // refresh any visible coin display
  document.querySelectorAll('#animeShopBalance, #animePlayerCoins').forEach(el => {
    el.textContent = d[_animeCurrentAnimeId] || 0;
  });
}
...
function _animeInitSynth() {
  ...
  const pick = () => {
    ...
    _animeSynthVoice = v.find(x => /ja[-_]JP/i.test(x.lang)) || v.find(x => /^ja/i.test(x.lang)) || null;
  };
  pick();
  speechSynthesis.onvoiceschanged = pick;
}
function animeSpeak(text) {
  if (!text || !window.speechSynthesis) return;
  const u = new SpeechSynthesisUtterance(text);
  u.lang = 'ja-JP';
  if (_animeSynthVoice) u.voice = _animeSynthVoice;
  u.rate = 0.9;
...
async function animeWordLookup(term) {
  ...
  if (ke) {
    ...
    _animeLookupCache.set(term, r);
    return r;
  }

  // tier 2: jisho via worker
  try {
  ...
  // tier 3: fallback bare entry
  const fb = { term, reading: term, meaning: '(definition unavailable)', jlpt: '', pos: '' };
  _animeLookupCache.set(term, fb);
  return fb;
}

// ── Watched log ──────────────────────────────────────────────────
function animeLogWord(animeId, entry) {
  const d = _aWatched();
  if (!d[animeId]) d[animeId] = [];
  if (!d[animeId].some(e => e.term === entry.term && e.episodeId === entry.episodeId)) {
    d[animeId].push(entry);
    _aSaveWatched(d);
  }
}
function animeMarkAdded(animeId, term, epId) {
  const d = _aWatched();
  const e = (d[animeId]||[]).find(x => x.term === term && x.episodeId === epId);
  if (e) { e.addedToStudy = true; _aSaveWatched(d); }
}
...
function animeAddToVault(entry) {
  ...
  lvAddToVault({
    ...
    source: 'anime',
    animeId: entry.animeId || _animeCurrentAnimeId,
    episodeId: entry.episodeId || _animeCurrentEpId,
    dateAdded: Date.now(),
  });
  animeMarkAdded(entry.animeId || _animeCurrentAnimeId, entry.term, entry.episodeId || _animeCurrentEpId);
  animeCoinsAdd(entry.animeId || _animeCurrentAnimeId, 5);
...
function animeShowLibrary() {
  document.getElementById('animeLibraryView').classList.remove('hidden');
  document.getElementById('animeSeriesView').classList.add('hidden');
  document.getElementById('animePlayerView').classList.add('hidden');
  _animeStopVideo();
  animeRenderLibrary();
}

function animeRenderLibrary() {
  const grid = document.getElementById('animeLibraryGrid');
  if (!grid) return;
  grid.innerHTML = '';

  const prog = _aProgress();
  const watched = _aWatched();

  ANIME_CATALOG.forEach(anime => {
    // calc total progress
    let totalPct = 0, epCount = anime.episodes.length;
    anime.episodes.forEach(ep => {
      const p = prog[anime.id + '_' + ep.id];
      totalPct += p?.pct || 0;
    });
    const avgPct = epCount ? totalPct / epCount : 0;
    const wordCount = (watched[anime.id]||[]).length;

    const card = document.createElement('div');
    card.className = 'anime-lib-card';
    card.innerHTML = anime.posterUrl
      ? `<img class="anime-lib-card-img" src="${anime.posterUrl}" alt="${anime.title}">`
      : `<div class="anime-lib-card-img-placeholder">📺</div>`;
    card.innerHTML += `
      <div class="anime-lib-card-title">${anime.title}</div>
      <div class="anime-lib-card-sub">${epCount} eps · ${wordCount} words · ${Math.round(avgPct*100)}% watched</div>
    `;
    card.onclick = () => animeShowSeries(anime.id);
...
// VIEW: SERIES
// ════════════════════════════════════════════════════════════════
function animeShowSeries(animeId) {
  if (animeId) _animeCurrentAnimeId = animeId;
  const anime = ANIME_CATALOG.find(a => a.id === _animeCurrentAnimeId);
  if (!anime) return;

  document.getElementById('animeLibraryView').classList.add('hidden');
  document.getElementById('animeSeriesView').classList.remove('hidden');
  document.getElementById('animePlayerView').classList.add('hidden');

  document.getElementById('animeSeriesTopLabel').textContent = anime.title;
  animeSeriesTab('episodes', document.getElementById('animeNavEpisodes'));
}

function animeSeriesTab(tab, btn) {
  // nav active state
  document.querySelectorAll('.anime-snav-btn').forEach(b => b.classList.remove('active'));
  if (btn) btn.classList.add('active');

  document.getElementById('animeSeriesPanelEpisodes').classList.toggle('hidden', tab !== 'episodes');
  document.getElementById('animeSeriesPanelShop').classList.toggle('hidden', tab !== 'shop');

  if (tab === 'episodes') animeRenderEpisodes();
  if (tab === 'shop')     animeRenderShop();
}

function animeRenderEpisodes() {
  const anime = ANIME_CATALOG.find(a => a.id === _animeCurrentAnimeId);
  if (!anime) return;
  const list = document.getElementById('animeEpList');
  const subEl = document.getElementById('animeEpListSub');
  if (!list) return;

  subEl.textContent = anime.episodes.length + ' episode' + (anime.episodes.length !== 1 ? 's' : '');
  list.innerHTML = '';
  ...
  anime.episodes.forEach(ep => {
    const p = prog[_animeCurrentAnimeId + '_' + ep.id] || {};
    const pct = p.pct || 0;
    const wordCount = (watched[_animeCurrentAnimeId]||[]).filter(w => w.episodeId === ep.id).length;
    const dur = _animeFmtDur(ep.duration || 0);

    const row = document.createElement('div');
    row.className = 'anime-ep-row' + (ep.locked ? ' locked' : '');
    row.innerHTML = `
      <div class="anime-ep-thumb">${ep.thumbUrl ? `<img src="${ep.thumbUrl}" style="width:100%;height:100%;object-fit:cover;border-radius:5px">` : '▶'}</div>
      <div class="anime-ep-row-info">
        ...
        <div class="anime-ep-row-meta">
          <span>${dur}</span>
          <span>·</span>
          <div class="anime-ep-row-bar"><div class="anime-ep-row-bar-fill" style="width:${Math.round(pct*100)}%"></div></div>
...
function animeRenderShop() {
  const anime = ANIME_CATALOG.find(a => a.id === _animeCurrentAnimeId);
  if (!anime) return;
  const list = document.getElementById('animeShopItemList');
  const balEl = document.getElementById('animeShopBalance');
  if (!list) return;

  const bal = animeCoinsGet(_animeCurrentAnimeId);
  if (balEl) balEl.textContent = bal;

  list.innerHTML = '';
  (anime.shopItems || []).forEach(item => {
    const canAfford = bal >= item.cost;
    const row = document.createElement('div');
    row.className = 'anime-shop-item-row';
    row.innerHTML = `
      <div class="anime-shop-item-info">
      ...
      <button class="anime-shop-buy-btn" ${canAfford?'':'disabled'}
        onclick="animeBuyItem('${item.id}',${item.cost},'${item.name.replace(/'/g,"\\'")}','${item.type}')">
...
function animeBuyItem(itemId, cost, name, type) {
  const d = _aCoins();
  const animeId = _animeCurrentAnimeId;
  if ((d[animeId]||0) < cost) { return; }
  d[animeId] -= cost;
  _aSaveCoins(d);
...
async function animeOpenEpisode(epId) {
  ...
  _animeCurrentEpId  = epId;
  _animeSegments     = [];
  _animeActiveSegIdx = -1;
  _animeSubHistory   = [];
  _animeQSQueue      = [];
  _animeLoopActive   = false;
  _animeLastCoinTick = 0;

  document.getElementById('animeLibraryView').classList.add('hidden');
  document.getElementById('animeSeriesView').classList.add('hidden');
  document.getElementById('animePlayerView').classList.remove('hidden');
  document.getElementById('animePlayerEpLabel').textContent = `EP ${String(ep.num).padStart(2,'0')} — ${ep.title}`;

  // update coin display
  document.querySelectorAll('#animePlayerCoins').forEach(el => el.textContent = animeCoinsGet(_animeCurrentAnimeId));

  // reset QS dock
  _animeQSRenderChips();
  document.getElementById('animeQSDockStart').style.display = 'none';
  document.getElementById('animeQSSession').classList.add('hidden');
  document.getElementById('animeQSDockHint').style.display = '';

  // load subtitles
  await _animeLoadSubtitles(ep);
  ...
  const prog = _aProgress();
  const saved = prog[_animeCurrentAnimeId + '_' + epId];
  if (saved?.lastTime > 5 && !saved.completed) {
    video.addEventListener('loadedmetadata', () => { video.currentTime = saved.lastTime; }, { once: true });
  }

  video.ontimeupdate  = () => _animeOnTick(video, ep);
  video.onplay        = () => {
    document.getElementById('animePlayBtn').textContent = '⏸';
    const box = document.querySelector('.anime-video-box');
    if (box) { box.classList.add('ctrl-show'); setTimeout(() => box.classList.remove('ctrl-show'), 2200); }
  };
  video.onpause = () => { document.getElementById('animePlayBtn').textContent = '▶'; };
}

async function _animeLoadSubtitles(ep) {
  _animeSegments = [];
  const subScroll = document.getElementById('animeSubScroll');
  if (subScroll) subScroll.innerHTML = '<div style="padding:12px 13px;color:var(--muted);font-size:12px">Loading subtitles…</div>';
  ...
  // fallback: local episode1.json (dev)
  if (!data && ep.id === 'yofukashi_ep1') {
    try {
  ...
  if (data?.segments) {
    _animeSegments = data.segments.filter(s => !s.no_speech_prob || s.no_speech_prob < 0.5);
    _animeRenderAllSubtitleLines();
  } else {
    if (subScroll) subScroll.innerHTML = '<div style="padding:12px 13px;color:var(--muted);font-size:12px">No subtitles available.</div>';
  }
}

// render every subtitle line into the scroll box up front
function _animeRenderAllSubtitleLines() {
  const scroll = document.getElementById('animeSubScroll');
  if (!scroll) return;
  scroll.innerHTML = '';

  const vault   = typeof lvLoad === 'function' ? lvLoad() : [];
  const watched = (_aWatched()[_animeCurrentAnimeId] || []);

  _animeSegments.forEach((seg, i) => {
    const div = document.createElement('div');
    div.className = 'anime-sub-line';
    div.id = 'anime-sub-line-' + i;
    div.innerHTML = _animeBuildLineHTML(seg, vault, watched);
    scroll.appendChild(div);
  });
}

// rebuild a single line's inner HTML (called on re-render after lookup)
function _animeBuildLineHTML(seg, vault, watched) {
  if (seg.words && seg.words.length > 0) {
    return seg.words.map(w => {
      ...
      const queued  = _animeQSQueue.some(q => q.term === t);
      let cls = 'anime-word';
      if (queued) cls += ' queued';
      else if (known) cls += ' known';
      else if (seen)  cls += ' seen';
      return `<span class="${cls}" onclick="animeWordClick(event,'${t.replace(/'/g,"\\'")}',${seg.id??0})">${t}</span>`;
    }).join('');
  }
  return seg.text || '';
}
...
// ── Playback tick ─────────────────────────────────────────────────
function _animeOnTick(video, ep) {
  const t = video.currentTime;

  // seek bar
  const fill = document.getElementById('animeSeekFill');
  const timeEl = document.getElementById('animeTimeDisp');
  if (fill && video.duration) fill.style.width = (t / video.duration * 100) + '%';
  if (timeEl) timeEl.textContent = _animeFmtTime(t) + ' / ' + _animeFmtTime(video.duration || ep.duration || 0);

  // loop
  if (_animeLoopActive && _animeActiveSegIdx >= 0) {
    const seg = _animeSegments[_animeActiveSegIdx];
    if (seg && t >= seg.end + 0.05) { video.currentTime = seg.start; return; }
  }

  // subtitle highlight
  const idx = _animeBinSearch(t);
  if (idx !== -1 && idx !== _animeActiveSegIdx) {
    _animeActiveSegIdx = idx;
    _animeHighlightLine(idx);
  }

  // save progress
  if (video.duration) {
    const pct = t / video.duration;
    const key = _animeCurrentAnimeId + '_' + _animeCurrentEpId;
    const prog = _aProgress();
    const was = prog[key] || {};
    prog[key] = { pct, lastTime: t, completed: was.completed || pct >= 0.95 };
    if (!was.completed && pct >= 0.95) animeCoinsAdd(_animeCurrentAnimeId, 25);
    _aSaveProgress(prog);
  }

  // coin drip: 1 coin/min of play
  if (!video.paused && t - _animeLastCoinTick >= 60) {
    _animeLastCoinTick = t;
    animeCoinsAdd(_animeCurrentAnimeId, 1);
  }
}

function _animeBinSearch(t) {
  let lo = 0, hi = _animeSegments.length - 1;
  while (lo <= hi) {
    const mid = (lo + hi) >> 1;
    const s = _animeSegments[mid];
...
function _animeHighlightLine(idx) {
  // remove previous active
  const prev = document.querySelector('.anime-sub-line.is-active');
  if (prev) {
    prev.classList.remove('is-active');
    prev.classList.add('is-past');
  }
  const el = document.getElementById('anime-sub-line-' + idx);
  if (!el) return;
  el.classList.remove('is-past');
  el.classList.add('is-active');
  // smooth scroll into view — keep 1/3 from top
...
function animeSeekLine() {
  const v = document.getElementById('animeVideo');
  if (!v || _animeActiveSegIdx < 0) return;
  const seg = _animeSegments[_animeActiveSegIdx];
  if (seg) v.currentTime = seg.start;
}

function animeToggleLoop() {
  _animeLoopActive = !_animeLoopActive;
  const btn = document.getElementById('animeLoopBtn');
  if (btn) btn.classList.toggle('active', _animeLoopActive);
}

function animeSkipOP() {
  const anime = ANIME_CATALOG.find(a => a.id === _animeCurrentAnimeId);
  const ep = anime?.episodes.find(e => e.id === _animeCurrentEpId);
  const v = document.getElementById('animeVideo');
...
function animeSeekClick(evt) {
  ...
  v.currentTime = ((evt.clientX - rect.left) / rect.width) * v.duration;
}

function _animeStopVideo() {
  const v = document.getElementById('animeVideo');
  if (v) { v.pause(); v.src = ''; v.ontimeupdate = null; }
}

function animeShowSeries() {
  _animeStopVideo();
  document.getElementById('animePlayerView').classList.add('hidden');
  document.getElementById('animeSeriesView').classList.remove('hidden');
  animeRenderEpisodes();
...
async function animeWordClick(evt, term, segId) {
  ...
  _animePopupEntry = {
    ...entry,
    animeId: _animeCurrentAnimeId,
    episodeId: _animeCurrentEpId,
    timestamp: Date.now(),
    addedToStudy: false,
  };
  animeLogWord(_animeCurrentAnimeId, _animePopupEntry);
  ...
  document.getElementById('animePopupMeta').textContent = meta;
  ...
  addBtn.className   = 'anime-popup-add' + (alreadyAdded ? ' added' : '');

  // qs btn state
  const inQueue = _animeQSQueue.some(q => q.term === term);
  const qsBtn = document.getElementById('animePopupQSBtn');
  qsBtn.textContent = inQueue ? '✓ In Queue' : '+ Quick Study';
  qsBtn.className   = 'anime-popup-qs' + (inQueue ? ' queued' : '');

  document.getElementById('animeWordPopup').classList.remove('hidden');
  document.getElementById('animePopupOverlay').classList.remove('hidden');
}

function animeClosePopup() {
  document.getElementById('animeWordPopup').classList.add('hidden');
  document.getElementById('animePopupOverlay').classList.add('hidden');
  _animePopupEntry = null;
}

function animePopupAdd() {
  if (!_animePopupEntry) return;
  animeAddToVault(_animePopupEntry);
  const btn = document.getElementById('animePopupAddBtn');
  btn.textContent = '✓ Added';
  btn.classList.add('added');
  // re-render that subtitle line
  _animeRefreshActiveLine();
}

function animePopupQueueQS() {
  if (!_animePopupEntry) return;
  const term = _animePopupEntry.term;
  const inQueue = _animeQSQueue.some(q => q.term === term);
  if (inQueue) {
    _animeQSQueue = _animeQSQueue.filter(q => q.term !== term);
  } else {
    _animeQSQueue.push({ ..._animePopupEntry });
  }
  const btn = document.getElementById('animePopupQSBtn');
  const nowIn = _animeQSQueue.some(q => q.term === term);
  btn.textContent = nowIn ? '✓ In Queue' : '+ Quick Study';
  btn.className   = 'anime-popup-qs' + (nowIn ? ' queued' : '');
  _animeQSRenderChips();
  _animeRefreshActiveLine();
}

function _animeRefreshActiveLine() {
  if (_animeActiveSegIdx < 0) return;
  const seg = _animeSegments[_animeActiveSegIdx];
  const el  = document.getElementById('anime-sub-line-' + _animeActiveSegIdx);
  if (!seg || !el) return;
  const vault   = typeof lvLoad === 'function' ? lvLoad() : [];
  const watched = (_aWatched()[_animeCurrentAnimeId] || []);
  el.innerHTML  = _animeBuildLineHTML(seg, vault, watched);
}

// ── QS chip display ──────────────────────────────────────────────
function _animeQSRenderChips() {
  const chips = document.getElementById('animeQSChips');
  const hint  = document.getElementById('animeQSDockHint');
  const startBtn = document.getElementById('animeQSDockStart');
  if (!chips) return;

  chips.innerHTML = '';
  _animeQSQueue.forEach(entry => {
    const chip = document.createElement('span');
    chip.className = 'anime-qs-chip';
    chip.innerHTML = `${entry.term}<button class="anime-qs-chip-rm" onclick="animeQSRemoveChip('${entry.term.replace(/'/g,"\\'")}')">✕</button>`;
    chips.appendChild(chip);
  });

  const has = _animeQSQueue.length > 0;
  if (hint) hint.style.display = has ? 'none' : '';
  if (startBtn) {
    startBtn.style.display = has ? '' : 'none';
    const countEl = document.getElementById('animeQSDockCount');
    if (countEl) countEl.textContent = _animeQSQueue.length;
  }
}

function animeQSRemoveChip(term) {
  _animeQSQueue = _animeQSQueue.filter(q => q.term !== term);
  _animeQSRenderChips();
  _animeRefreshActiveLine();
}
...
function animeOpenQS() {
  if (!_animeQSQueue.length) return;
  const session = document.getElementById('animeQSSession');
...
function animeQSPickMode(mode, btn) {
  _animeQSMode = mode;
  document.querySelectorAll('.anime-qs-mode-btn').forEach(b => b.classList.remove('active'));
  if (btn) btn.classList.add('active');
}

function animeQSStart() {
  // shuffle
  _animeQSDeck = [..._animeQSQueue];
  for (let i = _animeQSDeck.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1));
    [_animeQSDeck[i], _animeQSDeck[j]] = [_animeQSDeck[j], _animeQSDeck[i]];
  }
  _animeQSIdx   = 0;
  _animeQSWrong = [];
  _animeQSScore = 0;
...
function _animeQSRenderCard() {
  const word = _animeQSDeck[_animeQSIdx];
  if (!word) { _animeQSShowSummary(); return; }

  document.getElementById('animeQSProg').textContent = `${_animeQSIdx + 1}/${_animeQSDeck.length}`;
  document.getElementById('animeQSTerm').textContent = word.term;

  // hide all mode uis
  ['animeQSFlash','animeQSMCQ','animeQSWritten'].forEach(id => document.getElementById(id).classList.add('hidden'));
  document.getElementById('animeQSReveal').classList.add('hidden');

  if (_animeQSMode === 'flash') {
    document.getElementById('animeQSFlash').classList.remove('hidden');
    document.getElementById('animeQSFlipRow').classList.remove('hidden');
    document.getElementById('animeQSGradeRow').classList.add('hidden');
  } else if (_animeQSMode === 'mcq') {
    document.getElementById('animeQSMCQ').classList.remove('hidden');
    _animeQSBuildMCQ(word);
  } else if (_animeQSMode === 'written') {
    document.getElementById('animeQSWritten').classList.remove('hidden');
    const inp = document.getElementById('animeQSInput');
    inp.value = '';
    document.getElementById('animeQSWrittenFb').classList.add('hidden');
    document.getElementById('animeQSWrittenAns').textContent = word.meaning;
...
function animeQSFlip() {
  ...
  document.getElementById('animeQSFlipRow').classList.add('hidden');
  document.getElementById('animeQSGradeRow').classList.remove('hidden');
  animeSpeak(word.reading || word.term);
}

function _animeQSBuildMCQ(word) {
  const opts = document.getElementById('animeQSMCQ');
  opts.innerHTML = '';

  // distractors: other words in queue + vault
  const pool = [];
  _animeQSDeck.forEach(w => { if (w.term !== word.term && w.meaning) pool.push(w.meaning); });
  if (typeof lvLoad === 'function') {
    lvLoad().forEach(v => { if (v.term !== word.term && v.meaning && !pool.includes(v.meaning)) pool.push(v.meaning); });
  }
  ...
  choices.forEach(c => {
    ...
    btn.onclick = () => {
      ...
      opts.querySelectorAll('.anime-qs-mcq-opt').forEach(b => b.dataset.answered = '1');
      ...
      setTimeout(() => animeQSGrade(c === word.meaning ? 'know' : 'again'), 800);
    };
...
function animeQSWrittenCheck() {
  document.getElementById('animeQSWrittenFb').classList.remove('hidden');
}

function animeQSGrade(result) {
  if (result === 'know' || result === 'correct') { _animeQSScore++; if (typeof rewardStudy === 'function') rewardStudy(2); }
  else _animeQSWrong.push(_animeQSDeck[_animeQSIdx]);
  _animeQSIdx++;
  if (_animeQSIdx < _animeQSDeck.length) _animeQSRenderCard();
  else _animeQSShowSummary();
}

function _animeQSShowSummary() {
  document.getElementById('animeQSCard').classList.add('hidden');
  document.getElementById('animeQSSummary').classList.remove('hidden');
  document.getElementById('animeQSSumScore').textContent = _animeQSScore + ' / ' + _animeQSDeck.length;
  document.getElementById('animeQSSumDetail').textContent = _animeQSWrong.length + ' wrong · ' + (_animeQSDeck.length - _animeQSWrong.length) + ' correct';
  const retryBtn = document.getElementById('animeQSRetryBtn');
  if (retryBtn) retryBtn.style.display = _animeQSWrong.length ? '' : 'none';
}

function animeQSRetry() {
  _animeQSDeck = [..._animeQSWrong];
  _animeQSIdx = 0; _animeQSWrong = []; _animeQSScore = 0;
  document.getElementById('animeQSSummary').classList.add('hidden');
  document.getElementById('animeQSCard').classList.remove('hidden');
  _animeQSRenderCard();
}
...
function animeQSClose() {
  document.getElementById('animeQSSession').classList.add('hidden');
  const startBtn = document.getElementById('animeQSDockStart');
  if (startBtn && _animeQSQueue.length) startBtn.style.display = '';
}
...
function initAnimeTab() {
  _animeInitSynth();
  // reset to library view every time tab is opened
  document.getElementById('animeSeriesView').classList.add('hidden');
  document.getElementById('animePlayerView').classList.add('hidden');
  document.getElementById('animeLibraryView').classList.remove('hidden');
  animeRenderLibrary();
