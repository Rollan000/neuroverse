    ...
    episodes: [
      {
        ...
        thumbUrl: 'https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/anime/cotn_thumbnail.jpg',
        duration: 1404,
        opEnd: 90,
        locked: false,
      },
...
async function _anLookup(term) {
  ...
  if (ke) {
    const r = { term, reading: ke.readings?.[0]||ke.on?.[0]||ke.kun?.[0]||term, meaning: ke.meanings?.[0]||ke.meaning||'', jlpt: ke.jlpt||'', pos: 'kanji' };
    _AN.lookupCache.set(term, r); return r;
  }
  ...
  try {
    ...
    if (res.ok) {
      ...
      const f = j?.data?.[0];
      if (f) {
        const r = { term, reading: f.japanese?.[0]?.reading||term, meaning: f.senses?.[0]?.english_definitions?.join('; ')||'', jlpt: f.jlpt?.[0]||'', pos: f.senses?.[0]?.parts_of_speech?.join(', ')||'' };
        _AN.lookupCache.set(term, r); return r;
      }
    }
  } catch(_) {}
  ...
  // fallback
  const fb = { term, reading: term, meaning: '(unavailable)', jlpt: '', pos: '' };
  _AN.lookupCache.set(term, fb); return fb;
}
