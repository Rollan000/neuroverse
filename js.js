...
const ACHIEVEMENTS_DEF = [
  { id: 'ach_100_mcq',    name: 'Century',       desc: 'Answer 100 MCQ correctly',        target: 100,  track: 'mcq_correct_total',   reward: 20,  icon: 'trophy.png'   },
  { id: 'ach_50_trace',   name: 'Pensmith',      desc: 'Trace 50 kanji correctly',         target: 50,   track: 'trace_correct_total', reward: 15,  icon: 'quill.png'    },
  { id: 'ach_500_mcq',    name: 'Veteran',       desc: 'Answer 500 MCQ correctly',         target: 500,  track: 'mcq_correct_total',   reward: 50,  icon: 'trophy.png'   },
  { id: 'ach_streak_7',   name: 'Consistent',    desc: '7-day study streak',               target: 7,    track: 'day_streak',          reward: 15,  icon: 'fire.png'     },
  { id: 'ach_streak_30',  name: 'Devoted',       desc: '30-day study streak',              target: 30,   track: 'day_streak',          reward: 50,  icon: 'fire.png'     },
  { id: 'ach_vault_50',   name: 'Scholar',       desc: '50 items in learned vault',        target: 50,   track: 'vault_count',         reward: 20,  icon: 'chest.png'    },
  { id: 'ach_vault_100',  name: 'Archivist',     desc: '100 items in learned vault',       target: 100,  track: 'vault_count',         reward: 30,  icon: 'chest.png'    },
  { id: 'ach_review_100', name: 'Reviewer',      desc: 'Review 100 cards',                 target: 100,  track: 'review_graded_total', reward: 20,  icon: 'open_book.png'},
  { id: 'ach_dq_10',      name: 'On a Mission',  desc: 'Complete 10 daily quests',         target: 10,   track: 'dq_completed_total',  reward: 30,  icon: 'star.png'     },
  { id: 'ach_sac_1000',   name: 'Devoted Soul',  desc: 'Earn 1000 sacraments total',       target: 1000, track: 'sac_earned_total',    reward: 40,  icon: 'moon_tear.png'},
  { id: 'ach_flash_50',   name: 'Card Shepherd', desc: 'Mark 50 flashcards as known',      target: 50,   track: 'flash_known_total',   reward: 15,  icon: 'open_book.png'},
];
      ...
      shown.forEach(item => {
        ...
        div.innerHTML = `
          ...
          \${owned
            ? '<span class="shop-owned-tag">OWNED</span>'
            : canAfford
              ? '<button class="btn btn-secondary" style="font-size:12px;padding:6px 12px;white-space:nowrap;margin-left:10px">Buy ' + item.cost + ' <img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/moon_tear.png" style="width:11px;height:11px;vertical-align:-1px;image-rendering:pixelated" alt=""></button>'
...
const ANIME_CATALOG = [
  {
    ...
    shopItems: [
      { id: 'yoru_t1', name: '夜遊び',           type: 'Title',      cost: 80  },
      { id: 'yoru_t2', name: 'Call of the Night', type: 'Title',      cost: 120 },
      { id: 'yoru_b1', name: 'Midnight Street',   type: 'Background', cost: 200 },
      { id: 'yoru_c1', name: 'Night Palette',     type: 'Colour Set', cost: 150 },
    ],
  },
];
...
function _anRenderShop() {
  ...
  (a.shopItems||[]).forEach(item => {
    ...
    row.innerHTML = `
      <div class="anime-shop-row-info">
        <div class="anime-shop-row-name">${item.name}</div>
        <div class="anime-shop-row-type">${item.type}</div>
      </div>
      <div class="anime-shop-row-price">
        <img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/moon_tear.png" style="width:10px;height:10px;image-rendering:pixelated" alt="">
        ${item.cost}
      </div>
      <button class="btn ${canAfford?'btn-primary':'btn-secondary'}" style="font-size:11px;padding:6px 12px;${canAfford?'':'opacity:.45'}" ${canAfford?'':'disabled'}
        onclick="anBuyItem('${item.id}',${item.cost},'${item.name.replace(/'/g,"\\'")}')">
...
function anBuyItem(id, cost, name) {
  const d = _anCoins();
  if ((d[_AN.animeId]||0) < cost) return;
