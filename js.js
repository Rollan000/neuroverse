...
function placeNavbarWidget(idx) {
  ...
  if (span > 1) { rewardPopup('Only 1-row widgets fit in the navbar strip!'); return; }
  for (let i = 0; i < NAV_SLOTS; i++) {
    if (navbarSlots['nav-' + i] === bagSelectedWidgetId) { rewardPopup('Already in the navbar!'); return; }
  }
  ...
  rewardPopup('Widget added to navbar!');
}
  ...
  const navEditorHTML = `
    <div class="wgrid-nav-editor">
      <div class="wgrid-col-label" style="margin-bottom:8px">
        Navbar Strip <span style="font-size:8px;opacity:0.5">(1-row widgets only &middot; 3 slots)</span>
      </div>
      <div class="wgrid-nav-cells">${navCells}</div>
      ${tooTall ? `<div style="font-size:11px;color:var(--muted);margin-top:6px;opacity:0.7">Widget too tall for navbar slots</div>` : ''}
    </div>`;
...
function revSessionEnd() {
  ...
  const tickets = Math.floor(seen / 2);
  if (seen > 0) rewardStudy(tickets);
  updateReviewBadge();
  ...
  openSessionSummary({
    ...
    ticketsEarned: 0, // already rewarded above
    weakCount: again,
  });
}
...
// Shows on first page load if overdue review cards exist
// ============================================================

let qrevCards = [];
let qrevIdx   = 0;
let qrevDone  = { again:0, hard:0, good:0, easy:0 };
const QREV_MAX = 5;
...
function initQuickReview() {
  ...
  const lastShown = localStorage.getItem('qrevLastShown');
  const today = new Date().toDateString();
  if (lastShown === today) return;

  qrevCards = q.slice(0, QREV_MAX);
  qrevIdx = 0;
  qrevDone = { again:0, hard:0, good:0, easy:0 };

  const overlay = document.getElementById('quickReviewOverlay');
  if (!overlay) return;
  document.getElementById('qrevSub').textContent =
    q.length + ' card' + (q.length !== 1 ? 's' : '') + ' due — ' + QREV_MAX + '-card warm-up!';
...
function qrevShowCard() {
  ...
  document.getElementById('qrevQuestion').textContent = card.q || card.char || '';
  document.getElementById('qrevMeta').textContent = card.unit ? card.unit.toUpperCase() : '';
  qrevUpdateDots();
}

function qrevTap() {
  // Called onclick of qrevFront
  const card = qrevCards[qrevIdx];
  document.getElementById('qrevAnswer').textContent = card.a || card.meaning || '';
...
function qrevGrade(g) {
  ...
  qrevDone[labels[g]]++;
  // If good/easy, remove from queue
  if (g >= 2) reviewQueueRemove(qrevCards[qrevIdx].q);
  qrevIdx++;
  if (qrevIdx < qrevCards.length) {
    qrevShowCard();
  } else {
    qrevFinish();
  }
...
// Wire click-to-flip on front face
document.addEventListener('DOMContentLoaded', () => {
  const front = document.getElementById('qrevFront');
  if (front) front.addEventListener('click', qrevTap);

  // Trigger after a short delay so page is settled
...
// Reusable modal called after any session ends
// ============================================================
...
function openSessionSummary({ title, emoji, sub, rows, ticketsEarned, weakCount }) {
  ...
  // Stats rows
  const statsEl = document.getElementById('ssmStats');
  ...
  if (ticketsEarned > 0) {
    rewardStudy(ticketsEarned);
    rewardEl.innerHTML = `<div class="ssm-reward-pill">+${ticketsEarned} 🎟️ tickets earned</div>`;
  } else {
    rewardEl.innerHTML = '';
  }
...
// Hiragana / Katakana / Mixed · Practice / Timed / Survival
// SR per character · Weak kana priority · Streak tracking
// Milestone rewards · Instant retry on wrong
// ============================================================

// ── State ──────────────────────────────────────────────────
const KANA_TIMER_SECS    = 60;
const KANA_SURVIVAL_LIVES = 3;
const KANA_PROMPT_SECS   = 6;   // seconds per prompt in practice/timed

let kanaScript   = 'hiragana';  // 'hiragana' | 'katakana' | 'mixed'
let kanaGameMode = 'practice';  // 'practice' | 'timed' | 'survival'
let kanaPool     = [];          // shuffled kana for current session
let kanaPromptIdx = 0;
let kanaCorrect  = 0;
let kanaWrong    = 0;
let kanaStreak   = 0;
...
let kanaTimerInt = null;        // setInterval handle
let kanaPromptTimer = null;     // prompt countdown
let kanaPromptStart = 0;
let kanaRafHandle    = null;
let kanaMissedChars  = [];      // chars missed this session
let kanaSessionActive = false;
let kanaRetryPending  = false;  // instant retry after wrong answer
let kanaCurrentKana  = null;    // current kana object being shown

// ── SR storage ──────────────────────────────────────────────
function kanaSrLoad() {
  try { return JSON.parse(localStorage.getItem('kana_sr') || '{}'); } catch(e) { return {}; }
}
function kanaSrSave(sr) {
  try { localStorage.setItem('kana_sr', JSON.stringify(sr)); } catch(e) {}
}
function kanaSrGet(romaji) {
  const sr = kanaSrLoad();
  return sr[romaji] || { romaji, correct:0, wrong:0, streak:0, lastSeen:0, weight:1 };
}
function kanaSrRecord(romaji, wasCorrect) {
  const sr = kanaSrLoad();
  const entry = sr[romaji] || { romaji, correct:0, wrong:0, streak:0, lastSeen:0, weight:1 };
  entry.lastSeen = Date.now();
  if (wasCorrect) {
    entry.correct++;
    entry.streak++;
    // Lower weight (easier) — min 0.3
    entry.weight = Math.max(0.3, entry.weight * 0.85);
  } else {
    entry.wrong++;
    entry.streak = 0;
    // Raise weight (harder to forget) — max 5
    entry.weight = Math.min(5, entry.weight * 1.6 + 0.5);
  }
  sr[romaji] = entry;
  kanaSrSave(sr);
}
...
function kanaBuildPool() {
  ...
  if (kanaScript === 'hiragana' || kanaScript === 'mixed') {
    ...
    src = src.concat((APP_KANA_COMBOS || []).filter(k => k.script === 'hiragana').map(k => ({ ...k, script:'hira' })));
  }
  if (kanaScript === 'katakana' || kanaScript === 'mixed') {
    src = src.concat((APP_KATAKANA || []).map(k => ({ ...k, script:'kata' })));
    src = src.concat((APP_KANA_COMBOS || []).filter(k => k.script === 'katakana').map(k => ({ ...k, script:'kata' })));
  }
  // De-dupe by kana character
  const seen = new Set();
  src = src.filter(k => { if (seen.has(k.kana)) return false; seen.add(k.kana); return true; });
  return src;
}

// Weighted-random pick (weak kana get picked more)
function kanaPickPrompt(pool, lastRomaji) {
  const sr = kanaSrLoad();
  // Weight each entry
  const weighted = pool.map(k => {
    const s = sr[k.romaji] || {};
    const w = s.weight || 1;
    // Penalise recently seen to avoid repeats
    const timePenalty = s.lastSeen && (Date.now() - s.lastSeen < 8000) ? 0.05 : 1;
    const repeatPenalty = k.romaji === lastRomaji ? 0.01 : 1;
    return { k, w: w * timePenalty * repeatPenalty };
  });
...
// ── Grid rendering ──────────────────────────────────────────
function kanaRenderGrid(pool, correctKana) {
  const grid = document.getElementById('kanaGrid');
  if (!grid) return;
  // Shuffle pool for display (don't show in predictable order)
  const display = [...pool].sort(() => Math.random() - 0.5);
  grid.innerHTML = display.map(k =>
    `<button class="kana-cell" data-kana="${k.kana}" data-romaji="${k.romaji}"
      onclick="kanaAnswer('${k.romaji}','${correctKana}')">${k.kana}</button>`
  ).join('');
}
...
function kanaUpdateStartScreen() {
  const icon = document.getElementById('kanaStartIcon');
  const title = document.getElementById('kanaStartTitle');
  const sub = document.getElementById('kanaStartSub');
  const preview = document.getElementById('kanaSrPreview');
  if (!icon) return;
  const scripts = { hiragana:'あ', katakana:'ア', mixed:'あ/ア' };
  icon.textContent = scripts[kanaScript] || 'あ';
  const modeDesc = {
    practice: 'Click the correct kana — no pressure, just build accuracy.',
    timed: '60 seconds · answer as many as you can!',
    survival: `${KANA_SURVIVAL_LIVES} lives · one wrong answer costs a life.`,
  };
  title.textContent = kanaScript === 'mixed' ? 'Hiragana + Katakana' : kanaScript.charAt(0).toUpperCase() + kanaScript.slice(1);
  sub.textContent = modeDesc[kanaGameMode] || '';

  // SR preview: show weakest kana
  const pool = kanaBuildPool();
  const sr = kanaSrLoad();
  const weak = pool
    .map(k => ({ k, w: (sr[k.romaji] || {}).weight || 1 }))
    .filter(x => x.w > 1.2)
    .sort((a,b) => b.w - a.w)
    .slice(0, 8);
  if (preview) {
    preview.innerHTML = weak.length
...
function kanaStartSession() {
  ...
  kanaMissedChars = [];
  kanaSessionActive = true;
  kanaRetryPending = false;
  kanaCurrentKana = null;

  document.getElementById('kanaScore').textContent = '0';
  document.getElementById('kanaStreakVal').textContent = '0';
  document.getElementById('kanaLivesVal').textContent = kanaLives;
  document.getElementById('kanaTimerVal').textContent = kanaTimeLeft;
  document.getElementById('kanaFeedback').textContent = '';
...
function kanaNextPrompt() {
  ...
  const kana = kanaPickPrompt(kanaPool, _kanaLastRomaji);
  kanaCurrentKana = kana;
  _kanaLastRomaji = kana.romaji;

  document.getElementById('kanaPromptText').textContent = kana.romaji;
  kanaRenderGrid(kanaPool, kana.romaji);
  document.getElementById('kanaFeedback').textContent = '';
  document.getElementById('kanaFeedback').className = 'kana-feedback';

  // Animate ring countdown
  const ring = document.getElementById('kanaRingFill');
  const CIRC = 213.6;
  const duration = kanaGameMode === 'timed' ? KANA_PROMPT_SECS * 1000 : KANA_PROMPT_SECS * 1200;
  const start = performance.now();
  kanaPromptStart = start;
  if (ring) {
    function animRing(now) {
      const elapsed = now - start;
      const pct = Math.min(1, elapsed / duration);
      ring.style.strokeDashoffset = (CIRC * pct).toString();
      if (pct < 1 && kanaSessionActive) {
        kanaRafHandle = requestAnimationFrame(animRing);
      } else if (pct >= 1 && kanaSessionActive && !kanaRetryPending) {
...
function kanaAnswer(selectedRomaji, correctRomaji) {
  ...
  if (wasCorrect) {
    ...
    kanaRetryPending = false;
    document.getElementById('kanaScore').textContent = kanaCorrect;
    document.getElementById('kanaStreakVal').textContent = kanaStreak;
    fb.textContent = '✓ ' + kanaCurrentKana.kana + ' = ' + correctRomaji;
    fb.className = 'kana-feedback kana-feedback-correct';
    // Milestone reward
    kanaCheckMilestone();
    ...
    fb.textContent = '✗  ' + correctRomaji + ' = ' + kanaCurrentKana.kana;
    fb.className = 'kana-feedback kana-feedback-wrong';

    if (kanaGameMode === 'survival') {
      kanaLives--;
      document.getElementById('kanaLivesVal').textContent = kanaLives;
      if (kanaLives <= 0) {
        setTimeout(kanaEndSession, 700);
        return;
      }
    }
    // Instant retry — show same prompt once more
    if (!kanaRetryPending) {
      kanaRetryPending = true;
      setTimeout(kanaNextPrompt, 900);
    } else {
      kanaRetryPending = false;
      setTimeout(kanaNextPrompt, 500);
    }
  }
}
...
function kanaTimeoutAnswer() {
  ...
  kanaStreak = 0;
  document.getElementById('kanaStreakVal').textContent = '0';
  if (kanaCurrentKana && !kanaMissedChars.includes(kanaCurrentKana.romaji)) {
    kanaMissedChars.push(kanaCurrentKana.romaji);
  }
  if (kanaGameMode === 'survival') {
    kanaLives--;
    document.getElementById('kanaLivesVal').textContent = kanaLives;
  ...
  fb.className = 'kana-feedback kana-feedback-wrong';
  setTimeout(kanaNextPrompt, 700);
}

// ── Milestones ──────────────────────────────────────────────
const KANA_MILESTONES = [10, 25, 50, 100];
let kanaMilestoneSeen = new Set();

function kanaCheckMilestone() {
  for (const m of KANA_MILESTONES) {
    if (kanaCorrect === m && !kanaMilestoneSeen.has(m)) {
      kanaMilestoneSeen.add(m);
      rewardStudy(m >= 50 ? 5 : 3);
      rewardPopup('🎉 ' + m + ' correct! Bonus +' + (m >= 50 ? 5 : 3) + ' 🎟️');
      // Flash the streak display
...
function kanaEndSession() {
  ...
  const statsEl = document.getElementById('kanaEndStats');
  statsEl.innerHTML = `
  ...
  if (kanaMissedChars.length > 0) {
    const pool = kanaBuildPool();
    const missedKana = pool.filter(k => kanaMissedChars.includes(k.romaji));
    weakEl.innerHTML = '<div class="kana-weak-label">Missed:</div>' +
      missedKana.map(k => `<span class="kana-sr-chip kana-sr-chip-miss">${k.kana} <em>${k.romaji}</em></span>`).join('');
    if (reviewBtn) reviewBtn.style.display = '';
  } else {
    weakEl.innerHTML = '';
    if (reviewBtn) reviewBtn.style.display = 'none';
...
function kanaReviewMissed() {
  // Push missed kana to review queue and switch to review tab
  const pool = kanaBuildPool();
  kanaMissedChars.forEach(romaji => {
    const k = pool.find(x => x.romaji === romaji);
    if (k) reviewQueuePush({ q: k.kana, a: k.romaji, unit: kanaScript, diff: 'easy' });
  });
  switchMode('review', null);
  setSbActive(document.querySelector('.sb-nav-item[data-mode="review"]'));
}
