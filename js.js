...
function openSessionSummary({ title, emoji, sub, rows, ticketsEarned, weakCount }) {
  ...
  if (ticketsEarned > 0) { rewardStudy(ticketsEarned); rewardEl.innerHTML = `<div class="ssm-reward-pill">+${ticketsEarned} 🎟️ tickets earned</div>`; }
...
function mcqGameOver() {
  ...
  openSessionSummary({
    ...
    rows: [
      ...
      { label: 'Speed Bonus', val: mcqSpeedBonus + ' 🎟️' },
    ],
    ticketsEarned: 0,
...
function traceGotIt() {
  ...
  if (kanji) {
    ...
    rewardStudy(2);
    rewardPopup('✓ ' + kanji.char + ' mastered! +2 🎟️');
  }
  traceIdx++;
  traceRenderCard();
}
...
kanaEndSession = function(isTimeout) {
  ...
  if (window._kanaDailySession) {
    ...
    if (!alreadyDone) {
      ...
      rewardPopup('📅 Daily Challenge complete! +5 bonus 🎟️');
      // Mark button as done
      ...
      setTimeout(() => {
        openSessionSummary({
          ...
          rows: [
            ...
            { label: 'Reward',        val: '+' + reward + ' 🎟️', color: 'var(--correct)' },
          ],
...
// ============================================================
// TRACE IMPROVEMENTS — Info flash, toggles, grading, restart
// ============================================================

// Info visibility state
const traceInfoShow = { meaning: true, reading: true, hiragana: true, flash: true };

function traceToggleInfo(key, el) {
  traceInfoShow[key] = !traceInfoShow[key];
  el.classList.toggle('active', traceInfoShow[key]);
  _applyTraceInfoVisibility();
}

function _applyTraceInfoVisibility() {
  const m = document.getElementById('traceKanjiMeaning');
  const r = document.getElementById('traceKanjiReadings');
  const h = document.getElementById('traceKanjiHiragana');
  if (m) m.style.opacity = traceInfoShow.meaning  ? '1' : '0.08';
  if (r) r.style.opacity = traceInfoShow.reading  ? '1' : '0.08';
  if (h) h.style.opacity = traceInfoShow.hiragana ? '1' : '0.08';
  // Also update flash overlay elements visibility
  const fm = document.getElementById('traceFlashMeaning');
  const fr = document.getElementById('traceFlashReading');
  const fh = document.getElementById('traceFlashHiragana');
  if (fm) fm.style.display = traceInfoShow.meaning  ? '' : 'none';
  if (fr) fr.style.display = traceInfoShow.reading  ? '' : 'none';
  if (fh) fh.style.display = traceInfoShow.hiragana ? '' : 'none';
}

// Flash overlay timer
let _traceFlashTimer = null;

function traceShowFlashOverlay(kanji, stageLabel, onDone) {
  const overlay = document.getElementById('traceFlashOverlay');
  if (!overlay || !traceInfoShow.flash) { onDone && onDone(); return; }
  document.getElementById('traceFlashChar').textContent    = kanji.char;
  document.getElementById('traceFlashStageLabel').textContent = stageLabel;
  document.getElementById('traceFlashMeaning').textContent = kanji.meaning || '';
  const onReading = kanji.on  ? 'On: ' + kanji.on  : '';
  const kunReading = kanji.kun ? 'Kun: ' + kanji.kun : '';
  document.getElementById('traceFlashReading').textContent  = [onReading, kunReading].filter(Boolean).join('  ·  ');
  document.getElementById('traceFlashHiragana').textContent = kanji.kun || '';
  _applyTraceInfoVisibility();
  overlay.style.display = 'flex';
  // Auto-dismiss after 2.2s or on tap
  if (_traceFlashTimer) clearTimeout(_traceFlashTimer);
  const dismiss = () => {
    overlay.style.display = 'none';
    overlay.removeEventListener('click', dismiss);
    if (_traceFlashTimer) { clearTimeout(_traceFlashTimer); _traceFlashTimer = null; }
    onDone && onDone();
  };
  overlay.addEventListener('click', dismiss);
  _traceFlashTimer = setTimeout(dismiss, 2400);
}

// Patch traceRenderCard to show flash overlay at start of each new kanji
...
traceRenderCard = function() {
  ...
  // Fill hiragana field in persistent info bar
  const hiraganaEl = document.getElementById('traceKanjiHiragana');
  if (hiraganaEl) hiraganaEl.textContent = kanji.kun || '';
  ...
  traceShowFlashOverlay(kanji, 'STAGE 1 — STUDY', () => {
    _origTraceRenderCard();
    _applyTraceInfoVisibility();
  });
};

// Patch traceNextStage to also flash info overlay
...
traceNextStage = function() {
  ...
  if (traceStage < 2) {
    traceStage++;
    const stageLabels = ['STAGE 1 — STUDY', 'STAGE 2 — RECALL', 'STAGE 3 — MEMORY'];
    traceShowFlashOverlay(kanji, stageLabels[traceStage], () => {
      traceUpdateStageUI();
      traceClearCanvas();
    });
  }
...
function traceGrade(grade) {
  const gradeRow = document.getElementById('traceGradeRow');
  if (gradeRow) gradeRow.style.display = 'none';
  // Optionally save grade to SRS (hook into existing DS/Rev SM2 if available)
  const kanji = tracePool && tracePool[traceIdx - 1]; // already advanced
  if (kanji && typeof dsUpdateSRS === 'function') {
    try { dsUpdateSRS('trace_' + kanji.id, grade); } catch(e) {}
  }
  // Continue to next card (traceGotIt already advanced traceIdx)
  traceRenderCard();
}

// Patch traceGotIt to show grading row instead of immediately advancing
// We need to intercept BEFORE the chain of patches in the rest of the file
const _baseTraceGotIt = traceGotIt;
traceGotIt = function() {
  const gradeRow = document.getElementById('traceGradeRow');
  // Show grade row first, then advance after grade
  // We'll show grade row and defer card advance
  const kanji = tracePool && tracePool[traceIdx];
  if (!kanji) { _baseTraceGotIt(); return; }
  // Do rewards/vault but not card advance yet
  traceGotItCnt++;
  const vault = JSON.parse(localStorage.getItem('learnedVault') || '[]');
  if (!vault.some(v => v.term === kanji.char && v.type === 'trace')) {
    vault.push({ id: 'tr_' + kanji.id, type: 'trace', term: kanji.char, reading: kanji.kun, meaning: kanji.meaning, dateAdded: Date.now(), source: 'trace' });
  ...
  if (typeof rewardPopup === 'function') rewardPopup('✓ ' + kanji.char + ' mastered! +2 🎟️');
  traceIdx++;
  ...
  else traceRenderCard(); // fallback if no grading row
};

// Restart from beginning (not shuffle-restart)
