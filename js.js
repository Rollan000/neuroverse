...
const CURSOR_ITEMS = [
  ...
  },
  {
    id:       'cursor_konata',
    name:     '💙 Konata Dance',
    file:     'konatadance1.ani',
    cost:     350,
    rarity:   'rare',
    hotspot:  [0, 0],
    desc:     'Lucky Star shuffle — two variants available.',
    variants: [
      { label: 'Variant 1', file: 'konatadance1.ani' },
      { label: 'Variant 2', file: 'konatadance2.ani' },
    ],
  },
  {
    id:       'cursor_flame',
    name:     '🔥 Legendary Flame',
    file:     'legendaryflamingepiccursor1.ani',
    cost:     1500,
    rarity:   'legendary',
    hotspot:  [16, 16],
    desc:     'An epic flaming cursor. Two blazing variants.',
    variants: [
      { label: 'Blaze I',  file: 'legendaryflamingepiccursor1.ani' },
      { label: 'Blaze II', file: 'legendaryflamingepiccursor2.ani' },
    ],
  },
  {
    id:       'cursor_naruto',
    name:     '🍃 Naruto Stride',
    file:     'narutostride.ani',
    cost:     400,
    rarity:   'rare',
    hotspot:  [0, 0],
    desc:     'Believe it.',
  ...
  },
  {
    id:       'cursor_yinyang',
    name:     '☯️ Yin Yang',
    file:     'yinyang2.ani',    // .ani version (has 2 suffix = selectable)
    cost:     300,
    rarity:   'uncommon',
    hotspot:  [16, 16],
    desc:     'Balance. Two variants available.',
    variants: [
      { label: 'Static',   file: 'yinyang.cur'  },
      { label: 'Animated', file: 'yinyang2.ani' },
    ],
  },
];

// Active variant selection per cursor (persisted)
// shape: { [cursorId]: variantIndex }
...
function applyCursorCosmetic(cursorId, variantIdx) {
  ...
  const url  = CURSOR_R2 + file;
  const [hx, hy] = def.hotspot || [0, 0];

  // Apply via injected style — url() with hotspot, auto fallback
  styleEl.textContent = `
    *, *::before, *::after {
      cursor: url('${url}') ${hx} ${hy}, auto !important;
    }
  `;
...
if (typeof equipItem !== 'undefined') {
  ...
  equipItem = function(itemId) {
    ...
    if (item && item.type === 'cursor') {
      applyCursorCosmetic(item.data || item.id);
