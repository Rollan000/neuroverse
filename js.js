...
// ════════════════════════════════════════════════════════════════
// PHASE 5 — ANIME TAB ENGINE
// ════════════════════════════════════════════════════════════════

// ── State vars ──────────────────────────────────────────────────
let _animeCurrentId   = 'yofukashi';
let _animeCurrentEp   = null;
let _animeSegments    = [];        // loaded episode JSON segments
let _animeLookupCache = new Map(); // session word cache
let _animeSynthVoice  = null;
let _animeLastCoinTick = 0;
let _animeLoopActive  = false;
let _animeQuickStudyQueue = [];
let _animeQuickStudyMode  = 'flash';
let _animeQSIndex   = 0;
let _animeQSWrong   = [];
let _animeQSCorrect = 0;
let _animeHubSet    = [];
let _animeSubHistoryArr = [];
let _animeCurrentSegIdx = -1;
let _animePopupEntry = null;   // current word popup entry
    ...
    episodes: [
      {
        ...
        subtitleUrl: 'https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/data/episode1.json',
        thumbnailUrl: '',
        opEnd: 90,       // seconds — skip OP button
        duration: 1404,  // ~23.4 min
        locked: false,
      },
    ],
  },
...
const ANIME_SHOP_ITEMS = {
  yofukashi: [
    { id: 'yorutitle1', name: '夜遊び', type: 'title', cost: 80 },
    { id: 'yorutitle2', name: 'Call of the Night', type: 'title', cost: 120 },
    { id: 'yorubg1',    name: 'Midnight Street', type: 'background', cost: 200 },
    { id: 'yorucolor1', name: 'Night Palette', type: 'colorset', cost: 150 },
  ],
};

// ── localStorage helpers ─────────────────────────────────────────
function _animeWatchedLoad() {
  try { return JSON.parse(localStorage.getItem('animeWatched') || '{}'); } catch { return {}; }
}
function _animeWatchedSave(d) { localStorage.setItem('animeWatched', JSON.stringify(d)); }

function _animeProgressLoad() {
  try { return JSON.parse(localStorage.getItem('animeProgress') || '{}'); } catch { return {}; }
}
function _animeProgressSave(d) { localStorage.setItem('animeProgress', JSON.stringify(d)); }

function _animeCoinsLoad() {
  try { return JSON.parse(localStorage.getItem('animeCoins') || '{}'); } catch { return {}; }
}
function _animeCoinsSave(d) { localStorage.setItem('animeCoins', JSON.stringify(d)); }

function animeCoinsAdd(animeId, n) {
  const d = _animeCoinsLoad();
  d[animeId] = (d[animeId] || 0) + n;
  _animeCoinsSave(d);
  const el = document.getElementById('animeShopBalance');
  if (el) el.textContent = d[animeId] || 0;
}

function animeCoinsGet(animeId) {
  return _animeCoinsLoad()[animeId] || 0;
}

// ── Speech synthesis ─────────────────────────────────────────────
function _animeInitSynth() {
  if (!window.speechSynthesis) return;
  const pick = () => {
    const voices = speechSynthesis.getVoices();
    _animeSynthVoice = voices.find(v => /ja[-_]JP/i.test(v.lang)) || voices.find(v => /ja/i.test(v.lang)) || null;
  };
  pick();
  speechSynthesis.onvoiceschanged = pick;
}

function animeSpeak(reading) {
  if (!reading || !window.speechSynthesis) return;
  const utt = new SpeechSynthesisUtterance(reading);
  utt.lang = 'ja-JP';
  if (_animeSynthVoice) utt.voice = _animeSynthVoice;
  utt.rate = 0.9;
...
// ── Word lookup ──────────────────────────────────────────────────
async function animeWordLookup(term) {
  if (_animeLookupCache.has(term)) return _animeLookupCache.get(term);

  // Step 1: scan kanji DB
  const db = window.APP_KANJI_DB || [];
  const kEntry = db.find(k => k.char === term || (k.exampleWords||[]).some(w => w.word === term));
  if (kEntry) {
    const result = {
      term,
      reading: kEntry.readings?.[0] || kEntry.on?.[0] || kEntry.kun?.[0] || '',
      meaning: kEntry.meanings?.[0] || kEntry.meaning || '',
      jlpt: kEntry.jlpt || '',
      pos: 'kanji',
    };
    _animeLookupCache.set(term, result);
    return result;
  }

  // Step 2: Cloudflare Worker → Jisho
  try {
    const res = await fetch(`https://neuroverse.uk/api/jisho?word=${encodeURIComponent(term)}`);
    if (res.ok) {
      ...
      if (first) {
        const result = {
          term,
          reading: first.japanese?.[0]?.reading || term,
          meaning: first.senses?.[0]?.english_definitions?.join('; ') || '',
          jlpt: first.jlpt?.[0] || '',
          pos: first.senses?.[0]?.parts_of_speech?.join(', ') || '',
        };
        _animeLookupCache.set(term, result);
        return result;
      }
  ...
  // Step 3: Fallback — return bare entry
  const fallback = { term, reading: term, meaning: '(definition unavailable)', jlpt: '', pos: '' };
  _animeLookupCache.set(term, fallback);
  return fallback;
}

// ── animeWatched helpers ─────────────────────────────────────────
function animeWatchedAdd(animeId, entry) {
  const d = _animeWatchedLoad();
  if (!d[animeId]) d[animeId] = [];
  const existing = d[animeId].find(e => e.term === entry.term && e.episodeId === entry.episodeId);
  if (!existing) d[animeId].push(entry);
  _animeWatchedSave(d);
}

function animeWatchedMarkAdded(animeId, term, episodeId) {
  const d = _animeWatchedLoad();
  if (!d[animeId]) return;
  const e = d[animeId].find(x => x.term === term && x.episodeId === episodeId);
  if (e) { e.addedToStudy = true; _animeWatchedSave(d); }
}

// ── Vault save ───────────────────────────────────────────────────
function animeAddToVault(entry) {
  lvAddToVault({
    id: 'anime_' + entry.term + '_' + entry.episodeId,
    type: 'word',
    term: entry.term,
    reading: entry.reading,
    meaning: entry.meaning,
    source: 'anime',
    animeId: entry.animeId || _animeCurrentId,
    episodeId: entry.episodeId || _animeCurrentEp,
    dateAdded: Date.now(),
  });
  animeWatchedMarkAdded(entry.animeId || _animeCurrentId, entry.term, entry.episodeId || _animeCurrentEp);
  animeCoinsAdd(entry.animeId || _animeCurrentId, 5);
  if (typeof dailyQuestsTrack === 'function') dailyQuestsTrack('anime_word_saved', 1);
  if (typeof achievementsTrack === 'function') achievementsTrack('vault_count', 1);
}

// ── HOME VIEW ────────────────────────────────────────────────────
function animeRenderHome() {
  _animeCurrentEp = null;
  document.getElementById('animeHome').classList.remove('hidden');
  document.getElementById('animeEpisodeView').classList.add('hidden');
  document.getElementById('animeShopPanel').classList.add('hidden');

  const anime = ANIME_CATALOG.find(a => a.id === _animeCurrentId);
  if (!anime) return;

  document.getElementById('animeHomeTitle').textContent = anime.title;

  // Continue watching
  const prog = _animeProgressLoad();
  const inProg = anime.episodes.find(ep => {
    const p = prog[_animeCurrentId + '_' + ep.id];
    return p && p.lastTime > 5 && !p.completed;
  });
  const contStrip = document.getElementById('animeContinueStrip');
  if (inProg) {
    const p = prog[_animeCurrentId + '_' + inProg.id];
    document.getElementById('animeContinueEpLabel').textContent = 'EP 0' + inProg.num;
    document.getElementById('animeContinueTime').textContent = _animeFmtTime(p.lastTime);
    contStrip.dataset.epId = inProg.id;
    contStrip.classList.remove('hidden');
  } else {
    contStrip.classList.add('hidden');
  }

  // Episode grid
  const grid = document.getElementById('animeEpGrid');
  grid.innerHTML = '';
  const watched = _animeWatchedLoad();
  anime.episodes.forEach(ep => {
    const p = prog[_animeCurrentId + '_' + ep.id] || { pct: 0 };
    const wordCount = (watched[_animeCurrentId] || []).filter(w => w.episodeId === ep.id).length;
    const card = document.createElement('div');
    card.className = 'anime-ep-card' + (ep.locked ? ' locked' : '');
    card.innerHTML = `
      <img class="anime-ep-thumb" src="${ep.thumbnailUrl || ''}" onerror="this.style.display='none'" alt="">
      <div class="anime-ep-info">
        <div class="anime-ep-num">EP ${String(ep.num).padStart(2,'0')}</div>
        <div class="anime-ep-name">${ep.title}</div>
        <div class="anime-ep-meta">
          <div class="anime-ep-progress"><div class="anime-ep-progress-fill" style="width:${Math.round((p.pct||0)*100)}%"></div></div>
          <span>${Math.round((p.pct||0)*100)}% watched</span>
          <span>· ${wordCount} words</span>
        </div>
      </div>
    `;
    if (!ep.locked) card.onclick = () => animeOpenEpisode(ep.id);
    grid.appendChild(card);
  });

  // Hub update
  animeHubUpdateSet();
  animeSetHomeTab('hub', document.querySelector('.anime-home-tab'));
}

function animeSetHomeTab(tab, el) {
  document.querySelectorAll('.anime-home-tab').forEach(b => b.classList.remove('active'));
  if (el) el.classList.add('active');
  document.getElementById('animeHomeHub').classList.toggle('hidden', tab !== 'hub');
  document.getElementById('animeHomeEncountered').classList.toggle('hidden', tab !== 'encountered');
  document.getElementById('animeHomeStudying').classList.toggle('hidden', tab !== 'studying');
  if (tab === 'encountered') animeRenderEncountered();
  if (tab === 'studying') animeRenderStudying();
}
...
async function animeOpenEpisode(epId) {
  ...
  _animeCurrentEp = epId;
  _animeSubHistoryArr = [];
  _animeCurrentSegIdx = -1;

  document.getElementById('animeHome').classList.add('hidden');
  document.getElementById('animeEpisodeView').classList.remove('hidden');
  document.getElementById('animeEpViewTitle').textContent = `EP ${String(ep.num).padStart(2,'0')} — ${ep.title}`;

  // Load subtitles
  _animeSegments = [];
  try {
    let data;
    // Try R2 URL first; fall back to uploaded episode1.json if epId matches
    if (ep.subtitleUrl) {
      const r = await fetch(ep.subtitleUrl);
      if (r.ok) data = await r.json();
    }
    if (!data && epId === 'yofukashi_ep1') {
      // Load local upload as fallback (only works if served)
      const r2 = await fetch('episode1.json');
      if (r2.ok) data = await r2.json();
    }
    if (data?.segments) {
      _animeSegments = data.segments.filter(s => s.no_speech_prob == null || s.no_speech_prob < 0.5);
    }
  } catch(e) { console.warn('[Anime] Subtitle load failed', e); }
  ...
  const prog = _animeProgressLoad();
  const saved = prog[_animeCurrentId + '_' + epId];
  if (saved?.lastTime > 5) {
    video.currentTime = saved.lastTime;
  }

  video.ontimeupdate = () => _animeOnTimeUpdate(video.currentTime, ep);
  video.onplay = () => {
    document.getElementById('animePlayBtn').textContent = '⏸';
    document.getElementById('animeControls').parentElement.classList.add('controls-visible');
    setTimeout(() => document.getElementById('animeControls').parentElement.classList.remove('controls-visible'), 2000);
  };
  video.onpause = () => { document.getElementById('animePlayBtn').textContent = '▶'; };
  video.ondurationchange = () => _animeUpdateSeek();

  // Subtitle display
  document.getElementById('animeSubPrev').textContent = '';
  document.getElementById('animeSubCurrent').textContent = '—';
  document.getElementById('animeSubNext').textContent = '';
  document.getElementById('animeSubHistoryList').innerHTML = '';

  // QS badge reset
  _animeQuickStudyQueue = [];
  _animeUpdateQSBadge();
}

function animeGoHome() {
  const video = document.getElementById('animeVideo');
  if (video) { video.pause(); video.src = ''; }
  animeRenderHome();
}

function animeResume() {
  const strip = document.getElementById('animeContinueStrip');
  if (strip.dataset.epId) animeOpenEpisode(strip.dataset.epId);
}

// ── Subtitle engine ──────────────────────────────────────────────
function _animeBinarySearch(t) {
  let lo = 0, hi = _animeSegments.length - 1;
  while (lo <= hi) {
    const mid = (lo + hi) >> 1;
    const seg = _animeSegments[mid];
...
function _animeOnTimeUpdate(t, ep) {
  // Seek bar
  _animeUpdateSeek();

  // Loop line
  if (_animeLoopActive && _animeCurrentSegIdx >= 0) {
    const seg = _animeSegments[_animeCurrentSegIdx];
    if (seg && t >= seg.end) {
      document.getElementById('animeVideo').currentTime = seg.start;
      return;
    }
  }

  // Subtitle sync
  const idx = _animeBinarySearch(t);
  if (idx !== -1 && idx !== _animeCurrentSegIdx) {
    _animeCurrentSegIdx = idx;
    _animeRenderSubtitle(idx);
  }

  // Save progress
  if (!ep) return;
  const video = document.getElementById('animeVideo');
  const dur = video.duration || ep.duration;
  const pct = dur ? t / dur : 0;
  const progKey = _animeCurrentId + '_' + ep.id;
  const prog = _animeProgressLoad();
  const was = prog[progKey] || {};
  prog[progKey] = { pct, lastTime: t, completed: was.completed || (pct >= 0.95) };
  if (!was.completed && pct >= 0.95) {
    animeCoinsAdd(_animeCurrentId, 25);
    if (typeof rewardStudy === 'function') rewardStudy(50);
  }
  _animeProgressSave(prog);

  // Yoru coin tick every 60s of play
  if (!video.paused && t - _animeLastCoinTick >= 60) {
    _animeLastCoinTick = t;
    animeCoinsAdd(_animeCurrentId, 1);
  }
}

function _animeUpdateSeek() {
  const video = document.getElementById('animeVideo');
  if (!video) return;
  const fill = document.getElementById('animeSeekFill');
  const time = document.getElementById('animeTimeDisp');
  if (fill && video.duration) fill.style.width = (video.currentTime / video.duration * 100) + '%';
  if (time) time.textContent = _animeFmtTime(video.currentTime) + ' / ' + _animeFmtTime(video.duration || 0);
}

function _animeFmtTime(s) {
  s = Math.floor(s || 0);
  const m = Math.floor(s / 60), sec = s % 60;
  return m + ':' + String(sec).padStart(2, '0');
}

function _animeRenderSubtitle(idx) {
  const seg = _animeSegments[idx];
  if (!seg) return;

  const vault = typeof lvLoad === 'function' ? lvLoad() : [];
  const watched = _animeWatchedLoad()[_animeCurrentId] || [];
  ...
  const buildLine = (s) => {
    ...
    if (s.words && s.words.length > 0) {
      ...
      return s.words.map(w => {
        ...
        if (!term) return w.word;
        const known = vault.some(v => v.term === term);
        const seen = watched.some(e => e.term === term);
        const cls = 'anime-word' + (known ? ' anime-word-known' : seen ? ' anime-word-seen' : '');
        return `<span class="${cls}" onclick="animeWordClick(event,'${term.replace(/'/g,"\\'")}',${s.id})">${w.word}</span>`;
      }).join('');
    }
    return s.text;
  };

  document.getElementById('animeSubCurrent').innerHTML = buildLine(seg);
  const prev = _animeSegments[idx - 1];
  const next = _animeSegments[idx + 1];
  document.getElementById('animeSubPrev').textContent = prev ? prev.text : '';
  document.getElementById('animeSubNext').textContent = next ? next.text : '';

  // History
  if (!_animeSubHistoryArr.some(h => h.id === seg.id)) {
    _animeSubHistoryArr.unshift(seg);
    if (_animeSubHistoryArr.length > 60) _animeSubHistoryArr.pop();
    const list = document.getElementById('animeSubHistoryList');
    if (list && !document.getElementById('animeSubHistory').classList.contains('hidden')) {
      const row = document.createElement('div');
      row.className = 'anime-sub-hist-row';
      row.textContent = seg.text;
      row.onclick = () => { document.getElementById('animeVideo').currentTime = seg.start; };
...
// ── Word click + popup ───────────────────────────────────────────
async function animeWordClick(evt, term, segId) {
  evt.stopPropagation();
  if (!term) return;

  const entry = await animeWordLookup(term);
  ...
  const watchEntry = {
    ...
    jlpt: entry.jlpt,
    pos: entry.pos,
    animeId: _animeCurrentId,
    episodeId: _animeCurrentEp,
    timestamp: Date.now(),
    seenAt: new Date().toISOString(),
    addedToStudy: false,
  };
  animeWatchedAdd(_animeCurrentId, watchEntry);
  _animePopupEntry = watchEntry;

  // Show popup
  document.getElementById('animePopupTerm').textContent = entry.term;
  document.getElementById('animePopupReading').textContent = entry.reading;
  document.getElementById('animePopupMeaning').textContent = entry.meaning || '—';
  const metaParts = [];
  if (entry.jlpt) metaParts.push(entry.jlpt.toUpperCase());
  if (entry.pos) metaParts.push(entry.pos);
  document.getElementById('animePopupMeta').textContent = metaParts.join(' · ');

  // Check if already added
  const vault = typeof lvLoad === 'function' ? lvLoad() : [];
  const alreadyAdded = vault.some(v => v.term === term && v.source === 'anime');
  const addBtn = document.getElementById('animePopupAddBtn');
  addBtn.textContent = alreadyAdded ? '✓ Added' : '+ Add to Study';
  addBtn.className = 'anime-popup-btn anime-popup-add' + (alreadyAdded ? ' added' : '');

  document.getElementById('animePopupQSCheck').checked = _animeQuickStudyQueue.some(e => e.term === term);

  document.getElementById('animeWordPopup').classList.remove('hidden');
  document.getElementById('animeWordPopupOverlay').classList.remove('hidden');
}

function animeCloseWordPopup() {
  document.getElementById('animeWordPopup').classList.add('hidden');
  document.getElementById('animeWordPopupOverlay').classList.add('hidden');
  _animePopupEntry = null;
}

function animePopupAddToVault() {
  if (!_animePopupEntry) return;
  animeAddToVault(_animePopupEntry);
  const btn = document.getElementById('animePopupAddBtn');
  btn.textContent = '✓ Added';
  btn.classList.add('added');
}

function animePopupToggleQS() {
  if (!_animePopupEntry) return;
  const chk = document.getElementById('animePopupQSCheck');
  if (chk.checked) {
    if (!_animeQuickStudyQueue.some(e => e.term === _animePopupEntry.term)) {
      _animeQuickStudyQueue.push({ ..._animePopupEntry });
    }
  } else {
    _animeQuickStudyQueue = _animeQuickStudyQueue.filter(e => e.term !== _animePopupEntry.term);
  }
  _animeUpdateQSBadge();
}

function _animeUpdateQSBadge() {
  const badge = document.getElementById('animeQSBadge');
  if (!badge) return;
  const n = _animeQuickStudyQueue.length;
  badge.textContent = n;
...
function animeSeekLine() {
  const v = document.getElementById('animeVideo');
  if (_animeCurrentSegIdx >= 0 && _animeSegments[_animeCurrentSegIdx]) {
    v.currentTime = _animeSegments[_animeCurrentSegIdx].start;
  }
}

function animeToggleLoop() {
  _animeLoopActive = !_animeLoopActive;
  document.getElementById('animeLoopBtn').classList.toggle('active', _animeLoopActive);
}

function animeSkipOP() {
  const anime = ANIME_CATALOG.find(a => a.id === _animeCurrentId);
  const ep = anime?.episodes.find(e => e.id === _animeCurrentEp);
  if (!ep) return;
  document.getElementById('animeVideo').currentTime = ep.opEnd || 90;
}

function animeToggleHistory() {
  const h = document.getElementById('animeSubHistory');
  const hidden = h.classList.toggle('hidden');
  if (!hidden) {
    // populate
    const list = document.getElementById('animeSubHistoryList');
    list.innerHTML = '';
    _animeSubHistoryArr.forEach(seg => {
      const row = document.createElement('div');
      row.className = 'anime-sub-hist-row';
      row.textContent = seg.text;
      row.onclick = () => { document.getElementById('animeVideo').currentTime = seg.start; };
...
// ── Encountered panel ────────────────────────────────────────────
function animeRenderEncountered() {
  const filter = document.getElementById('animeEncEpFilter')?.value || 'all';
  const watched = _animeWatchedLoad()[_animeCurrentId] || [];
  const vault = typeof lvLoad === 'function' ? lvLoad() : [];
  const list = document.getElementById('animeEncList');
  if (!list) return;
  const items = filter === 'all' ? watched : watched.filter(e => e.episodeId === filter);
  list.innerHTML = '';
  if (!items.length) { list.innerHTML = '<div style="color:var(--muted);font-size:13px;padding:12px 0">No words encountered yet.</div>'; return; }
  items.slice().reverse().forEach(e => {
    const inVault = vault.some(v => v.term === e.term && v.source === 'anime');
    const row = document.createElement('div');
    row.className = 'anime-enc-row';
    row.innerHTML = `
      <span class="anime-enc-term">${e.term}</span>
      <span class="anime-enc-reading">${e.reading||''}</span>
      <span class="anime-enc-meaning">${e.meaning||''}</span>
      <span class="anime-enc-ep">EP${e.episodeId?.replace(/\D+(\d+)$/,'$1')||'?'}</span>
      <button class="anime-enc-add-btn${inVault?' added':''}" onclick="animeEncAddWord(this,'${e.term.replace(/'/g,"\\'")}','${e.episodeId||_animeCurrentEp||''}')">
        ${inVault ? '✓ Added' : '+ Add'}
      </button>
    `;
    list.appendChild(row);
...
function animeEncAddWord(btn, term, epId) {
  const watched = _animeWatchedLoad()[_animeCurrentId] || [];
  const entry = watched.find(e => e.term === term && e.episodeId === epId) || { term, reading: '', meaning: '', animeId: _animeCurrentId, episodeId: epId };
  animeAddToVault(entry);
  btn.textContent = '✓ Added';
  btn.classList.add('added');
}

function animeAddAllToVault() {
  const filter = document.getElementById('animeEncEpFilter')?.value || 'all';
  const watched = _animeWatchedLoad()[_animeCurrentId] || [];
  const items = filter === 'all' ? watched : watched.filter(e => e.episodeId === filter);
  items.forEach(e => animeAddToVault(e));
  animeRenderEncountered();
}

// ── Studying panel ───────────────────────────────────────────────
function animeRenderStudying() {
  const list = document.getElementById('animeStudyingList');
  if (!list) return;
  const vault = typeof lvLoad === 'function' ? lvLoad() : [];
  const items = vault.filter(v => v.source === 'anime' && v.animeId === _animeCurrentId);
  list.innerHTML = '';
  if (!items.length) { list.innerHTML = '<div style="color:var(--muted);font-size:13px;padding:12px 0">No words added to study yet.</div>'; return; }
  items.forEach(e => {
    const row = document.createElement('div');
    row.className = 'anime-enc-row';
    row.innerHTML = `
      ...
      <span class="anime-enc-reading">${e.reading||''}</span>
      <span class="anime-enc-meaning">${e.meaning||''}</span>
      ${typeof lvDimBarsHTML === 'function' ? lvDimBarsHTML(e) : ''}
    `;
    list.appendChild(row);
  });
}
...
// ── Quick Study ──────────────────────────────────────────────────
function animeOpenQuickStudy(words) {
  if (words) _animeQuickStudyQueue = [...words];
  if (!_animeQuickStudyQueue.length) {
    // use all encountered words for this episode if queue empty
    const watched = _animeWatchedLoad()[_animeCurrentId] || [];
    const epWords = _animeCurrentEp ? watched.filter(e => e.episodeId === _animeCurrentEp) : watched;
    if (!epWords.length) { alert('No words to study yet. Click words in subtitles first.'); return; }
    _animeQuickStudyQueue = [...epWords];
  }
  const video = document.getElementById('animeVideo');
  if (video && !video.paused) video.pause();

  document.getElementById('animeQSPickerCount').textContent = _animeQuickStudyQueue.length + ' words';
  document.getElementById('animeQSPicker').classList.remove('hidden');
  document.getElementById('animeQSCard').classList.add('hidden');
  document.getElementById('animeQSSummary').classList.add('hidden');
  document.getElementById('animeQSOverlay').classList.remove('hidden');
}

function animeQSSelectMode(mode, el) {
  _animeQuickStudyMode = mode;
  document.querySelectorAll('.anime-qs-mode-btn').forEach(b => b.classList.remove('active'));
  if (el) el.classList.add('active');
}

function animeStartQuickStudy() {
  _animeQSIndex = 0;
  _animeQSWrong = [];
  _animeQSCorrect = 0;
  // Shuffle
  const q = [..._animeQuickStudyQueue];
  for (let i = q.length - 1; i > 0; i--) { const j = Math.floor(Math.random()*(i+1)); [q[i],q[j]]=[q[j],q[i]]; }
  _animeQuickStudyQueue = q;

  document.getElementById('animeQSPicker').classList.add('hidden');
  document.getElementById('animeQSSummary').classList.add('hidden');
  document.getElementById('animeQSCard').classList.remove('hidden');
  _animeQSRender();
}

function _animeQSRender() {
  const word = _animeQuickStudyQueue[_animeQSIndex];
  if (!word) { _animeQSSummaryShow(); return; }

  document.getElementById('animeQSProgress').textContent = (_animeQSIndex+1) + ' / ' + _animeQuickStudyQueue.length;
  document.getElementById('animeQSTerm').textContent = word.term;

  // Hide all mode UIs
  ['animeQSFlashBtns','animeQSFlashGrade','animeQSMCQOpts','animeQSWritten'].forEach(id => document.getElementById(id).classList.add('hidden'));
  document.getElementById('animeQSReveal').classList.add('hidden');

  if (_animeQuickStudyMode === 'flash') {
    document.getElementById('animeQSFlashBtns').classList.remove('hidden');
  } else if (_animeQuickStudyMode === 'mcq') {
    _animeQSRenderMCQ(word);
  } else if (_animeQuickStudyMode === 'written') {
    document.getElementById('animeQSWritten').classList.remove('hidden');
    document.getElementById('animeQSWrittenInput').value = '';
    document.getElementById('animeQSWrittenReveal').classList.add('hidden');
    document.getElementById('animeQSWrittenMeaning').textContent = word.meaning;
    setTimeout(() => document.getElementById('animeQSWrittenInput').focus(), 50);
  }
}
...
function animeQSFlip() {
  ...
  document.getElementById('animeQSReveal').classList.remove('hidden');
  document.getElementById('animeQSFlashBtns').classList.add('hidden');
  document.getElementById('animeQSFlashGrade').classList.remove('hidden');
  animeSpeak(word.reading || word.term);
}

function _animeQSRenderMCQ(word) {
  ...
  // Build distractors
  const watched = _animeWatchedLoad()[_animeCurrentId] || [];
  const epWords = _animeCurrentEp ? watched.filter(e => e.episodeId === _animeCurrentEp && e.term !== word.term) : watched.filter(e => e.term !== word.term);
  const vault = typeof lvLoad === 'function' ? lvLoad() : [];
  const vaultWords = vault.filter(v => v.term !== word.term && v.meaning);

  let pool = [...epWords, ...vaultWords].filter((v,i,a) => a.findIndex(x=>x.meaning===v.meaning)===i && v.meaning && v.meaning !== word.meaning);
  // shuffle & take 3
  for (let i=pool.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[pool[i],pool[j]]=[pool[j],pool[i]];}
  const distractors = pool.slice(0,3).map(d=>d.meaning);
  while (distractors.length < 3) distractors.push('—');

  const choices = [word.meaning, ...distractors];
  for (let i=choices.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[choices[i],choices[j]]=[choices[j],choices[i]];}

  choices.forEach(c => {
    const btn = document.createElement('button');
    btn.className = 'anime-qs-mcq-opt';
    btn.textContent = c;
    btn.onclick = () => {
      if (btn.classList.contains('correct') || btn.classList.contains('wrong')) return;
      const correct = c === word.meaning;
      opts.querySelectorAll('.anime-qs-mcq-opt').forEach(b => { if (b.textContent === word.meaning) b.classList.add('correct'); });
      if (!correct) btn.classList.add('wrong');
      setTimeout(() => animeQSGrade(correct ? 'know' : 'again'), 900);
    };
    opts.appendChild(btn);
  });
}

function animeQSWrittenCheck() {
  const word = _animeQuickStudyQueue[_animeQSIndex];
  document.getElementById('animeQSWrittenReveal').classList.remove('hidden');
  document.getElementById('animeQSWrittenMeaning').textContent = word.meaning;
}

function animeQSGrade(result) {
  if (result === 'know' || result === 'correct' || result === 'close') {
    _animeQSCorrect++;
    if (typeof rewardStudy === 'function') rewardStudy(2);
    if (typeof dailyQuestsTrack === 'function') dailyQuestsTrack('anime_flash_know', 1);
  } else {
    _animeQSWrong.push(_animeQuickStudyQueue[_animeQSIndex]);
  }
  _animeQSIndex++;
  if (_animeQSIndex < _animeQuickStudyQueue.length) {
    _animeQSRender();
  } else {
    _animeQSSummaryShow();
  }
...
function _animeQSSummaryShow() {
  document.getElementById('animeQSCard').classList.add('hidden');
  document.getElementById('animeQSSummary').classList.remove('hidden');
  const total = _animeQuickStudyQueue.length;
  document.getElementById('animeQSSummaryScore').textContent = _animeQSCorrect + ' / ' + total;
  const added = _animeQuickStudyQueue.filter(w => {
    const vault = typeof lvLoad === 'function' ? lvLoad() : [];
    return vault.some(v => v.term === w.term && v.source === 'anime');
  }).length;
  document.getElementById('animeQSSummaryDetail').innerHTML = `${added} words in vault &nbsp;·&nbsp; ${_animeQSWrong.length} wrong`;
  document.getElementById('animeQSRetryBtn').style.display = _animeQSWrong.length ? '' : 'none';
}

function animeQSRetryWrong() {
  _animeQuickStudyQueue = [..._animeQSWrong];
  animeStartQuickStudy();
}

function animeCloseQS() {
  document.getElementById('animeQSOverlay').classList.add('hidden');
  const video = document.getElementById('animeVideo');
  // don't auto-resume; let user decide
}

// ── Study Hub ────────────────────────────────────────────────────
function animeHubUpdateSet() {
  const epSel = document.getElementById('animeHubEpSelect')?.value || 'all';
  const filterSel = document.getElementById('animeHubFilterSelect')?.value || 'both';

  const watched = _animeWatchedLoad()[_animeCurrentId] || [];
  const vault = typeof lvLoad === 'function' ? lvLoad() : [];

  let base = epSel === 'all' ? watched : watched.filter(e => e.episodeId === epSel);

  let set = [];
  base.forEach(e => {
    const inVault = vault.some(v => v.term === e.term && v.source === 'anime');
    if (filterSel === 'encountered' && inVault) return;
    if (filterSel === 'studying' && !inVault) return;
    set.push(e);
  });

  _animeHubSet = set;

  const statsEl = document.getElementById('animeHubStats');
  if (statsEl) {
    const enc = set.filter(e => !vault.some(v => v.term === e.term && v.source === 'anime')).length;
    const studying = set.length - enc;
    statsEl.textContent = `Words in set: ${set.length}  (${enc} encountered, ${studying} studying)`;
  }
}

function animeOpenStudyHub() {
  animeHubUpdateSet();
  animeSetHomeTab('hub', null);
  // scroll to hub
  document.getElementById('animeHome').classList.remove('hidden');
  document.getElementById('animeEpisodeView').classList.add('hidden');
}

function animeHubLaunch(mode) {
  animeHubUpdateSet();
  if (!_animeHubSet.length) { alert('No words in this set. Watch some episodes and click words first.'); return; }

  if (mode === 'mcq') {
    _animeQuickStudyQueue = [..._animeHubSet];
    _animeQuickStudyMode = 'mcq';
    animeOpenQuickStudy(_animeHubSet);
  } else if (mode === 'written') {
    _animeQuickStudyQueue = [..._animeHubSet];
    _animeQuickStudyMode = 'written';
    animeOpenQuickStudy(_animeHubSet);
  } else if (mode === 'trace') {
    // Extract kanji chars
    const chars = [];
    _animeHubSet.forEach(e => {
      [...(e.term||'')].forEach(ch => {
        if (/[\u4e00-\u9faf]/.test(ch) && !chars.includes(ch)) chars.push(ch);
      });
    });
    if (!chars.length) { alert('No kanji found in this set.'); return; }
    if (typeof window !== 'undefined') window._traceCustomPool = chars;
    if (typeof switchMode === 'function') switchMode('trace', null);
    if (typeof setTraceMode === 'function') setTimeout(() => setTraceMode('normal', null), 300);
  } else if (mode === 'deep') {
    const vault = typeof lvLoad === 'function' ? lvLoad() : [];
    const deepWords = _animeHubSet.filter(e => vault.some(v => v.term === e.term && v.source === 'anime'));
    if (!deepWords.length) { alert('Add words to your study vault first (+ Add to Study).'); return; }
    // Build temp review cards and open review
    if (typeof REVIEW_SETS !== 'undefined') {
      const tempSet = { id: 'animeHub', label: 'Anime Hub', source: 'animeHub', _cards: deepWords };
      if (!REVIEW_SETS.find(s => s.id === 'animeHub')) REVIEW_SETS.push(tempSet);
    }
    if (typeof switchMode === 'function') switchMode('review', null);
  }
}

// ── Anime Shop ───────────────────────────────────────────────────
function animeOpenShop() {
  const items = ANIME_SHOP_ITEMS[_animeCurrentId] || [];
  const balance = animeCoinsGet(_animeCurrentId);
  document.getElementById('animeShopBalance').textContent = balance;

  const grid = document.getElementById('animeShopItems');
  grid.innerHTML = '';
  items.forEach(item => {
    const el = document.createElement('div');
    el.className = 'anime-shop-item';
    const canAfford = balance >= item.cost;
    el.innerHTML = `
      <div class="anime-shop-item-name">${item.name}</div>
      <div class="anime-shop-item-type">${item.type}</div>
      <div class="anime-shop-item-price">
        <img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/moon_tear.png" style="width:11px;height:11px;image-rendering:pixelated" alt="">
        ${item.cost}
      </div>
      <button class="anime-shop-buy-btn" ${canAfford?'':'disabled'} onclick="animeBuyShopItem('${item.id}',${item.cost},'${_animeCurrentId}','${item.type}','${item.name.replace(/'/g,"\\'")}')">
  ...
  document.getElementById('animeShopPanel').classList.remove('hidden');
}

function animeCloseShop() {
  document.getElementById('animeShopPanel').classList.add('hidden');
}

function animeBuyShopItem(itemId, cost, animeId, type, name) {
  const bal = animeCoinsGet(animeId);
  if (bal < cost) { alert('Not enough Yoru Coins.'); return; }
  const d = _animeCoinsLoad();
  d[animeId] = (d[animeId] || 0) - cost;
  _animeCoinsSave(d);
  // Apply if applicable
  if (type === 'title' && typeof equipTitle === 'function') {
    equipTitle(itemId);
  } else if (type === 'background' && typeof applyBg === 'function') {
    applyBg({ type: 'gradient', value: '#1a1a2e' }); // placeholder
  }
  if (typeof rewardPopup === 'function') rewardPopup('Purchased: ' + name);
  animeOpenShop(); // re-render with new balance
}
...
// Patch switchMode to call initAnimeTab
(function() {
  const _origSwitch = window.switchMode;
  window.switchMode = function(mode, event) {
    if (_origSwitch) _origSwitch(mode, event);
    if (mode === 'anime') {
      setTimeout(initAnimeTab, 50);
    }
  };
})();
