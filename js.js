...
// ── Fake cursor overlay ──
// Browsers cannot animate GIF/ANI via CSS cursor: url().
// For animated cursors we hide the native cursor and drive a
// positioned <img> element with mousemove instead.
...
function _getFakeCursorEl() {
  if (!_fakeCursorEl) {
    ...
    document.addEventListener('mousemove', e => {
      ...
      _fakeCursorEl.style.transform = `translate(${e.clientX}px,${e.clientY}px)`;
    }, { passive: true });
  }
...
function applyCursorCosmetic(cursorId, variantIdx) {
  ...
  // Resolve which file to use (variant or default)
  ...
  // .cur files work natively in browsers as static CSS cursors.
  // Everything else (.gif, .ani, etc.) needs the fake cursor approach
  // so animation actually plays.
