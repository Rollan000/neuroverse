...
// ============================================================
// DAILY QUESTS SYSTEM (Phase 6.5.8)
// ============================================================

const DAILY_QUESTS_DEF = [
  { id: 'q_mcq_10',       desc: 'Answer 25 MCQ correctly',         target: 25, track: 'mcq_correct',    reward: 20 },
  { id: 'q_review_5',     desc: 'Review 15 cards',                  target: 15, track: 'review_graded',  reward: 20 },
  { id: 'q_kana_session', desc: 'Complete 2 kana sessions',         target: 2,  track: 'kana_complete',  reward: 20 },
  { id: 'q_trace_5',      desc: 'Trace 10 kanji correctly',         target: 10, track: 'trace_correct',  reward: 20 },
  { id: 'q_streak_10',    desc: 'Reach a 20-answer MCQ streak',     target: 20, track: 'mcq_streak',     reward: 25 },
  { id: 'q_flash_5',      desc: 'Mark 10 flashcards as known',      target: 10, track: 'flash_known',    reward: 20 },
];

// Daily quests are keyed by date so old ones auto-expire
function _dqDateKey() { return 'dq_' + new Date().toISOString().slice(0,10); }

function _dqLoad() {
  try { return JSON.parse(localStorage.getItem(_dqDateKey()) || '{}'); } catch(_) { return {}; }
}

function _dqSave(data) {
  localStorage.setItem(_dqDateKey(), JSON.stringify(data));
  // Clean up keys older than 2 days
  try {
    const yesterday = new Date(); yesterday.setDate(yesterday.getDate() - 2);
    const yKey = 'dq_' + yesterday.toISOString().slice(0,10);
    localStorage.removeItem(yKey);
  } catch(_) {}
}

// Call this from answer handlers to track quest progress
function dailyQuestsTrack(trackKey, amount) {
  amount = amount == null ? 1 : amount;
  const data = _dqLoad();
  let changed = false;
  DAILY_QUESTS_DEF.forEach(q => {
    if (q.track !== trackKey) return;
    const prev = data[q.id] || { progress: 0, claimed: false };
    if (prev.claimed) return;
    let newProg;
    // mcq_streak passes absolute value — use max rather than sum
    if (trackKey === 'mcq_streak') {
      newProg = Math.min(q.target, Math.max(prev.progress || 0, amount));
    } else {
      newProg = Math.min(q.target, (prev.progress || 0) + amount);
    }
    if (newProg !== (prev.progress || 0)) {
      data[q.id] = { progress: newProg, claimed: prev.claimed };
      changed = true;
      if (newProg >= q.target && !prev.claimed) {
        _dqComplete(q, data);
      }
    }
  });
  if (changed) { _dqSave(data); renderDailyQuests(); _updateQuestsNotifDot(); }
}

function _dqComplete(q, data) {
  if ((data[q.id] || {}).claimed) return;
  data[q.id] = { progress: q.target, claimed: true };
  sacramentCurrency += q.reward;
  updateCurrencies();
  _showQuestCompletePop(q);
}

function _showQuestCompletePop(q) {
  const el = document.createElement('div');
  el.className = 'quest-complete-pop';
  const moonImg = '<img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/moon_tear.png" style="width:14px;height:14px;vertical-align:-2px;image-rendering:pixelated" alt="">';
  el.innerHTML = `
    <div class="qcp-icon"><img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/trophy.png" style="width:24px;height:24px;image-rendering:pixelated" alt=""></div>
    <div class="qcp-body">
      <div class="qcp-title">Quest Complete!</div>
      <div class="qcp-desc">${q.desc}</div>
      <div class="qcp-reward">+${q.reward} ${moonImg} sacraments</div>
    </div>`;
  document.body.appendChild(el);
...
function renderDailyQuests() {
  const container = document.getElementById('dailyQuestsPanel');
  if (!container) return;
  const data = _dqLoad();
  const moonImg = '<img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/moon_tear.png" style="width:12px;height:12px;vertical-align:-1px;image-rendering:pixelated" alt="">';

  const completedCount = DAILY_QUESTS_DEF.filter(q => (data[q.id] || {}).claimed).length;
  const totalReward = DAILY_QUESTS_DEF.reduce((s, q) => s + q.reward, 0);

  container.innerHTML = `
    <div class="dq-header">
      <div class="dq-title"><img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/trophy.png" style="width:1em;height:1em;vertical-align:-0.1em;image-rendering:pixelated" alt=""> Daily Quests</div>
      <div class="dq-meta">${completedCount}/${DAILY_QUESTS_DEF.length} · ${moonImg} ${totalReward} total</div>
    </div>
    <div class="dq-list">
      ${DAILY_QUESTS_DEF.map(q => {
        const state = data[q.id] || { progress: 0, claimed: false };
        const prog = Math.min(q.target, state.progress || 0);
        const pct = Math.round(prog / q.target * 100);
        const done = state.claimed;
        return `<div class="dq-item ${done ? 'dq-done' : ''}">
          <div class="dq-item-top">
            <span class="dq-desc">${q.desc}</span>
            <span class="dq-reward">${done ? '✓' : '+' + q.reward} ${moonImg}</span>
          </div>
          <div class="dq-bar-wrap">
            <div class="dq-bar-track">
              <div class="dq-bar-fill" style="width:${done?100:pct}%"></div>
            </div>
            <span class="dq-bar-label">${done ? q.target : prog}/${q.target}</span>
          </div>
        </div>`;
      }).join('')}
    </div>`;
...
// ── Wire tracking calls into existing answer handlers ──────
// MCQ correct answers
(function _patchMcqForQuests() {
  const _origAnswerMCQ = typeof answerMCQ !== 'undefined' ? answerMCQ : null;
  if (!_origAnswerMCQ) return;
  const _base = answerMCQ;
  answerMCQ = function(choice) {
    const q = mcqPool[mcqIdx];
    const correct = q && choice === q.ans;
    _base(choice);
    if (correct) { dailyQuestsTrack('mcq_correct'); achievementsTrack('mcq_correct_total'); }
  };
})();

// MCQ streak tracking
(function _patchMcqStreakForQuests() {
  const _orig = typeof updateMcqStreak !== 'undefined' ? updateMcqStreak : null;
  // Track via mcqStreak global — hook on rewardStudy instead (simpler)
})();

// Review card graded
(function _patchRevForQuests() {
  const _orig = typeof gradeCard !== 'undefined' ? gradeCard : null;
  if (!_orig) return;
  const _base = gradeCard;
  gradeCard = function(grade) {
    _base(grade);
    dailyQuestsTrack('review_graded');
    achievementsTrack('review_graded_total');
  };
})();

// Kana session complete — hook openSessionSummary
(function _patchKanaForQuests() {
  const _orig = typeof openSessionSummary !== 'undefined' ? openSessionSummary : null;
  if (!_orig) return;
  const _base = openSessionSummary;
  openSessionSummary = function(opts) {
    _base(opts);
    // Detect kana by emoji/title heuristic
    if (opts && opts.title && (opts.title.includes('Kana') || opts.title.includes('kana'))) {
      dailyQuestsTrack('kana_complete');
    }
  };
})();

// Trace correct
(function _patchTraceForQuests() {
  const _orig = typeof traceGotIt !== 'undefined' ? traceGotIt : null;
  if (!_orig) return;
  const _base = traceGotIt;
  traceGotIt = function() {
    _base();
    dailyQuestsTrack('trace_correct');
    achievementsTrack('trace_correct_total');
  };
})();

// ============================================================
// ACHIEVEMENTS SYSTEM (Phase 6.5.9)
// ============================================================

const ACHIEVEMENTS_DEF = [
  { id: 'ach_100_mcq',    name: 'Century',       desc: 'Answer 100 MCQ correctly',        target: 100,  track: 'mcq_correct_total',   reward: 50,  icon: 'trophy.png'   },
  { id: 'ach_50_trace',   name: 'Pensmith',      desc: 'Trace 50 kanji correctly',         target: 50,   track: 'trace_correct_total', reward: 38,  icon: 'quill.png'    },
  { id: 'ach_500_mcq',    name: 'Veteran',       desc: 'Answer 500 MCQ correctly',         target: 500,  track: 'mcq_correct_total',   reward: 125, icon: 'trophy.png'   },
  { id: 'ach_streak_7',   name: 'Consistent',    desc: '7-day study streak',               target: 7,    track: 'day_streak',          reward: 38,  icon: 'fire.png'     },
  { id: 'ach_streak_30',  name: 'Devoted',       desc: '30-day study streak',              target: 30,   track: 'day_streak',          reward: 125, icon: 'fire.png'     },
  { id: 'ach_vault_50',   name: 'Scholar',       desc: '50 items in learned vault',        target: 50,   track: 'vault_count',         reward: 50,  icon: 'chest.png'    },
  { id: 'ach_vault_100',  name: 'Archivist',     desc: '100 items in learned vault',       target: 100,  track: 'vault_count',         reward: 75,  icon: 'chest.png'    },
  { id: 'ach_review_100', name: 'Reviewer',      desc: 'Review 100 cards',                 target: 100,  track: 'review_graded_total', reward: 50,  icon: 'open_book.png'},
  { id: 'ach_dq_10',      name: 'On a Mission',  desc: 'Complete 10 daily quests',         target: 10,   track: 'dq_completed_total',  reward: 75,  icon: 'star.png'     },
  { id: 'ach_sac_1000',   name: 'Devoted Soul',  desc: 'Earn 1000 sacraments total',       target: 1000, track: 'sac_earned_total',    reward: 100, icon: 'moon_tear.png'},
  { id: 'ach_flash_50',   name: 'Card Shepherd', desc: 'Mark 50 flashcards as known',      target: 50,   track: 'flash_known_total',   reward: 38,  icon: 'open_book.png'},
];

// Achievement progress is cumulative (not date-keyed)
function _achLoad() {
  try { return JSON.parse(localStorage.getItem('ach_progress') || '{}'); } catch(_) { return {}; }
}
function _achSave(data) { localStorage.setItem('ach_progress', JSON.stringify(data)); }

let _achClaimed = JSON.parse(localStorage.getItem('ach_claimed') || '{}');
function _achSaveClaimed() { localStorage.setItem('ach_claimed', JSON.stringify(_achClaimed)); }

function achievementsTrack(trackKey, amount) {
  amount = amount || 1;
  const data = _achLoad();
  let changed = false;
  ACHIEVEMENTS_DEF.forEach(a => {
    if (a.track !== trackKey) return;
    if (_achClaimed[a.id]) return;
    const prev = data[a.id] || 0;
    const next = prev + amount;
    data[a.id] = next;
    changed = true;
    if (next >= a.target && !_achClaimed[a.id]) {
      _achClaimed[a.id] = true;
      _achSaveClaimed();
      _achUnlocked(a);
    }
  });
  if (changed) { _achSave(data); renderAchievements(); }
}

function _achUnlocked(a) {
  sacramentCurrency += a.reward;
  updateCurrencies();
  const moonImg = '<img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/moon_tear.png" style="width:14px;height:14px;vertical-align:-2px;image-rendering:pixelated" alt="">';
  const el = document.createElement('div');
  el.className = 'ach-unlock-pop';
  el.innerHTML = `
    <div class="aup-glow"></div>
    <img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/${a.icon}" class="aup-icon" alt="">
    <div class="aup-body">
      <div class="aup-eyebrow">Achievement Unlocked</div>
      <div class="aup-name">${a.name}</div>
      <div class="aup-desc">${a.desc}</div>
      <div class="aup-reward">+${a.reward} ${moonImg}</div>
    </div>`;
  document.body.appendChild(el);
  setTimeout(() => el.classList.add('show'), 60);
  setTimeout(() => { el.classList.remove('show'); setTimeout(() => el.remove(), 500); }, 5000);
  // Also track dq_completed_total when a daily quest achievement fires (not circular, only for its own ach)
}

// Track sacraments earned total
const _origRewardStudy2 = rewardStudy;
rewardStudy = function(amount) {
  _origRewardStudy2(amount);
  if (amount > 0) achievementsTrack('sac_earned_total', amount);
};

// Track vault count
function _achSyncVaultCount() {
  try {
    const vault = JSON.parse(localStorage.getItem('learnedVault') || '[]');
    achievementsTrack('vault_count', 0); // no increment, just check
    // Set absolute value instead
    const data = _achLoad();
    const relevant = ACHIEVEMENTS_DEF.filter(a => a.track === 'vault_count');
    relevant.forEach(a => {
      if (_achClaimed[a.id]) return;
      const prev = data[a.id] || 0;
      if (vault.length > prev) {
        data[a.id] = vault.length;
        if (vault.length >= a.target) {
          _achClaimed[a.id] = true;
          _achSaveClaimed();
          _achUnlocked(a);
        }
...
// Track day streak for achievements
function _achSyncStreak() {
  try {
    const streak = typeof revGetDayStreak === 'function' ? revGetDayStreak() : (typeof _revDayStreak !== 'undefined' ? _revDayStreak : 0);
    const data = _achLoad();
    const relevant = ACHIEVEMENTS_DEF.filter(a => a.track === 'day_streak');
    relevant.forEach(a => {
      if (_achClaimed[a.id]) return;
      const prev = data[a.id] || 0;
      if (streak > prev) {
...
// Track daily quests completed total
function _achTrackDqComplete() {
  achievementsTrack('dq_completed_total');
}

// Call vault sync periodically and on vault open
window.addEventListener('load', () => {
  setTimeout(() => { _achSyncVaultCount(); _achSyncStreak(); }, 1500);
});
...
function renderAchievements() {
  ...
  const unlockedCount = ACHIEVEMENTS_DEF.filter(a => _achClaimed[a.id]).length;

  container.innerHTML = `
    <div class="dq-header">
      <div class="dq-title"><img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/trophy.png" style="width:1em;height:1em;vertical-align:-0.1em;image-rendering:pixelated" alt=""> Achievements</div>
      <div class="dq-meta">${unlockedCount}/${ACHIEVEMENTS_DEF.length} unlocked</div>
    </div>
    <div class="ach-grid">
      ${ACHIEVEMENTS_DEF.map(a => {
        const progress = data[a.id] || 0;
        const claimed = _achClaimed[a.id] || false;
        const pct = Math.min(100, Math.round(progress / a.target * 100));
        return `<div class="ach-card ${claimed ? 'ach-done' : ''}">
          <img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/${a.icon}" class="ach-icon" alt="">
          <div class="ach-name">${a.name}</div>
          <div class="ach-desc">${a.desc}</div>
          <div class="ach-bar-track"><div class="ach-bar-fill" style="width:${claimed?100:pct}%"></div></div>
          <div class="ach-footer">
            <span class="ach-prog">${claimed ? '✓ Done' : progress + '/' + a.target}</span>
            <span class="ach-reward">${claimed ? '' : '+' + a.reward} ${moonImg}</span>
          </div>
        </div>`;
      }).join('')}
    </div>`;
...
function getFiltered(arr) {
  ...
  // anime episode source — pull from vault
  if (activeUnit.startsWith('anime_')) {
    const epId = activeUnit.replace('anime_', '');
    const vault = typeof lvLoad === 'function' ? lvLoad() : [];
    const epWords = vault.filter(v => v.source === 'anime' && v.episodeId === epId && v.term && v.meaning);
    return epWords.map(v => ({
      q: v.term + (v.reading && v.reading !== v.term ? '（' + v.reading + '）' : ''),
      a: v.meaning,
      unit: 'anime',
      diff: 'medium',
      source: 'anime',
...
function rateCard(knew) {
  if(knew) { fcKnown.add(fcIdx); fcMissed.delete(fcIdx); session.fcKnow++; rewardStudy(1); dailyQuestsTrack('flash_known'); achievementsTrack('flash_known_total'); }
  else {
...
function devUnlockAll() {
  ...
  WIDGET_CATALOG.forEach(w => {
    if (!ownedWidgets.includes(w.id)) {
      ownedWidgets.push(w.id);
    }
  });
  saveOwnedWidgets();
...
const WIDGET_CATALOG = [
  ...
  // ── Today's Kanji (functional widget) ──────────────────────
  {
    id: 'widget_today_kanji',
    name: "Today's Kanji",
    rarity: 'rare',
    type: 'widget',
    span: 2,
    interactive: 'todayKanji',
    desc: 'A new featured kanji every day. Tap to reveal readings.',
    cardNum: '#006',
  },
  ...
  // ── Album CD widgets ──────────────────────────────────────
  {
    id: 'widget_yofukashi_disc1',
    name: 'Call of the Night — BGM Vol.1',
    rarity: 'legendary',
    type: 'widget',
    span: 1,
    cols: 1,
    interactive: 'albumCd',
    playlistId: 'PLl-vhnGPY7crxGbQ9ILKMd3uiJe0K1Smh',
    imgUrl: 'https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/widgets/records/yofukashi_no_uta_bgm_disc1.png',
    discImgUrl: 'https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/widgets/records/yofukashi_no_uta_bgm_disc1.png',
    desc: 'Call of the Night original BGM. Plays the full soundtrack.',
    cardNum: '#009',
    auctionOnly: false,
  },
];
    ...
    if (widgetId) {
      ...
      if (w) {
        if (w.interactive) {
          ...
          el.innerHTML = `${w.imgUrl ? `<img src="${w.imgUrl}" alt="${w.name}" draggable="false">` : ''}
            <button class="widget-slot-remove" onclick="event.stopPropagation();removeNavbarSlot(${i})" title="Remove">x</button>
  ...
  ['left','right'].forEach(side => {
    ...
    for (const [key, entry] of Object.entries(widgetSlots)) {
      ...
      if (w.interactive) {
        ...
        el.innerHTML = `
          ${w.imgUrl ? `<img src="${w.imgUrl}" alt="${w.name}" draggable="false">` : ''}
    ...
    <div class="wg-picker">
      ...
      ${ownedWidgets.map(id => {
        ...
        return `<div class="wg-picker-item ${isSelected ? 'selected' : ''} ${placed ? 'placed' : ''}"
          ...
          <div class="wg-picker-thumb">${w.imgUrl ? `<img src="${w.imgUrl}" alt="${w.name}">` : '<span style="font-size:1.5rem">🃏</span>'}</div>
    ...
    <div class="widget-card ${rarityClass}"
      ...
      <div class="widget-card-inner">
        ...
        <div class="widget-card-art">
          ${widget.imgUrl ? `<img src="${widget.imgUrl}" alt="${widget.name}" draggable="false">` : `<div style="font-size:2rem;display:flex;align-items:center;justify-content:center;height:100%">🃏</div>`}
        </div>
...
function applyBg(bgId) {
  ...
  if (!bgId || bgId === 'none' || (!preset.css && !preset.isImage)) {
    bgStyle.textContent = '';
    return;
  }
...
const shopPool = [
  ...
  // Yofukashi no Uta titles (purchased with Yoru Coins in anime shop)
  {id:'title_yoru_child_night', name:'Title: Child of the Night', cost:0, type:'title', data:'Child of the Night', yoruCost:200},
  {id:'title_yoru_vampire',     name:'Title: Night Crawler',      cost:0, type:'title', data:'Night Crawler',     yoruCost:300},
  {id:'title_yoru_nazuna',      name:'Title: Kou\'s Confidant',   cost:0, type:'title', data:"Kou's Confidant",   yoruCost:500},

  // (theme_*, vfx_*, effect_*, dlg_*, bg_* items removed — replaced by COSMETIC_PACKS system)
];
...
const COSMETIC_PACKS = [
  ...
  // ── Color Set Packs ───────────────────────────────────────
  {
    ...
    packCost: 175, source: 'shop',
    coverColor: 'linear-gradient(135deg,#0e1a2e,#1a2840)',
    items: [
      { id: 'frost_cs_01', name: 'Bone',         cost: 0,  data: 'cs_bone'        },
      { id: 'frost_cs_02', name: 'Violet Hour',  cost: 50, data: 'cs_violet_hour' },
      { id: 'frost_cs_03', name: 'Frost',        cost: 65, data: 'cs_frost'       },
    ]
  },
  {
    ...
    items: [
      ...
      { id: 'ember_cs_02', name: 'Ember Glow', cost: 50, data: 'cs_ember_glow' },
      { id: 'ember_cs_03', name: 'Rust Blush', cost: 65, data: 'cs_rust_blush' },
    ]
  },
  // ── Yofukashi no Uta Background Packs ────────────────────
  {
    id: 'pack_yoru_bg1', name: 'Yoru BG Pack 1', type: 'pack', category: 'bg',
    desc: 'Background set from Yofukashi no Uta. Unlock each image in sequence with Yoru Coins.',
    packCost: 300, source: 'shop', yoruOnly: true,
    coverColor: 'linear-gradient(135deg,#0a0010,#1a0030)',
    items: [
      { id: 'yoru_bg1_01', name: 'Yoru Night 1',  cost: 0,   data: 'yoru1_1',  yoruCost: 0   },
      { id: 'yoru_bg1_02', name: 'Yoru Night 2',  cost: 0,   data: 'yoru1_2',  yoruCost: 50  },
      { id: 'yoru_bg1_03', name: 'Yoru Night 3',  cost: 0,   data: 'yoru1_3',  yoruCost: 50  },
      { id: 'yoru_bg1_04', name: 'Yoru Night 4',  cost: 0,   data: 'yoru1_4',  yoruCost: 60  },
      { id: 'yoru_bg1_05', name: 'Yoru Night 5',  cost: 0,   data: 'yoru1_5',  yoruCost: 60  },
      { id: 'yoru_bg1_06', name: 'Yoru Night 6',  cost: 0,   data: 'yoru1_6',  yoruCost: 70  },
      { id: 'yoru_bg1_07', name: 'Yoru Night 7',  cost: 0,   data: 'yoru1_7',  yoruCost: 70  },
      { id: 'yoru_bg1_08', name: 'Yoru Night 8',  cost: 0,   data: 'yoru1_8',  yoruCost: 80  },
      { id: 'yoru_bg1_09', name: 'Yoru Night 9',  cost: 0,   data: 'yoru1_9',  yoruCost: 80  },
      { id: 'yoru_bg1_10', name: 'Yoru Night 10', cost: 0,   data: 'yoru1_10', yoruCost: 90  },
      { id: 'yoru_bg1_11', name: 'Yoru Night 11', cost: 0,   data: 'yoru1_11', yoruCost: 90  },
      { id: 'yoru_bg1_12', name: 'Yoru Night 12', cost: 0,   data: 'yoru1_12', yoruCost: 100 },
      { id: 'yoru_bg1_13', name: 'Yoru Night 13', cost: 0,   data: 'yoru1_13', yoruCost: 100 },
    ]
  },
  {
    id: 'pack_yoru_bg2', name: 'Yoru BG Pack 2', type: 'pack', category: 'bg',
    desc: 'Second background set from Yofukashi no Uta. Unlock each image in sequence with Yoru Coins.',
    packCost: 300, source: 'shop', yoruOnly: true,
    coverColor: 'linear-gradient(135deg,#0a0018,#1a0038)',
    items: [
      { id: 'yoru_bg2_01', name: 'Yoru Dawn 1',  cost: 0, data: 'yoru2_1',  yoruCost: 0   },
      { id: 'yoru_bg2_02', name: 'Yoru Dawn 2',  cost: 0, data: 'yoru2_2',  yoruCost: 50  },
      { id: 'yoru_bg2_03', name: 'Yoru Dawn 3',  cost: 0, data: 'yoru2_3',  yoruCost: 50  },
      { id: 'yoru_bg2_04', name: 'Yoru Dawn 4',  cost: 0, data: 'yoru2_4',  yoruCost: 60  },
      { id: 'yoru_bg2_05', name: 'Yoru Dawn 5',  cost: 0, data: 'yoru2_5',  yoruCost: 60  },
      { id: 'yoru_bg2_06', name: 'Yoru Dawn 6',  cost: 0, data: 'yoru2_6',  yoruCost: 70  },
      { id: 'yoru_bg2_07', name: 'Yoru Dawn 7',  cost: 0, data: 'yoru2_7',  yoruCost: 70  },
      { id: 'yoru_bg2_08', name: 'Yoru Dawn 8',  cost: 0, data: 'yoru2_8',  yoruCost: 80  },
      { id: 'yoru_bg2_09', name: 'Yoru Dawn 9',  cost: 0, data: 'yoru2_9',  yoruCost: 80  },
      { id: 'yoru_bg2_10', name: 'Yoru Dawn 10', cost: 0, data: 'yoru2_10', yoruCost: 90  },
    ]
  },
];
...
// Register Yoru background image presets
(function _registerYoruBgPresets() {
  const R2 = 'https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev';
  for (let i = 1; i <= 13; i++) {
    BG_PRESETS['yoru1_' + i] = {
      label: 'Yoru Night ' + i,
      isImage: true,
      url: `${R2}/bg/yoru1/${i}.${i === 13 ? 'webp' : 'jpg'}`,
    };
  }
  for (let i = 1; i <= 10; i++) {
    BG_PRESETS['yoru2_' + i] = {
...
const COLORSET_VARS = {
  cs_bone:        { '--accent':'#e8dcc8','--accent2':'#c8b89a','--accent3':'#d4c4a8','--surface':'#0e0c0a','--surface2':'#1a1814','--border':'rgba(232,220,200,0.12)','--muted':'rgba(232,220,200,0.45)' },
  cs_violet_hour: { '--accent':'#9b7fc4','--accent2':'#c47f9b','--accent3':'#b89fd4','--surface':'#0c0a14','--surface2':'#161428','--border':'rgba(155,127,196,0.12)','--muted':'rgba(155,127,196,0.45)' },
  cs_frost:       { '--accent':'#7fc4d4','--accent2':'#4a8fa8','--accent3':'#a0dce8','--surface':'#080e14','--surface2':'#0e1828','--border':'rgba(127,196,212,0.12)','--muted':'rgba(127,196,212,0.45)' },
  cs_copper:      { '--accent':'#c4845a','--accent2':'#a0603c','--accent3':'#d8a070','--surface':'#0e0800','--surface2':'#1a1008','--border':'rgba(196,132,90,0.12)','--muted':'rgba(196,132,90,0.45)' },
  cs_ember_glow:  { '--accent':'#e08040','--accent2':'#c05010','--accent3':'#f09060','--surface':'#100600','--surface2':'#1e0e00','--border':'rgba(224,128,64,0.12)','--muted':'rgba(224,128,64,0.45)' },
  cs_rust_blush:  { '--accent':'#c46060','--accent2':'#a04040','--accent3':'#d48080','--surface':'#0e0404','--surface2':'#180808','--border':'rgba(196,96,96,0.12)','--muted':'rgba(196,96,96,0.45)' },
};
...
function buyCosmPack(packId) {
  ...
  // Yoru packs use Yoru Coins
  if (pack.yoruOnly) {
    const currentYoru = typeof animePlayerCoins !== 'undefined' ? animePlayerCoins : 0;
    if (currentYoru < pack.packCost) {
      rewardPopup(`Need ${pack.packCost} Yoru Coins! You have ${currentYoru}.`); return;
    }
    animePlayerCoins -= pack.packCost;
    try { localStorage.setItem('animePlayerCoins', animePlayerCoins); } catch(_) {}
    const coinEl = document.getElementById('animePlayerCoins');
    if (coinEl) coinEl.textContent = animePlayerCoins;
    const balEl = document.getElementById('animeShopBalance');
    if (balEl) balEl.textContent = animePlayerCoins;
...
function buyCosmPackItem(packId, itemId) {
  ...
  // Yoru packs use Yoru Coins (animePlayerCoins), not sacraments
  if (pack.yoruOnly && itemDef.yoruCost > 0) {
    const currentYoru = typeof animePlayerCoins !== 'undefined' ? animePlayerCoins : 0;
    if (currentYoru < itemDef.yoruCost) {
      rewardPopup(`Need ${itemDef.yoruCost} Yoru Coins! You have ${currentYoru}.`); return;
    }
    animePlayerCoins -= itemDef.yoruCost;
    try { localStorage.setItem('animePlayerCoins', animePlayerCoins); } catch(_) {}
    const coinEl = document.getElementById('animePlayerCoins');
    if (coinEl) coinEl.textContent = animePlayerCoins;
    const balEl = document.getElementById('animeShopBalance');
    ...
    saveCosmPacks();
    rewardPopup('Unlocked: ' + itemDef.name + '! (' + itemDef.yoruCost + ' Yoru Coins)');
...
function buyVialFromShop() {
  ...
  if (gambleCurrency < cost) { rewardPopup('Need 200 gold!'); return; }
  gambleCurrency -= cost;
  vialCount++;
  saveVials();
  updateCurrencies();
...
function updateCurrencies() {
  ...
  const sbGC = document.getElementById('sbGambleCoins');
  if (sbGC) sbGC.textContent = gambleCurrency;
...
function rewardGamble(amount) {
  ...
  updateCurrencies();
  rewardPopup('+' + amount + ' <img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/gold_coin_stack.png" style="width:14px;height:14px;vertical-align:-2px;image-rendering:pixelated" alt=""> gold');
}
...
document.addEventListener('click', (e) => {
  ...
  if (option && !option.classList.contains('locked')) {
    ...
    if (match) {
      ...
      if (isCorrect) {
        // Screen flash only — no gold reward for MCQ
...
function switchMode(mode, event) {
  ...
  const allModes = ['flashMode','mcqMode','casinoMode','shopMode','inventoryMode',
                    'animeMode','kanaMode','reviewMode','learnedMode','questsMode','kanjiMode'];
  allModes.forEach(id => {
...
if (mode === 'review') {
  ...
  if (mode === 'quests') {
    document.getElementById('questsMode').classList.remove('hidden');
    renderDailyQuests();
    renderAchievements();
    _updateQuestsNotifDot();
  }
  if (mode === 'kanji') {
    const kanjiEl = document.getElementById('kanjiMode');
    if (kanjiEl) kanjiEl.classList.remove('hidden');
    try { KNJ_ENGINE.openKanjiMode(); } catch(e) { console.error('[KNJ]', e); }
  }
...
function _shopBuyItem(item) {
  ...
  if (ownedAlready) {
    ...
    updateCurrencies();
    rewardPopup('Already owned! +' + gemReward + ' gold (duplicate)');
    generateShopMain();
    return;
  }
  if (gambleCurrency < item.cost) {
    rewardPopup('Not enough gold! Need ' + item.cost);
    return;
  }
...
function shopGetWeeklyItem() {
  // Weekly featured = a random cosmetic pack at 15% off
  const weekNum = Math.floor(Date.now() / (7 * 24 * 60 * 60 * 1000));
  const shopPacks = COSMETIC_PACKS.filter(p => p.source === 'shop');
  if (!shopPacks.length) return null;
  return shopPacks[weekNum % shopPacks.length];
}
...
function renderShopWeeklyItem() {
  const pack = shopGetWeeklyItem();
  const existing = document.getElementById('shopWeeklyPanel');
  if (existing) existing.remove();
  const container = document.getElementById('shopPacksMain');
  if (!container || !pack) return;
  const panel = document.createElement('div');
  panel.id = 'shopWeeklyPanel';
  panel.className = 'shop-weekly-panel';
  const owned = ownedCosmPacks[pack.id];
  const discountedCost = Math.floor(pack.packCost * 0.85);
  const moonImg = '<img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/moon_tear.png" style="width:12px;height:12px;vertical-align:-1px;image-rendering:pixelated" alt="">';
  panel.innerHTML = `
    <div class="shop-weekly-header">
      <span class="shop-weekly-badge">Featured Pack</span>
      <span class="shop-weekly-timer">Resets in ${shopWeeklyTimeLeft()}</span>
    </div>
    <div class="shop-weekly-body">
      <div class="shop-pack-color-swatch" style="background:${pack.coverColor};width:40px;height:40px;border-radius:8px;flex-shrink:0"></div>
      <div class="shop-weekly-info">
        <div class="shop-weekly-name">${pack.name}</div>
        <div class="shop-weekly-type">${pack.category.toUpperCase()} · ${pack.items.length} ITEMS</div>
        <div class="shop-weekly-desc">${pack.desc}</div>
      </div>
      <div class="shop-weekly-right">
        <div class="shop-weekly-cost">${discountedCost} ${moonImg}</div>
        <div class="shop-weekly-orig">${pack.packCost}</div>
        ${owned
          ? `<span class="shop-owned-tag">OWNED</span>`
          : `<button class="btn shop-weekly-btn">Unlock</button>`
        }
      </div>
    </div>
  `;
  if (!owned) {
    panel.querySelector('.shop-weekly-btn').onclick = () => {
      if (sacramentCurrency < discountedCost) { rewardPopup('Need more sacraments!'); return; }
      sacramentCurrency -= discountedCost;
      ownedCosmPacks[pack.id] = { ownedItems: [pack.items[0].id] };
      saveCosmPacks();
      updateCurrencies();
      rewardPopup('Pack unlocked: ' + pack.name + '!');
      generateShopMain();
    };
  }
...
function generateShopMain() {
  ...
  // Render weekly featured pack first
  renderShopWeeklyItem();

  // Update shop completion bar for packs
  shopRenderPacksCompletionBar();

  container.innerHTML = '';

  // ── Cosmetic Packs section ──────────────────────────────────
  const filteredPacks = COSMETIC_PACKS.filter(p => {
    if (p.source !== 'shop') return false;
    if (activeCat === 'all' || activeCat === 'packs') return true;
    if (activeCat === 'bg' && p.category === 'bg') return true;
    if (activeCat === 'colorset' && p.category === 'colorset') return true;
    return false;
  });

  if (filteredPacks.length && (activeCat === 'all' || activeCat === 'packs' || activeCat === 'bg' || activeCat === 'colorset')) {
    const sectionHeader = document.createElement('div');
    sectionHeader.className = 'shop-section-header';
    sectionHeader.textContent = 'Cosmetic Packs';
    container.appendChild(sectionHeader);

    const packGrid = document.createElement('div');
    packGrid.className = 'shop-pack-grid';

    filteredPacks.forEach(pack => {
      const owned = ownedCosmPacks[pack.id];
      const ownedCount = owned ? owned.ownedItems.length : 0;
      const discountCost = pack.packCost;
      const canAfford = sacramentCurrency >= discountCost;

      const card = document.createElement('div');
      card.className = 'shop-pack-card' + (owned ? ' owned' : '') + (!canAfford && !owned ? ' cant-afford' : '');
      card.style.background = pack.coverColor;
      card.innerHTML = owned
        ? `<div class="shop-pack-owned-badge">Owned</div>
           <div class="shop-pack-name">\${pack.name}</div>
           <div class="shop-pack-meta">\${ownedCount}/\${pack.items.length} unlocked</div>
           <button class="shop-pack-action-btn owned-btn" onclick="event.stopPropagation();_shopOpenPackDetail('\${pack.id}')">View Items</button>`
        : `<div class="shop-pack-name">\${pack.name}</div>
           <div class="shop-pack-meta">\${pack.items.length} items · \${pack.category}</div>
           <div class="shop-pack-cost">\${discountCost} <img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/moon_tear.png" style="width:11px;height:11px;vertical-align:-1px;image-rendering:pixelated" alt=""></div>
           <button class="shop-pack-action-btn \${canAfford?'':'disabled-btn'}" onclick="event.stopPropagation();buyCosmPack('\${pack.id}')" \${canAfford?'':'disabled'}>
             \${canAfford ? 'Unlock Pack' : 'Need more sacraments'}
           </button>`;
      packGrid.appendChild(card);
    });
  ...
  // ── Titles section ────────────────────────────────────────
  if (activeCat === 'all' || activeCat === 'title') {
    const titlePool = shopPool.filter(i => i.type === 'title');
    if (titlePool.length) {
      const cycleN = Math.floor(now / shopCycleTime);
      const seeded = [...titlePool].sort((a, b) => {
        return Math.sin(cycleN * 17.3 + titlePool.indexOf(a) * 43.1) - Math.sin(cycleN * 17.3 + titlePool.indexOf(b) * 43.1);
      });
      const shown = seeded.slice(0, 6);

      const sectionHeader = document.createElement('div');
      sectionHeader.className = 'shop-section-header';
      sectionHeader.textContent = 'Titles';
      container.appendChild(sectionHeader);

      const listEl = document.createElement('div');
      listEl.className = 'shop-items-list';
      shown.forEach(item => {
        const owned = inventory.find(i => i.id === item.id);
        const canAfford = sacramentCurrency >= item.cost;
        const div = document.createElement('div');
        div.className = 'shop-item' + (!canAfford && !owned ? ' shop-item--locked' : '');
        div.innerHTML = `
          <div style="flex:1;min-width:0">
            <div style="font-weight:700">${!canAfford && !owned ? '🔒 ' : ''}${item.name.replace('Title: ','')}</div>
            <div style="font-family:'Space Mono',monospace;font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:1px;margin-top:2px">Title</div>
          </div>
          ${owned
            ? '<span class="shop-owned-tag">OWNED</span>'
            : canAfford
              ? `<button class="btn btn-secondary" style="font-size:12px;padding:6px 12px;white-space:nowrap;margin-left:10px" onclick="void(0)">Buy ${item.cost} <img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/moon_tear.png" style="width:11px;height:11px;vertical-align:-1px;image-rendering:pixelated" alt=""></button>`
              : `<button class="btn" style="font-size:12px;padding:6px 12px;white-space:nowrap;margin-left:10px;opacity:0.4;cursor:not-allowed" disabled>${item.cost} <img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/moon_tear.png" style="width:11px;height:11px;vertical-align:-1px;image-rendering:pixelated" alt=""></button>`
          }
        `;
        if (!owned && canAfford) {
          div.querySelector('button').onclick = () => _shopBuyTitleWithSacraments(item);
        }
  ...
  // ── Consumables section ───────────────────────────────────
  if (activeCat === 'all' || activeCat === 'consumable') {
    const consSection = document.createElement('div');
    consSection.className = 'shop-section-header';
    consSection.textContent = 'Consumables';
    container.appendChild(consSection);

    const consDiv = document.createElement('div');
    consDiv.className = 'shop-items-list';
    const vialCanAfford = gambleCurrency >= 200;
    const vialEl = document.createElement('div');
    vialEl.className = 'shop-item' + (!vialCanAfford ? ' shop-item--locked' : '');
    vialEl.innerHTML = `
      <img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/vial1.png" style="width:28px;height:28px;image-rendering:pixelated;margin-right:8px;flex-shrink:0" alt="">
      <div style="flex:1;min-width:0">
        <div style="font-weight:700">Vial</div>
        <div style="font-family:'Space Mono',monospace;font-size:10px;color:var(--muted)">+\${VIAL_SACRAMENT_AMOUNT} sacraments · You have \${vialCount}</div>
      </div>
      \${vialCanAfford
        ? '<button class="btn btn-secondary" style="font-size:12px;padding:6px 12px;white-space:nowrap;margin-left:10px">Buy 200 gold</button>'
        : '<button class="btn" style="font-size:12px;padding:6px 12px;white-space:nowrap;margin-left:10px;opacity:0.4;cursor:not-allowed" disabled>Need 200 gold</button>'
      }
    `;
    if (vialCanAfford) {
      vialEl.querySelector('button').onclick = () => buyVialFromShop();
    }
    consDiv.appendChild(vialEl);
    container.appendChild(consDiv);
  }

  if (container.children.length === 0) {
    container.innerHTML = '<p style="color:var(--muted);font-size:13px;padding:12px 0">Nothing here yet.</p>';
  }
...
function _shopOpenPackDetail(packId) {
  // Switch to bag → cosmetics and open pack detail
  switchMode('inventory');
  setTimeout(() => {
    setBagTab('new', document.getElementById('bagTabNew'));
    _bagPackDetailOpen = packId;
    renderBagNew();
  }, 80);
}

function _shopBuyTitleWithSacraments(item) {
  const owned = inventory.find(i => i.id === item.id);
  if (owned) {
    const sacramentReward = Math.max(10, Math.floor(item.cost * 0.15));
    sacramentCurrency += sacramentReward;
    updateCurrencies();
    rewardPopup('Already owned! +' + sacramentReward + ' sacraments');
    generateShopMain();
    return;
  }
  if (sacramentCurrency < item.cost) { rewardPopup('Need more sacraments!'); return; }
  sacramentCurrency -= item.cost;
  addToInventory({id: item.id, name: item.name, type: item.type, data: item.data || item.name});
  updateCurrencies();
  rewardPopup('Unlocked: ' + item.name + ' — check Bag!');
  generateShopMain();
...
function shopRenderPacksCompletionBar() {
  const shopPacks = COSMETIC_PACKS.filter(p => p.source === 'shop');
  const total = shopPacks.length;
  const owned = shopPacks.filter(p => ownedCosmPacks[p.id]).length;
  const pct = total ? Math.round(owned / total * 100) : 0;
  let el = document.getElementById('shopCompletionBar');
  const container = document.getElementById('shopPacksMain');
  if (!container) return;
  ...
  el.innerHTML = `
    <div class="shop-completion-label">
      <span>Collection</span>
      <span class="shop-completion-pct">\${owned} / \${total} packs · \${pct}%</span>
    </div>
    <div class="shop-completion-track"><div class="shop-completion-fill" style="width:\${pct}%"></div></div>
  `;
}
...
function setSlotBet(amount, el) {
  ...
  if (el) el.classList.add('active');
  document.getElementById('slotBetLabel').textContent = `Bet: ${amount} gold`;
}
...
function spinSlots5() {
  ...
  if (gambleCurrency < slotBet) { rewardPopup('Not enough gold!'); return; }
...
function evaluateSlots5(lockedBet) {
  ...
  if (wins.length > 0) {
    ...
    resultEl.innerHTML = `<span class="${cls}">+${payout} gold${wins.length > 1 ? ` <span style="font-size:11px;opacity:0.7">(${wins.length} wins!)</span>` : ''}</span>`;
...
function triggerJackpot(payout) {
  ...
  // Popup
  setTimeout(() => rewardPopup(`🎉 JACKPOT! +${payout} gold`), 200);
}
...
function setBjBet(amount, el) {
  ...
  document.getElementById('bjBetDisplay').textContent = `Bet: ${bjBet} gold`;
}
...
function updateStreak(correct) {
  if (correct) {
    ...
    // Quest: reach 10-answer streak (pass absolute value so quest knows high-water mark)
    dailyQuestsTrack('mcq_streak', mcqStreak);
  } else {
    mcqStreak = 0;
...
function openLootBox(rarity) {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      rewardsEl.innerHTML = `
        <div class="loot-reward-row"><img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/gold_coin_stack.png" style="width:14px;height:14px;vertical-align:-2px;image-rendering:pixelated" alt=""> <strong>+${money}</strong> <span>gold</span></div>
...
function dealerConvert() {
  ...
  if (result) result.innerHTML = `Converted ${amount} <img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/moon_tear.png" style="width:14px;height:14px;vertical-align:-2px;image-rendering:pixelated" alt=""> → ${coins} <img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/gold_coin_stack.png" style="width:14px;height:14px;vertical-align:-2px;image-rendering:pixelated" alt=""> gold`;
  const dt = document.getElementById('dealerSacraments');
  const dm = document.getElementById('dealerMoney');
  if (dt) dt.textContent = sacramentCurrency;
  if (dm) dm.textContent = gambleCurrency;
  rewardPopup(`Converted! +${coins} <img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/gold_coin_stack.png" style="width:14px;height:14px;vertical-align:-2px;image-rendering:pixelated" alt=""> gold`);
  input.value = '';
}
...
function dealerQuick(tickets) {
  ...
  updateCurrencies();
  const result = document.getElementById('dealerResult');
  if (result) result.innerHTML = `Converted ${tickets} <img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/moon_tear.png" style="width:14px;height:14px;vertical-align:-2px;image-rendering:pixelated" alt=""> → ${coins} <img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/gold_coin_stack.png" style="width:14px;height:14px;vertical-align:-2px;image-rendering:pixelated" alt=""> gold`;
  const dt = document.getElementById('dealerSacraments');
  const dm = document.getElementById('dealerMoney');
  if (dt) dt.textContent = sacramentCurrency;
  if (dm) dm.textContent = gambleCurrency;
  rewardPopup(`+${coins} <img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/gold_coin_stack.png" style="width:14px;height:14px;vertical-align:-2px;image-rendering:pixelated" alt=""> gold`);
}
...
function ensureMascotVisible() {
  if (!mascotImg) return;
  if (mascotImg.classList.contains('hidden')) return; // don't force-show if intentionally hidden
  // Only write if actually different to avoid layout thrashing
  if (mascotImg.style.display !== 'block') mascotImg.style.display = 'block';
  if (mascotImg.style.opacity !== '0.92') mascotImg.style.opacity = '0.92';
}

setInterval(ensureMascotVisible, 2000); // reduced from 1000ms
...
// Use requestAnimationFrame instead of setInterval for mascot overlay sync
// Only runs when mascot is actually visible - avoids constant layout thrashing
(function _mascotOverlayRAF() {
  let _rafRunning = false;
  let _lastSync = 0;
  function _rafTick(ts) {
    if (!_isMascotActuallyVisible()) {
      _rafRunning = false;
      return; // stop RAF loop when mascot hidden
    }
    if (ts - _lastSync > 250) { // max 4fps for overlay positioning
      _lastSync = ts;
      syncAllMascotOverlays();
    }
    requestAnimationFrame(_rafTick);
  }
  window._startMascotOverlayRAF = function() {
    if (!_rafRunning) { _rafRunning = true; requestAnimationFrame(_rafTick); }
  };
  // Also sync on scroll/resize but debounced
  let _debTimer = null;
  function _debSync() { clearTimeout(_debTimer); _debTimer = setTimeout(syncAllMascotOverlays, 100); }
  window.addEventListener('scroll', _debSync, { passive: true });
  window.addEventListener('resize', _debSync, { passive: true });
})();
...
function toggleMascotVisible() {
  ...
  if (typeof window._startMascotOverlayRAF === "function") window._startMascotOverlayRAF();
...
document.addEventListener('DOMContentLoaded', () => {
  // KNJ: Apply saved theme on load
  const _knjTheme = localStorage.getItem('knj_theme') || 'dark';
  document.documentElement.dataset.theme = _knjTheme;
...
// ============================================================
// ALBUM CD WIDGET — YouTube playlist audio player
// ============================================================
let _cdPlayer       = null;   // global YT.Player instance (shared)
let _cdPlayerReady  = false;  // true once onReady fires
let _cdActiveKey    = null;   // slot key of the currently playing CD
let _cdPendingPlay  = null;   // key to play once player is ready

// Called by YouTube IFrame API once it loads
window.onYouTubeIframeAPIReady = function() {
  // Player created lazily on first use — nothing to do here
};

function _cdEnsurePlayer(playlistId, startIndex, onReadyCb) {
  if (_cdPlayer && _cdPlayerReady) {
    if (playlistId) {
      _cdPlayer.loadPlaylist({ listType: 'playlist', list: playlistId, index: startIndex || 0 });
      _cdPlayer.pauseVideo(); // load but don't auto-play
    }
  ...
  let holderEl = document.getElementById('_cdPlayerHolder');
  if (!holderEl) {
    holderEl = document.createElement('div');
    holderEl.id = '_cdPlayerHolder';
    holderEl.style.cssText = 'position:fixed;width:0;height:0;overflow:hidden;pointer-events:none;opacity:0;z-index:-1';
    document.body.appendChild(holderEl);
  }

  if (!_cdPlayer) {
    _cdPlayer = new YT.Player('_cdPlayerHolder', {
      width: '1',
      height: '1',
      playerVars: {
        listType: 'playlist',
      ...
      events: {
        onReady: function(e) {
          ...
          if (onReadyCb) onReadyCb();
          if (_cdPendingPlay) { _cdPlay(_cdPendingPlay); _cdPendingPlay = null; }
        },
        onStateChange: function(e) {
          // YT.PlayerState.ENDED = 0; loop by replaying playlist
          if (e.data === 0) { try { _cdPlayer.playVideoAt(0); } catch(_) {} }
          // Keep play/pause button in sync
          _cdSyncAllButtons();
        },
      },
    });
...
function _cdPlay(key) {
  // Stop any other active CD first
  if (_cdActiveKey && _cdActiveKey !== key) {
    const prevDisc = document.querySelector(`#cd-${_cdActiveKey.replace('-','_')} .cd-disc`);
    if (prevDisc) prevDisc.classList.remove('spinning');
  }
  _cdActiveKey = key;

  if (!_cdPlayerReady) {
    _cdPendingPlay = key;
    const w = _cdGetWidget(key);
    if (w) _cdEnsurePlayer(w.playlistId, 0, null);
    return;
  }
  try {
    _cdPlayer.playVideo();
    const disc = document.querySelector(`#cd-${key.replace('-','_')} .cd-disc`);
...
function _cdPause() {
  if (!_cdPlayerReady || !_cdPlayer) return;
  try {
    _cdPlayer.pauseVideo();
    const disc = _cdActiveKey ? document.querySelector(`#cd-${_cdActiveKey.replace('-','_')} .cd-disc`) : null;
    if (disc) disc.classList.remove('spinning');
    _cdSyncAllButtons();
  } catch(e) {}
}

function _cdToggle(key) {
  if (!_cdPlayerReady) {
    // First click — init player and play
    const w = _cdGetWidget(key);
    if (!w) return;
    _cdActiveKey = key;
    _cdPendingPlay = key;
    _cdEnsurePlayer(w.playlistId, 0, null);
    return;
  }
  try {
    const state = _cdPlayer.getPlayerState();
    // YT.PlayerState.PLAYING = 1
    if (_cdActiveKey !== key) {
      // Different CD clicked — switch to it
      const w = _cdGetWidget(key);
      if (w) {
        _cdPlayer.loadPlaylist({ listType: 'playlist', list: w.playlistId, index: 0 });
      }
      _cdPlay(key);
    } else if (state === 1) {
      _cdPause();
...
function _cdNext() {
  if (!_cdPlayerReady || !_cdPlayer) return;
  try { _cdPlayer.nextVideo(); } catch(e) {}
}

function _cdPrev() {
  if (!_cdPlayerReady || !_cdPlayer) return;
  try { _cdPlayer.previousVideo(); } catch(e) {}
}

function _cdGetWidget(key) {
  const entry = widgetSlots[key];
  if (!entry) return null;
  return WIDGET_CATALOG.find(w => w.id === entry.widgetId) || null;
}

function _cdSyncAllButtons() {
  // Update all play/pause buttons on all CD widgets
  document.querySelectorAll('.cd-playpause').forEach(btn => {
    const key = btn.dataset.cdKey;
    if (!key) return;
    let isPlaying = false;
    try {
      isPlaying = _cdActiveKey === key && _cdPlayerReady && _cdPlayer && _cdPlayer.getPlayerState() === 1;
    } catch(e) {}
...
// ── Today's Kanji widget renderer ──────────────────────────────
function _buildTodayKanjiWidget(w, key) {
  const db = APP_KANJI_DB || (typeof KANJI_DB !== 'undefined' ? KANJI_DB : []);
  if (!db.length) return '<div style="padding:12px;color:var(--muted);font-size:11px">Loading...</div>';
  // Seed by date so it changes daily but is consistent across sessions
  const dateStr = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
  const seed = dateStr.split('-').reduce((a, v) => a + parseInt(v), 0);
  const kanji = db[seed % db.length];
  const cardId = 'tkw_' + key.replace('-', '_');
  return `
    <button class="widget-slot-remove" onclick="event.stopPropagation();removeWidgetSlot('${key}')" title="Remove">✕</button>
    <div class="widget-slot-name">${w.name}</div>
    <div class="today-kanji-widget" id="${cardId}" onclick="_tkwFlip('${cardId}')">
      <div class="tkw-front">
        <div class="tkw-date">${dateStr}</div>
        <div class="tkw-char">${kanji.char}</div>
        <div class="tkw-meaning">${kanji.meaning || ''}</div>
        <div class="tkw-tap-hint">tap to reveal →</div>
      </div>
      <div class="tkw-back hidden">
        <div class="tkw-char tkw-char--small">${kanji.char}</div>
        <div class="tkw-reading-row"><span class="tkw-reading-label">On</span><span class="tkw-reading-val">${kanji.on || '—'}</span></div>
        <div class="tkw-reading-row"><span class="tkw-reading-label">Kun</span><span class="tkw-reading-val">${kanji.kun || '—'}</span></div>
        <div class="tkw-reading-row"><span class="tkw-reading-label">JLPT</span><span class="tkw-reading-val">${(kanji.jlpt || '').toUpperCase()}</span></div>
        <button class="tkw-trace-btn" onclick="event.stopPropagation();switchMode('flash');setTimeout(()=>{setFlashTab('trace')},300)">✍️ Trace it</button>
      </div>
    </div>`;
}

window._tkwFlip = function(id) {
  const el = document.getElementById(id);
  if (!el) return;
  const front = el.querySelector('.tkw-front');
  const back  = el.querySelector('.tkw-back');
...
function buildInteractiveWidgetHTML(w, key, span, wcols) {
  ...
  if (w.interactive === 'albumCd') {
    const safeKey = key.replace('-', '_');
    const discUrl = w.discImgUrl || '';
    return `
      <div class="cd-widget" id="cd-${safeKey}">
        <img class="cd-disc" src="${discUrl}" alt="${w.name}" draggable="false">
        <div class="cd-controls">
          <button class="cd-btn" onclick="event.stopPropagation();_cdPrev()" title="Previous">‹</button>
          <button class="cd-btn cd-playpause" id="cd-pp-${safeKey}" data-cd-key="${key}" onclick="event.stopPropagation();_cdToggle('${key}')">▶</button>
          <button class="cd-btn" onclick="event.stopPropagation();_cdNext()" title="Next">›</button>
        </div>
      </div>
      <button class="widget-slot-remove" onclick="event.stopPropagation();removeWidgetSlot('${key}')" title="Remove">✕</button>
      <div class="widget-slot-name">${w.name}</div>`;
  ...
  if (w.interactive === 'todayKanji') {
    return _buildTodayKanjiWidget(w, key);
  }
...
function initInteractiveWidget(w, key, span, wcols) {
  if (w.interactive === 'albumCd') {
    // Pre-warm the YT player lazily so it's ready when user clicks
    // Only init if YT API is already loaded
    if (typeof YT !== 'undefined' && YT.Player) {
      _cdEnsurePlayer(w.playlistId, 0, null);
    }
    // If YT not loaded yet it'll be ready when they click
  }
  if (w.interactive === 'grok') {
...
const REVIEW_SETS = [
  ...
  { id: 'today',    label: 'Today Learned',    icon: '★',  desc: 'Cards added to vault today — reinforce while fresh', source: 'today' },
  { id: 'random',   label: 'Random Mix',       icon: '?',  desc: 'Random draw from all vault items — ignores SR intervals', source: 'random' },
...
let _revConsecFails = {}; // tracks consecutive fails per card this session
let _revIsolationActive = false; // true while a focused drill overlay is open
...
function revBuildCards(set) {
  ...
  // 'today' source — items added to vault today
  if (set.source === 'today') {
    try {
      const vault = JSON.parse(localStorage.getItem('learnedVault') || '[]');
      ...
      return vault
        .filter(item => {
          const ts = item.dateAdded || (item.addedAt ? new Date(item.addedAt).getTime() : 0);
          return ts >= todayStart && ts < todayEnd;
        })
        .map(item => ({
          q: item.term || item.char || item.q || item.front || String(item.id || ''),
          a: item.meaning || item.reading || item.a || item.back || '',
          type: 'vault',
          _fromVault: true,
        })).filter(c => c.q);
    } catch(_) { return []; }
  }
  if (set.source === 'random') {
    // Pull all vault items + all flashcards, shuffle fully (ignore SR)
    const vault = lvLoad ? lvLoad() : (JSON.parse(localStorage.getItem('learnedVault') || '[]'));
    const vaultCards = vault.map(item => ({
      id:   item.id || ('v_' + (item.term || '')),
      type: 'vault',
      q:    item.term || item.char || item.q || '',
      a:    [item.reading, item.meaning].filter(Boolean).join(' — ') || item.a || '',
      _fromVault: true,
    })).filter(c => c.q);
    const allCards = [
      ...vaultCards,
      ...(APP_FLASHCARDS || []).map(c => ({ ...c })),
    ];
    ...
    return allCards.slice(0, 40); // cap at 40 per session
  }
...
function revInit() {
  ...
  grid.innerHTML = REVIEW_SETS.map(set => {
    ...
    const isToday = set.source === 'today';
    const isRandom = set.source === 'random';
    const count = isQueue ? qLen : total;
    const badge = isQueue && qLen > 0
      ? '<span class="rev-set-badge">' + qLen + ' due</span>'
      : isToday && total > 0
        ? '<span class="rev-set-badge rev-set-badge--neutral">today</span>'
        : isRandom
          ? '<span class="rev-set-badge rev-set-badge--neutral">shuffle</span>'
          : pct >= 80
          ? '<span class="rev-set-badge rev-set-badge--strong">★ ' + pct + '%</span>'
          : total > 0
            ? '<span class="rev-set-badge rev-set-badge--neutral">' + pct + '%</span>'
            : '';
    const isEmpty = (isQueue && qLen === 0) || (isToday && total === 0);
    return `<div class="rev-set-card ${isEmpty ? 'rev-set-card--empty' : ''}" onclick="revPreviewSet('${set.id}')">
      ...
      <div class="rev-set-info">
        ...
        ${!isQueue && !isRandom && total > 0 ? '<div class="rev-set-bar"><div class="rev-set-bar-fill" style="width:' + pct + '%"></div></div>' : ''}
      </div>
      <div class="rev-set-count">${isRandom ? '40' : (count > 0 ? count : '—')}</div>
    </div>`;
  }).join('');
}
...
function revStartSet(setId) {
  ...
  _revConsecFails = {}; // { cardQ: failCount } reset each session
...
function revGrade(gradeBtn) {
  ...
  // Update dimension mastery on vault item
  (function() {
    try {
      const dim = (revActiveSet === 'n5' || revActiveSet === 'n4' || revActiveSet === 'random')
        ? 'reading'   // flashcard review = reading dimension
        : null;
      if (!dim) return;
      const masteryGain = gradeBtn >= 2 ? 8 : gradeBtn === 1 ? 3 : 0;
      if (!masteryGain) return;
      const vault = lvLoad ? lvLoad() : JSON.parse(localStorage.getItem('learnedVault') || '[]');
      const idx = vault.findIndex(v => v.term === card.q || v.id === card.id);
      if (idx === -1) return;
      if (!vault[idx].mastery) vault[idx].mastery = { meaning: 20, reading: 0, writing: 0 };
      vault[idx].mastery[dim] = Math.min(100, (vault[idx].mastery[dim] || 0) + masteryGain);
      if (lvSave) lvSave(vault); else localStorage.setItem('learnedVault', JSON.stringify(vault));
    } catch(_) {}
  })();
  ...
  // Track consecutive fails for isolation
  if (gradeBtn === 0) {
    _revConsecFails[card.q] = (_revConsecFails[card.q] || 0) + 1;
    if (_revConsecFails[card.q] >= 3 && !_revIsolationActive) {
      _revTriggerIsolation(card);
    }
  } else {
    _revConsecFails[card.q] = 0; // reset on any correct-ish grade
  }
...
// ============================================================
// 3-CONSECUTIVE-FAIL ISOLATION (Phase 2.3)
// When a card fails 3 times in a row, pull it into a focused
// mini-drill overlay so the user can consolidate it before
// continuing the main session.
// ============================================================

function _revTriggerIsolation(card) {
  _revIsolationActive = true;
  const overlay = document.getElementById('revIsolationOverlay');
  if (!overlay) { _revIsolationActive = false; return; }

  // Build a tiny 5-card focused drill of just this card repeated
  // with a prompt to focus and then a self-rate
  document.getElementById('revIsoCardQ').textContent = card.q;
  document.getElementById('revIsoCardA').textContent =
    (card.a || '').split('\n')[0].slice(0, 80);
  document.getElementById('revIsoCardBack').classList.add('hidden');
  document.getElementById('revIsoCardFront').classList.remove('hidden');
  document.getElementById('revIsoResult').textContent = '';

  _revIsoCard = card;
  _revIsoDrillsLeft = 3; // show card 3 more times in isolation before returning
  _revIsoPhase = 'front';

  overlay.classList.add('show');
}

let _revIsoCard = null;
let _revIsoDrillsLeft = 0;
let _revIsoPhase = 'front';

function revIsoFlip() {
  document.getElementById('revIsoCardBack').classList.remove('hidden');
  document.getElementById('revIsoCardFront').classList.add('hidden');
  document.getElementById('revIsoResult').textContent = '';
  // Mirror the question on the back face too
  const q2 = document.getElementById('revIsoCardQ2');
  if (q2 && _revIsoCard) q2.textContent = _revIsoCard.q;
  _revIsoPhase = 'back';
}

function revIsoGrade(knew) {
  if (_revIsoPhase !== 'back') return;
  if (knew) {
    _revIsoDrillsLeft--;
    if (_revIsoDrillsLeft <= 0) {
      // Done! Close isolation and mark consec fails reset
      _revConsecFails[_revIsoCard.q] = 0;
      _revIsolationActive = false;
      document.getElementById('revIsolationOverlay').classList.remove('show');
      rewardPopup('Isolated: +consolidated');
      return;
    }
    document.getElementById('revIsoResult').textContent = 'Good — ' + _revIsoDrillsLeft + ' more';
  } else {
    _revIsoDrillsLeft = Math.min(_revIsoDrillsLeft + 1, 5);
    document.getElementById('revIsoResult').textContent = 'Try again — ' + _revIsoDrillsLeft + ' left';
  }
  // Show front again
  setTimeout(() => {
    document.getElementById('revIsoCardBack').classList.add('hidden');
...
function revIsoDismiss() {
  _revConsecFails[_revIsoCard ? _revIsoCard.q : ''] = 0;
  _revIsolationActive = false;
  document.getElementById('revIsolationOverlay').classList.remove('show');
}
...
function traceGotIt() {
  ...
  if (kanji) {
    ...
    if (!vault.some(v => v.term === kanji.char && v.type === 'trace')) {
      vault.push({ id: 'tr_' + kanji.id, type: 'trace', term: kanji.char, reading: kanji.kun, meaning: kanji.meaning, dateAdded: Date.now(), source: 'trace', mastery: { meaning: 0, reading: 0, writing: 30 } });
...
function lvDimBarsHTML(item, detailed) {
  const dim = lvDimMastery(item);
  const dims = [
    { key: 'meaning', label: 'M', color: '#a78bfa', title: 'Meaning' },
    { key: 'reading', label: 'R', color: '#60a5fa', title: 'Reading' },
    { key: 'writing', label: 'W', color: '#34d399', title: 'Writing' },
  ];
  if (detailed) {
    return dims.map(d => `
      <div class="lv-dim-row">
        <span class="lv-dim-label" title="${d.title}">${d.title}</span>
        <div class="lv-dim-track"><div class="lv-dim-fill" style="width:${dim[d.key]}%;background:${d.color}"></div></div>
        <span class="lv-dim-pct">${dim[d.key]}%</span>
      </div>`).join('');
  }
  return `<div class="lv-dim-mini">${dims.map(d =>
    `<div class="lv-dim-mini-bar" title="${d.title}: ${dim[d.key]}%">
      <div class="lv-dim-mini-fill" style="height:${dim[d.key]}%;background:${d.color}"></div>
    </div>`
  ).join('')}</div>`;
}

function lvDimMastery(item) {
  // Returns { meaning: 0-100, reading: 0-100, writing: 0-100 }
  const base = item.mastery ? { ...item.mastery } : { meaning: 0, reading: 0, writing: 0 };
  // Also check SR state to boost reading dimension from review history
  try {
    ...
    for (const s of sets) {
      ...
      const state = parsed[item.term] || parsed[item.q || ''];
      if (state) {
        const reps = Math.min(state.reps || 0, 10);
        const ease = Math.min((state.ease || 2.5) - 1.3, 2.2);
        const srPct = Math.min(100, Math.round((reps / 10) * 60 + (ease / 2.2) * 40));
        base.reading = Math.max(base.reading, srPct);
        break;
      }
    }
  } catch(_) {}
  // Trace source gets writing credit
  if (item.source === 'trace') base.writing = Math.max(base.writing, 30);
  return base;
}

function lvMasteryPct(item) {
  const dim = lvDimMastery(item);
  // Overall = weighted average: reading 40%, meaning 40%, writing 20%
  return Math.round(dim.reading * 0.4 + dim.meaning * 0.4 + dim.writing * 0.2);
}
...
function lvAddToVault(item) {
  ...
  // Attach initial dimension mastery if missing
  if (!item.mastery) {
    const base = item.source === 'trace' ? { meaning: 0, reading: 0, writing: 30 }
               : item.source === 'flash' ? { meaning: 30, reading: 0, writing: 0 }
               : { meaning: 20, reading: 0, writing: 0 };
    item = { ...item, mastery: base };
  }
  const vault = lvLoad();
  ...
  grid.innerHTML = items.map(item => {
    ...
    return `
      <div class="lv-card" onclick="lvOpenDetail(${itemJson})">
        ...
        <div class="lv-dim-bars">${lvDimBarsHTML(item)}</div>
      </div>
    `;
  }).join('');
...
function lvOpenDetail(item) {
  ...
  // Mastery bars — dimensions
  const _dimEl = document.getElementById('lvDetailDimBars');
  if (_dimEl) _dimEl.innerHTML = lvDimBarsHTML(item, true);
  const pct   = lvMasteryPct(item);
...
// Use window assignment (not function declaration) to avoid hoisting self-reference
(function() {
  const _lv_origRateCard = rateCard; // captures the real rateCard at line ~873
  window.rateCard = function(knew) {
    _lv_origRateCard(knew);
    if (knew) {
      try {
...
function renderShopDailyItem() {
  ...
  panel.innerHTML = `
    ...
    <div class="shop-daily-body">
      ...
      <div class="shop-daily-price-col">
        <div class="shop-daily-orig"><img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/gold_coin_stack.png" style="width:14px;height:14px;vertical-align:-2px;image-rendering:pixelated" alt=""> ${item.cost}</div>
        <div class="shop-daily-disc"><img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/gold_coin_stack.png" style="width:14px;height:14px;vertical-align:-2px;image-rendering:pixelated" alt=""> ${discount}</div>
        ${owned
  ...
  if (!owned) {
    ...
    if (buyBtn) {
      buyBtn.onclick = () => {
        if (gambleCurrency < discount) { rewardPopup('Not enough gold!'); return; }
  ...
  _shopBuyItem = function(item) {
    ...
    if (item.type === 'consumable' && item.data === 'vial1') {
      ...
      if (gambleCurrency < vialCoinCost) {
        rewardPopup('Not enough gold! Need ' + vialCoinCost);
        return;
      }
...
// ============================================================
// PHASE 3.2 — Quick trace mode, Speed trace, Pin kanji
// ============================================================
(function() {

  // ── State ───────────────────────────────────────────────────
  let _traceMode    = 'normal'; // 'normal' | 'quick' | 'speed'
  let _traceSpeedSecs = 10;
  let _traceSpeedTimer = null;
  let _traceSpeedLeft  = 10;
  let _traceSpeedInterval = null;

  // Pin set persisted to localStorage
  function _loadPinned() {
    try { return new Set(JSON.parse(localStorage.getItem('tracePinned') || '[]')); } catch(_) { return new Set(); }
  }
  function _savePinned(set) {
    localStorage.setItem('tracePinned', JSON.stringify([...set]));
  }
  let _tracePinned = _loadPinned();

  // ── Speed timer helpers ─────────────────────────────────────
  function _speedStop() {
    if (_traceSpeedTimer)    { clearTimeout(_traceSpeedTimer);    _traceSpeedTimer = null; }
    if (_traceSpeedInterval) { clearInterval(_traceSpeedInterval); _traceSpeedInterval = null; }
    _updateSpeedBar(0);
  }

  function _updateSpeedBar(pct) {
    const bar = document.getElementById('traceSpeedFill');
    if (!bar) return;
    bar.style.width = pct + '%';
    bar.style.background = pct > 50 ? 'var(--correct)' : pct > 25 ? 'var(--warn)' : 'var(--wrong)';
  }

  function _speedStart() {
    _speedStop();
    if (_traceMode !== 'speed') return;
    const wrap = document.getElementById('traceSpeedBar');
    if (wrap) wrap.style.display = '';
    _traceSpeedLeft = _traceSpeedSecs;
    _updateSpeedBar(100);
    _traceSpeedInterval = setInterval(() => {
      _traceSpeedLeft -= 0.1;
      const pct = Math.max(0, (_traceSpeedLeft / _traceSpeedSecs) * 100);
      _updateSpeedBar(pct);
      if (_traceSpeedLeft <= 0) {
        _speedStop();
        // Time's up — auto Try Again
        if (typeof traceTryAgain === 'function') traceTryAgain();
        if (typeof rewardPopup   === 'function') rewardPopup("⏱ Time's up!");
      }
    }, 100);
  }

  // ── Public: set trace mode ──────────────────────────────────
  window.setTraceMode = function(mode, el) {
    _traceMode = mode;
    document.querySelectorAll('.trace-mode-pill').forEach(b => b.classList.remove('active'));
    if (el) el.classList.add('active');
    // Hide speed bar if not speed mode
    const speedBar = document.getElementById('traceSpeedBar');
    if (speedBar) speedBar.style.display = _traceMode === 'speed' ? '' : 'none';
    _speedStop();
    // Re-init session so mode takes effect immediately
    if (typeof traceInitSession === 'function') traceInitSession();
  };

  // ── Public: toggle pin on current kanji ─────────────────────
  window.tracePinToggle = function() {
    const kanji = tracePool && tracePool[traceIdx];
    if (!kanji) return;
    const btn = document.getElementById('tracePinBtn');
    if (_tracePinned.has(kanji.char)) {
      _tracePinned.delete(kanji.char);
      if (btn) { btn.textContent = '📌 Pin'; btn.classList.remove('active'); }
      rewardPopup && rewardPopup(kanji.char + ' unpinned');
    } else {
      _tracePinned.add(kanji.char);
      if (btn) { btn.textContent = '📌 Pinned'; btn.classList.add('active'); }
      rewardPopup && rewardPopup('📌 ' + kanji.char + ' pinned!');
    }
    _savePinned(_tracePinned);
    // Refresh dot map
    _refreshDotMap();
  };

  function _refreshDotMap() {
    const dotMap = document.getElementById('traceDotMap');
    if (!dotMap || !tracePool) return;
    dotMap.innerHTML = tracePool.slice(0, Math.min(40, tracePool.length)).map((k, i) => {
      let cls = 'dot';
      if (i === traceIdx)           cls += ' current';
      if (_tracePinned.has(k.char)) cls += ' pinned';
      return `<div class="${cls}" title="${k.char}${_tracePinned.has(k.char) ? ' 📌' : ''}"></div>`;
    }).join('');
  }

  // ── Patch traceRenderCard to handle modes ───────────────────
  const _origTRCMode = window.traceRenderCard;
  window.traceRenderCard = function() {
    _speedStop();

    if (!tracePool || tracePool.length === 0) { _origTRCMode && _origTRCMode(); return; }
    if (traceIdx >= tracePool.length) { if (typeof traceShowEnd === 'function') traceShowEnd(); return; }

    const kanji = tracePool[traceIdx];
    traceStage = 0;

    // Dot map with pin markers
    _refreshDotMap();
    ...
    // Update pin button
    const pinBtn = document.getElementById('tracePinBtn');
    if (pinBtn) {
      const pinned = _tracePinned.has(kanji.char);
      pinBtn.textContent = pinned ? '📌 Pinned' : '📌 Pin';
      pinBtn.classList.toggle('active', pinned);
    }

    if (_traceMode === 'quick' || _traceMode === 'speed') {
      // Skip study phase — go straight to draw with guide hidden (stage 2 = blank)
      const studyEl = document.getElementById('traceStudyPhase');
      const drawEl  = document.getElementById('traceDrawPhase');
      const compRow = document.getElementById('traceCompareRow');
      if (studyEl) studyEl.style.display = 'none';
      if (drawEl)  drawEl.classList.remove('hidden');
      ...
      const hintR = document.getElementById('traceHintRomaji');
      const guideChar = document.getElementById('traceGuideChar');
      if (hintM) hintM.textContent = kanji.meaning || '';
      if (hintH) hintH.textContent = kanji.kun || '';
      if (hintR) hintR.textContent = (typeof _kanaToRomaji === 'function') ? _kanaToRomaji(kanji) : (kanji.romaji || '');
      if (guideChar) guideChar.textContent = kanji.char || '';

      // Quick: stage 0 (guide visible). Speed: stage 2 (blank - no guide)
      traceStage = _traceMode === 'speed' ? 2 : 0;
      if (typeof traceUpdateStageUI === 'function') traceUpdateStageUI();
      if (typeof traceClearCanvas   === 'function') traceClearCanvas();

      // Start speed timer
      if (_traceMode === 'speed') _speedStart();
    } else {
      // Normal mode — delegate to the study-phase renderer
      if (_origTRCMode) _origTRCMode();
      return;
    }
  };

  // ── Patch traceInitSession to respect pinned filter ──────────
  const _origTIS2 = window.traceInitSession;
  window.traceInitSession = function() {
    _speedStop();
    _origTIS2 && _origTIS2();
    // If filter === 'pinned', override pool with pinned chars only
    if (traceFilter === 'pinned') {
      const db = APP_KANJI_DB || (typeof KANJI_DB !== 'undefined' ? KANJI_DB : []);
      const pool = db.filter(k => _tracePinned.has(k.char));
      if (pool.length > 0) {
        tracePool = [...pool].sort(() => Math.random() - 0.5);
      } else {
        rewardPopup && rewardPopup('No pinned kanji yet — pin some with 📌 during trace!');
      }
    }
  };

  // ── Expose pinned count for UI ──────────────────────────────
  window.getTracePinnedCount = function() { return _tracePinned.size; };

  // ── Public: set speed timer duration ───────────────────────
  window._traceSetSpeed = function(secs, el) {
    _traceSpeedSecs = secs;
    document.querySelectorAll('#traceSpeedTimerRow .unit-btn').forEach(b => b.classList.remove('active'));
    if (el) el.classList.add('active');
  };

  // ── Patch setTraceFilter to support pinned ──────────────────
  const _origSTF = window.setTraceFilter;
  window.setTraceFilter = function(filter, el) {
    if (filter === 'pinned') {
      traceFilter = 'pinned';
      document.querySelectorAll('#flashTracePanel .unit-btn, .trace-filter-pill').forEach(b => b.classList.remove('active'));
      if (el) el.classList.add('active');
      traceInitSession();
    } else {
      _origSTF && _origSTF(filter, el);
    }
  };
...
// QUESTS TAB SWITCHER
// ============================================================
function setQuestTab(tab, el) {
  document.querySelectorAll('#questsMode .bag-tab').forEach(b => b.classList.remove('active'));
  if (el) el.classList.add('active');
  document.getElementById('questsPanelQuests').classList.toggle('hidden', tab !== 'quests');
  document.getElementById('questsPanelAch').classList.toggle('hidden', tab !== 'achievements');
  if (tab === 'quests') renderDailyQuests();
  if (tab === 'achievements') renderAchievements();
}

function _updateQuestsNotifDot() {
  const dot = document.getElementById('questsNotifDot');
  if (!dot) return;
  const data = _dqLoad();
  // Show dot if any quest is completable (progress >= target, not claimed)
  const hasReady = DAILY_QUESTS_DEF.some(q => {
    const state = data[q.id] || { progress: 0, claimed: false };
    return !state.claimed && (state.progress || 0) >= q.target;
  });
  dot.style.display = hasReady ? '' : 'none';
}

// Update notif dot periodically
window.addEventListener('load', () => {
  setInterval(_updateQuestsNotifDot, 30000);
  setTimeout(_updateQuestsNotifDot, 2000);
});
...
const _AN = {
  animeId:     'yofukashi',
  epId:        null,
  segments:    [],        // loaded subtitle segments
  segIdx:      -1,        // currently active segment index
  lookupCache: new Map(),
  synthVoice:  null,
  loopActive:  false,
  coinTick:    0,
  subHistory:  [],
  qsQueue:     [],        // words queued for quick study
  qsMode:      'flash',
  qsDeck:      [],
  qsIdx:       0,
  qsWrong:     [],
  qsScore:     0,
  popEntry:    null,
  videoColW:   null,      // px width of video column (set by resize)
};
...
const ANIME_CATALOG = [
  {
    ...
    titleJP: '夜ふかしの唄',
    posterUrl: 'https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/anime/cotn_thumbnail.jpg',
    episodes: [
      {
        id: 'yofukashi_ep1',
        num: 1,
        title: 'Call of the Night',
        videoUrl: 'https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/anime/cotn_ep1.mp4',
        subtitleUrl: 'https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/anime/episode1.json',
        thumbUrl: 'https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/anime/cotn_thumbnail_episode.jpg',
        duration: 1404,
        opEnd: 90,
        locked: false,
      },
    ...
    shopItems: [
      // BGM Disc widget
      { id: 'yoru_bgm_disc1', name: 'BGM Disc — Vol.1', type: 'Widget', cost: 400,
        desc: 'Unlocks the Call of the Night BGM disc widget for your sidebar.',
        action: 'widget', widgetId: 'widget_yofukashi_disc1' },
      // Titles
      { id: 'yoru_t1', name: '夜遊び (Yoru Asobi)', type: 'Title', cost: 80,
        desc: 'Night play. Equipped as your player title.', action: 'title', data: '夜遊び' },
      { id: 'yoru_t2', name: 'Child of the Night', type: 'Title', cost: 200,
        desc: 'A creature of the dark.', action: 'title', data: 'Child of the Night' },
      { id: 'yoru_t3', name: "Kou's Confidant", type: 'Title', cost: 500,
        desc: 'Trusted by the wandering boy himself.', action: 'title', data: "Kou's Confidant" },
      // Background set packs (sequential unlock within pack)
      { id: 'yoru_pack1', name: 'BG Pack 1 — Yoru Nights', type: 'BG Pack', cost: 300,
        desc: '13 backgrounds from Yofukashi no Uta. Unlock in the Bag → Cosmetics tab.',
        action: 'pack', packId: 'pack_yoru_bg1' },
      { id: 'yoru_pack2', name: 'BG Pack 2 — Yoru Dawn', type: 'BG Pack', cost: 300,
        desc: '10 backgrounds from Yofukashi no Uta. Unlock in the Bag → Cosmetics tab.',
        action: 'pack', packId: 'pack_yoru_bg2' },
    ],
  },
];

// ── Storage helpers ───────────────────────────────────────────────
function _anGet(k, def) {
  try { return JSON.parse(localStorage.getItem(k) ?? 'null') ?? def; } catch { return def; }
}
function _anSet(k, v) { try { localStorage.setItem(k, JSON.stringify(v)); } catch {} }

const _anWatched  = () => _anGet('anWatched', {});
const _anProgress = () => _anGet('anProgress', {});
const _anCoins    = () => _anGet('anCoins', {});
const _anSaveW    = d  => _anSet('anWatched', d);
const _anSaveP    = d  => _anSet('anProgress', d);
const _anSaveC    = d  => _anSet('anCoins', d);

function anCoinsGet(id) { return _anCoins()[id] || 0; }
function anCoinsAdd(id, n) {
  const d = _anCoins(); d[id] = (d[id] || 0) + n; _anSaveC(d);
  const el = document.getElementById('animePlayerCoins');
  if (el) el.textContent = d[_AN.animeId] || 0;
}

// ── Speech ────────────────────────────────────────────────────────
function _anInitSynth() {
  if (!window.speechSynthesis) return;
  const pick = () => {
    const v = speechSynthesis.getVoices();
    _AN.synthVoice = v.find(x => /ja[-_]JP/i.test(x.lang)) || v.find(x => /^ja/i.test(x.lang)) || null;
  };
  pick(); speechSynthesis.onvoiceschanged = pick;
}
function animeSpeak(t) {
  if (!t || !window.speechSynthesis) return;
  const u = new SpeechSynthesisUtterance(t);
  u.lang = 'ja-JP'; if (_AN.synthVoice) u.voice = _AN.synthVoice; u.rate = 0.9;
  speechSynthesis.cancel(); speechSynthesis.speak(u);
}

// ── Word lookup ───────────────────────────────────────────────────
// Builds a mini-dictionary from all bundled subtitle segments on first call
let _anDictReady = false;
const _anDict = new Map(); // term -> {reading, meaning}

function _anBuildDict() {
  if (_anDictReady) return;
  _anDictReady = true;
  // Pull every unique word from all bundled subs
  if (window.ANIME_SUBS) {
    Object.values(window.ANIME_SUBS).forEach(data => {
      (data.segments || []).forEach(seg => {
        (seg.words || []).forEach(w => {
          const t = (w.word || '').trim();
          if (t && !_anDict.has(t)) {
            _anDict.set(t, { reading: t, meaning: '' });
          }
        });
      });
    });
  }
  // Overlay kanji DB readings
  const db = window.APP_KANJI_DB || window.KANJI_DB || [];
  db.forEach(k => {
    const char = k.char;
    if (char && _anDict.has(char)) {
      _anDict.set(char, {
        reading: k.readings?.[0] || k.on?.[0] || k.kun?.[0] || char,
        meaning: k.meanings?.[0] || k.meaning || '',
        jlpt: k.jlpt || '',
        pos: 'kanji',
      });
    }
...
async function _anLookup(term) {
  if (_AN.lookupCache.has(term)) return _AN.lookupCache.get(term);
  _anBuildDict();

  // tier 1: kanji DB exact match
  const db = window.APP_KANJI_DB || window.KANJI_DB || [];
  const ke = db.find(k => k.char === term || (k.exampleWords || []).some(w => w.word === term));
  if (ke) {
    const r = {
      term,
      reading: ke.readings?.[0] || ke.on?.[0] || ke.kun?.[0] || term,
      meaning: ke.meanings?.[0] || ke.meaning || '',
      jlpt: ke.jlpt || '',
      pos: 'kanji',
    };
    _AN.lookupCache.set(term, r);
    return r;
  }

  // tier 2: jisho via worker (best-effort, may fail)
  try {
    const ctrl = new AbortController();
    const timer = setTimeout(() => ctrl.abort(), 3000);
    const res = await fetch(`https://neuroverse.uk/api/jisho?word=${encodeURIComponent(term)}`, { signal: ctrl.signal });
    clearTimeout(timer);
    if (res.ok) {
      const j = await res.json();
      const f = j?.data?.[0];
      if (f && f.senses?.[0]?.english_definitions?.length) {
        const r = {
          term,
          reading: f.japanese?.[0]?.reading || term,
          meaning: f.senses[0].english_definitions.join('; '),
          jlpt: f.jlpt?.[0] || '',
          pos: f.senses[0].parts_of_speech?.join(', ') || '',
        };
        _AN.lookupCache.set(term, r);
        return r;
      }
  ...
  // tier 3: sub-dict reading (at least shows the word exists in the episode)
  const dictEntry = _anDict.get(term);
  if (dictEntry && dictEntry.reading) {
    const r = { term, reading: dictEntry.reading, meaning: dictEntry.meaning || '(tap 🔊 to hear)', jlpt: dictEntry.jlpt || '', pos: dictEntry.pos || '' };
    _AN.lookupCache.set(term, r);
    return r;
  }

  // tier 4: bare fallback
  const fb = { term, reading: term, meaning: '(definition unavailable)', jlpt: '', pos: '' };
  _AN.lookupCache.set(term, fb);
  return fb;
}

// ── Watched log ────────────────────────────────────────────────────
function _anLogWord(entry) {
  const d = _anWatched();
  if (!d[_AN.animeId]) d[_AN.animeId] = [];
  if (!d[_AN.animeId].some(e => e.term === entry.term && e.episodeId === entry.episodeId)) {
    d[_AN.animeId].push(entry); _anSaveW(d);
  }
}

// ── Vault ─────────────────────────────────────────────────────────
function _anAddVault(entry) {
  if (typeof lvAddToVault !== 'function') return;
  lvAddToVault({ id: 'an_' + entry.term + '_' + (entry.episodeId||''), type: 'word', term: entry.term, reading: entry.reading, meaning: entry.meaning, source: 'anime', animeId: _AN.animeId, episodeId: entry.episodeId, dateAdded: Date.now() });
  anCoinsAdd(_AN.animeId, 5);
  if (typeof dailyQuestsTrack === 'function') dailyQuestsTrack('anime_word_saved', 1);
}

// ── View transitions ───────────────────────────────────────────────
function _anShowView(name) {
  const target = 'animeView' + name; // e.g. 'animeViewLibrary'
  ['animeViewLibrary','animeViewSeries','animeViewPlayer'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.style.display = (id === target) ? '' : 'none';
  });
  // panel
  ['animePanelLibrary','animePanelSeries','animePanelPlayer'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.style.display = (id === 'animePanelLibrary' && name === 'Library') ||
                                (id === 'animePanelSeries'  && name === 'Series')  ||
                                (id === 'animePanelPlayer'  && name === 'Player')  ? '' : 'none';
  });
}
function _anCap(s) { return s.charAt(0).toUpperCase() + s.slice(1); }

// ─────────────────────────────────────────────────────────────────
// VIEW: LIBRARY
// ─────────────────────────────────────────────────────────────────
function animeShowLibrary() {
  _anStopVideo();
  _anShowView('Library');
  _anRenderPanelList();
  _anRenderLibrary();
...
function _anRenderPanelList() {
  const el = document.getElementById('animePanelList');
  if (!el) return;
  el.innerHTML = '';
  ANIME_CATALOG.forEach(a => {
    const item = document.createElement('div');
    item.className = 'anime-panel-item' + (a.id === _AN.animeId ? ' active' : '');
    item.innerHTML = `<span class="anime-panel-dot"></span><span>${a.title}</span>`;
    item.onclick = () => animeShowSeries(a.id);
    el.appendChild(item);
  });
}

function _anRenderLibrary() {
  const grid = document.getElementById('animeLibraryGrid');
  if (!grid) return;
  grid.innerHTML = '';
  const prog = _anProgress(), watched = _anWatched();
  ANIME_CATALOG.forEach((a, i) => {
    let totPct = 0;
    a.episodes.forEach(ep => { totPct += (_anProgress()[a.id+'_'+ep.id]?.pct||0); });
    const avgPct = a.episodes.length ? totPct/a.episodes.length : 0;
    const wc = (watched[a.id]||[]).length;
    const card = document.createElement('div');
    card.className = 'anime-lib-card';
    card.style.animationDelay = (i * 60) + 'ms';
    card.innerHTML = a.posterUrl
      ? `<img class="anime-lib-poster" src="${a.posterUrl}" alt="${a.title}">`
      : `<div class="anime-lib-poster-ph">📺</div>`;
    card.innerHTML += `<div class="anime-lib-title">${a.title}</div><div class="anime-lib-sub">${a.episodes.length} ep · ${wc} words · ${Math.round(avgPct*100)}%</div>`;
    card.onclick = () => animeShowSeries(a.id);
...
// VIEW: SERIES
// ─────────────────────────────────────────────────────────────────
function animeShowSeries(id) {
  if (id) _AN.animeId = id;
  _anShowView('Series');
  const a = ANIME_CATALOG.find(x => x.id === _AN.animeId);
  if (!a) return;
  document.getElementById('animeSeriesTitle').textContent = a.title;
  document.getElementById('animeSeriesSubtitle').textContent = a.titleJP + ' · ' + a.episodes.length + ' episodes';
  // panel
  _anRenderPanelProgress();
  // default to episodes tab
  animeSeriesTab('episodes', document.getElementById('animeNavEpBtn'));
}

function animeSeriesTab(tab, btn) {
  document.querySelectorAll('#animePanelSeries .panel-pill').forEach(b => b.classList.remove('active'));
  if (btn) btn.classList.add('active');
  const epBox   = document.getElementById('animeEpBox');
  const shopBox = document.getElementById('animeShopBox');
  if (tab === 'episodes') {
    if (epBox) epBox.style.display = '';
    if (shopBox) shopBox.style.display = 'none';
    _anRenderEpisodes();
  } else {
    if (epBox) epBox.style.display = 'none';
    if (shopBox) shopBox.style.display = '';
    _anRenderShop();
  }
}

function _anRenderPanelProgress() {
  const el = document.getElementById('animePanelProgressStats');
  if (!el) return;
  const a = ANIME_CATALOG.find(x => x.id === _AN.animeId);
  if (!a) return;
  const watched = (_anWatched()[_AN.animeId]||[]).length;
  const coins   = anCoinsGet(_AN.animeId);
  el.innerHTML = `
    <div class="panel-stat-row"><span class="panel-stat-key">Words seen</span><span class="panel-stat-val">${watched}</span></div>
    <div class="panel-stat-row"><span class="panel-stat-key">Yoru Coins</span><span class="panel-stat-val">${coins}</span></div>
  `;
}

function _anRenderEpisodes() {
  const a    = ANIME_CATALOG.find(x => x.id === _AN.animeId);
  const list = document.getElementById('animeEpList');
  const cnt  = document.getElementById('animeEpBoxCount');
  if (!list || !a) return;
  if (cnt) cnt.textContent = a.episodes.length + ' episode' + (a.episodes.length!==1?'s':'');
  list.innerHTML = '';
  const prog    = _anProgress();
  const watched = _anWatched();
  a.episodes.forEach(ep => {
    const p   = prog[_AN.animeId+'_'+ep.id] || {};
    const pct = p.pct || 0;
    const wc  = (watched[_AN.animeId]||[]).filter(w => w.episodeId===ep.id).length;
    const dur = _anFmtDur(ep.duration||0);
    const row = document.createElement('div');
    row.className = 'anime-ep-row' + (ep.locked ? ' locked' : '');
    row.innerHTML = `
      <div class="anime-ep-thumb">${ep.thumbUrl ? `<img src="${ep.thumbUrl}" style="width:100%;height:100%;object-fit:cover">` : '▶'}</div>
      <div class="anime-ep-row-info">
        <div class="anime-ep-row-num">EP ${String(ep.num).padStart(2,'0')}</div>
        <div class="anime-ep-row-title">${ep.title}</div>
        <div class="anime-ep-row-meta">
          <span>${dur}</span>·
          <div class="anime-ep-progbar"><div class="anime-ep-progbar-fill" style="width:${Math.round(pct*100)}%"></div></div>
          <span>${Math.round(pct*100)}%</span>·
          <span>${wc} words</span>
        </div>
      </div>
      ${!ep.locked ? `<button class="anime-ep-play-btn" onclick="animeOpenEpisode('${ep.id}');event.stopPropagation()">▶ Play</button>` : ''}
    `;
    if (!ep.locked) row.onclick = () => animeOpenEpisode(ep.id);
    list.appendChild(row);
  });
}

function _anRenderShop() {
  const a    = ANIME_CATALOG.find(x => x.id === _AN.animeId);
  const list = document.getElementById('animeShopList');
  const balEl = document.getElementById('animeShopBalance');
  if (!list || !a) return;
  const bal = anCoinsGet(_AN.animeId);
  if (balEl) balEl.textContent = bal;
  list.innerHTML = '';

  (a.shopItems||[]).forEach(item => {
    const canAfford = bal >= item.cost;

    // Check if already owned
    let isOwned = false;
    if (item.action === 'widget' && item.widgetId) {
      isOwned = (typeof ownedWidgets !== 'undefined') && ownedWidgets.includes(item.widgetId);
    } else if (item.action === 'title' && item.data) {
      isOwned = (typeof inventory !== 'undefined') && inventory.some(i => i.data === item.data && i.type === 'title');
    } else if (item.action === 'pack' && item.packId) {
      isOwned = (typeof ownedCosmPacks !== 'undefined') && !!ownedCosmPacks[item.packId];
    }

    const moonIcon = '<img src="https://pub-aa6bdcd20c8a4f75bf3984b6b5f04a96.r2.dev/icons/moon_tear.png" style="width:10px;height:10px;image-rendering:pixelated" alt="">';
    const row = document.createElement('div');
    row.className = 'anime-shop-row';
    const itemNameSafe = (item.name + '').replace(/'/g, "\'");
    row.innerHTML = `
      <div class="anime-shop-row-info" style="flex:1;min-width:0">
        <div class="anime-shop-row-name">${item.name}</div>
        <div class="anime-shop-row-type" style="font-size:10px;color:var(--muted)">${item.type}${item.desc ? ' · ' + item.desc : ''}</div>
      </div>
      <div class="anime-shop-row-price" style="font-family:'Space Mono',monospace;font-size:12px;white-space:nowrap;flex-shrink:0">${moonIcon} ${item.cost}</div>
      ${isOwned
        ? '<span style="font-family:var(--font-mono,monospace);font-size:10px;color:var(--correct);white-space:nowrap;flex-shrink:0">✓ Owned</span>'
        : `<button class="btn ${canAfford ? 'btn-primary' : 'btn-secondary'}" style="font-size:11px;padding:6px 12px;${canAfford ? '' : 'opacity:.45'};white-space:nowrap;flex-shrink:0" ${canAfford ? '' : 'disabled'}
            onclick="anBuyItem('${item.id}',${item.cost},'${itemNameSafe}')">
...
function anBuyItem(id, cost, name) {
  const d = _anCoins();
  if ((d[_AN.animeId]||0) < cost) { if (typeof rewardPopup === 'function') rewardPopup('Need more Yoru Coins!'); return; }

  // Find item definition
  const a    = ANIME_CATALOG.find(x => x.id === _AN.animeId);
  const item = (a?.shopItems||[]).find(i => i.id === id);

  d[_AN.animeId] -= cost; _anSaveC(d);

  if (item) {
    if (item.action === 'widget' && item.widgetId) {
      if (typeof addWidgetToInventory === 'function') addWidgetToInventory(item.widgetId);
      if (typeof rewardPopup === 'function') rewardPopup('Widget unlocked: ' + name + '! Check Bag → Widgets.');
    } else if (item.action === 'title' && item.data) {
      const titleItem = { id: 'yoru_title_' + id, name: 'Title: ' + item.data, type: 'title', data: item.data };
      if (typeof addToInventory === 'function') addToInventory(titleItem);
      if (typeof rewardPopup === 'function') rewardPopup('Title unlocked: ' + item.data + '! Check Bag → Items.');
    } else if (item.action === 'pack' && item.packId) {
      if (typeof ownedCosmPacks !== 'undefined' && !ownedCosmPacks[item.packId]) {
        const pack = typeof COSMETIC_PACKS !== 'undefined' ? COSMETIC_PACKS.find(p => p.id === item.packId) : null;
        if (pack) {
          ownedCosmPacks[item.packId] = { ownedItems: [pack.items[0].id] };
          if (typeof saveCosmPacks === 'function') saveCosmPacks();
        }
      }
      if (typeof rewardPopup === 'function') rewardPopup('BG Pack unlocked: ' + name + '! Check Bag → Cosmetics.');
    } else {
      if (typeof rewardPopup === 'function') rewardPopup('Purchased: ' + name);
    }
  } else {
    if (typeof rewardPopup === 'function') rewardPopup('Purchased: ' + name);
  }

  _anRenderShop(); _anRenderPanelProgress();
}

// ─────────────────────────────────────────────────────────────────
// VIEW: PLAYER
// ─────────────────────────────────────────────────────────────────
async function animeOpenEpisode(epId) {
  const a  = ANIME_CATALOG.find(x => x.id === _AN.animeId);
  const ep = a?.episodes.find(e => e.id === epId);
  if (!ep) return;
  ...
  _AN.segIdx      = -1;
  _AN.subHistory  = [];
  _AN.loopActive  = false;
  _AN.coinTick    = 0;
  // DON'T clear qsQueue — keep words across episode opens

  _anShowView('Player');

  // labels
  const lbl = `EP ${String(ep.num).padStart(2,'0')} — ${ep.title}`;
  const barLbl = document.getElementById('animePlayerBarLabel');
  if (barLbl) barLbl.textContent = lbl;

  // coin display
  const coinsEl = document.getElementById('animePlayerCoins');
  if (coinsEl) coinsEl.textContent = anCoinsGet(_AN.animeId);

  // reset loop UI
  const lt = document.getElementById('animeLoopToggle');
  if (lt) lt.textContent = '○';

  // reset QS dock
  _anRenderQSChips();

  // load subtitles first (so they're ready before video plays)
  await _anLoadSubtitles(ep);
  ...
  const saved = _anProgress()[_AN.animeId + '_' + epId];
  if (saved?.lastTime > 5 && !saved.completed) {
    video.addEventListener('loadedmetadata', () => { video.currentTime = saved.lastTime; }, { once: true });
  }

  // hide overlay msg on play
  video.addEventListener('play', () => {
    const msg = document.getElementById('animeVideoMsg');
    if (msg) msg.style.opacity = '0';
    document.getElementById('animePlayBtn').textContent = '⏸ Pause';
  }, { once: false });
  video.addEventListener('pause', () => {
    document.getElementById('animePlayBtn').textContent = '▶ Play';
  });

  video.ontimeupdate = () => _anTick(video, ep);
...
// ── Subtitle loading ───────────────────────────────────────────────
async function _anLoadSubtitles(ep) {
  const scroll = document.getElementById('animeSubScroll');
  if (scroll) scroll.innerHTML = '<div style="padding:16px;color:var(--muted);font-size:12px;text-align:center">Loading subtitles…</div>';

  let data = null;

  // Tier 1: bundled data (episode1_bundle.js pre-loaded in <head>)
  if (window.ANIME_SUBS && window.ANIME_SUBS[ep.id]) {
    data = window.ANIME_SUBS[ep.id];
    console.log('[anime] using bundled subtitles for', ep.id);
  }

  // Tier 2: remote R2 URL
  if (!data && ep.subtitleUrl) {
    try {
      const r = await fetch(ep.subtitleUrl);
      if (r.ok) { data = await r.json(); }
    } catch(e) { console.warn('[anime] remote fetch failed', e); }
  }

  // Tier 3: local file (dev server)
  if (!data) {
    try {
      const r = await fetch('anime/episode1.json');
      if (r.ok) { data = await r.json(); }
    } catch(e) { console.warn('[anime] local fetch failed', e); }
  }

  if (data && Array.isArray(data.segments) && data.segments.length > 0) {
    // bundled data is already filtered; raw JSON needs filtering
    _AN.segments = data.segments.filter(s => (s.no_speech_prob ?? 0) < 0.5);
    _anRenderSubtitles();
    console.log('[anime] loaded', _AN.segments.length, 'segments');
  } else {
    _AN.segments = [];
    if (scroll) scroll.innerHTML = '<div style="padding:16px;color:var(--muted);font-size:12px;text-align:center">No subtitles available.</div>';
  }
}

// render all lines into the scroll box
function _anRenderSubtitles() {
  const scroll = document.getElementById('animeSubScroll');
  if (!scroll) return;
  scroll.innerHTML = '';

  const vault   = typeof lvLoad === 'function' ? lvLoad() : [];
  const watched = _anWatched()[_AN.animeId] || [];

  _AN.segments.forEach((seg, idx) => {
    const div = document.createElement('div');
    div.className = 'anime-line';
    div.id = 'anline_' + idx;
    div.innerHTML = _anBuildLine(seg, vault, watched);
    scroll.appendChild(div);
  });
}

// build HTML for one segment's inner content
function _anBuildLine(seg, vault, watched) {
  if (!seg.words || seg.words.length === 0) {
    return _anEsc(seg.text || '');
  }
  return seg.words.map(w => {
    const t = w.word;
    if (!t || !t.trim()) return _anEsc(t);
    // only wrap CJK / Japanese characters as clickable — skip romaji/punctuation
    const hasCJK = /[\u3040-\u9faf\u3000-\u303f]/.test(t);
    if (!hasCJK) return _anEsc(t);
    const known  = vault.some(v => v.term === t);
    const seen   = watched.some(e => e.term === t);
    const queued = _AN.qsQueue.some(q => q.term === t);
    let cls = 'anime-word';
    if (queued)      cls += ' queued';
    else if (known)  cls += ' known';
    else if (seen)   cls += ' seen';
    const safe = t.replace(/'/g, '&#39;').replace(/"/g, '&quot;');
    return `<span class="${cls}" data-term="${safe}" onclick="anWordClick(event,'${safe}',${seg.id??idx})">${_anEsc(t)}</span>`;
  }).join('');
}
...
// ── Playback tick ──────────────────────────────────────────────────
function _anTick(video, ep) {
  const t = video.currentTime;

  // seek bar
  const fill = document.getElementById('animeSeekFill');
  const td   = document.getElementById('animeTimeDisp');
  if (fill && video.duration) fill.style.width = (t / video.duration * 100) + '%';
  if (td) td.textContent = _anFmtTime(t) + ' / ' + _anFmtTime(video.duration || ep.duration || 0);

  // loop
  if (_AN.loopActive && _AN.segIdx >= 0) {
    const seg = _AN.segments[_AN.segIdx];
    if (seg && t >= seg.end + 0.08) { video.currentTime = seg.start; return; }
  }

  // subtitle highlight — binary search
  const idx = _anBinSearch(t);
  if (idx !== -1 && idx !== _AN.segIdx) {
    _AN.segIdx = idx;
    _anHighlight(idx);
  }
  ...
  // save progress
  if (video.duration) {
    const pct = t / video.duration;
    const key = _AN.animeId + '_' + _AN.epId;
    const prog = _anProgress();
    const was  = prog[key] || {};
    prog[key]  = { pct, lastTime: t, completed: was.completed || pct >= 0.95 };
    if (!was.completed && pct >= 0.95) anCoinsAdd(_AN.animeId, 25);
    _anSaveP(prog);
  }

  // coin drip
  if (!video.paused && t - _AN.coinTick >= 60) {
    _AN.coinTick = t;
    anCoinsAdd(_AN.animeId, 1);
  }
}

function _anBinSearch(t) {
  const segs = _AN.segments;
  if (!segs.length) return -1;
  let lo = 0, hi = segs.length - 1;
  while (lo <= hi) {
    const mid = (lo + hi) >> 1;
    const s   = segs[mid];
    if      (t < s.start - 0.05) hi = mid - 1;
    else if (t > s.end   + 0.05) lo = mid + 1;
    else return mid;
  }
  return -1;
}

function _anHighlight(idx) {
  // remove previous active / mark as past
  document.querySelectorAll('.anime-line.active').forEach(el => {
    el.classList.remove('active');
    el.classList.add('past');
  });
  const el = document.getElementById('anline_' + idx);
  if (!el) return;
...
// ── Video controls ─────────────────────────────────────────────────
function animePlayPause() {
  const v = document.getElementById('animeVideo');
  if (!v) return;
  v.paused ? v.play() : v.pause();
}
function animeSeekLine() {
  const v = document.getElementById('animeVideo');
  if (!v || _AN.segIdx < 0) return;
  const s = _AN.segments[_AN.segIdx];
  if (s) v.currentTime = s.start;
}
function animeToggleLoop() {
  _AN.loopActive = !_AN.loopActive;
  const lt = document.getElementById('animeLoopToggle');
  if (lt) { lt.textContent = _AN.loopActive ? '●' : '○'; lt.style.color = _AN.loopActive ? 'var(--accent3)' : 'var(--muted)'; }
}
function animeSkipOP() {
  const a  = ANIME_CATALOG.find(x => x.id === _AN.animeId);
  const ep = a?.episodes.find(e => e.id === _AN.epId);
  const v  = document.getElementById('animeVideo');
  if (v && ep) v.currentTime = ep.opEnd || 90;
}
function animeSeekClick(e) {
  const track = document.getElementById('animeSeekTrack');
  const v     = document.getElementById('animeVideo');
  if (!v?.duration) return;
  const rect = track.getBoundingClientRect();
  v.currentTime = ((e.clientX - rect.left) / rect.width) * v.duration;
}
function _anStopVideo() {
  const v = document.getElementById('animeVideo');
  if (v) { v.pause(); v.src = ''; v.ontimeupdate = null; }
}
function animeShowSeriesFromPlayer() {
  _anStopVideo();
  animeShowSeries();
}

// ── Resize handle ─────────────────────────────────────────────────
function _anInitResize(ep) {
  const handle = document.getElementById('animeResizeHandle');
  const col    = document.getElementById('animeVideoCol');
  const stage  = document.getElementById('animeStage');
  if (!handle || !col || !stage) return;

  // set initial width
  const stageW = stage.getBoundingClientRect().width || 600;
  const initW  = _AN.videoColW || Math.round(stageW * 0.58);
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    const newW  = Math.max(200, Math.min(startW + delta, stage.getBoundingClientRect().width - 160));
    col.style.width = newW + 'px';
    _AN.videoColW = newW;

    // match subtitle col height to video+qs dock height
    _anSyncSubHeight();
  });
  ...
  document.addEventListener('touchend', () => { dragging = false; _anSyncSubHeight(); });

  // initial sync
  setTimeout(_anSyncSubHeight, 100);
}

function _anSyncSubHeight() {
  const col    = document.getElementById('animeVideoCol');
  const subCol = document.getElementById('animeSubCol');
  if (!col || !subCol) return;
  const h = col.getBoundingClientRect().height;
  if (h > 0) subCol.style.height = h + 'px';
}

// ── Word click ─────────────────────────────────────────────────────
async function anWordClick(evt, term, segIdx) {
  evt.stopPropagation();
  if (!term) return;

  const entry = await _anLookup(term);
  _AN.popEntry = { ...entry, animeId: _AN.animeId, episodeId: _AN.epId, timestamp: Date.now(), addedToStudy: false };
  _anLogWord(_AN.popEntry);

  // populate popup
  document.getElementById('animePopTerm').textContent    = entry.term;
  document.getElementById('animePopReading').textContent = entry.reading || '';
  document.getElementById('animePopMeaning').textContent = entry.meaning || '—';
  const meta = [entry.jlpt?.toUpperCase(), entry.pos].filter(Boolean).join(' · ');
  document.getElementById('animePopMeta').textContent = meta;

  const vault = typeof lvLoad === 'function' ? lvLoad() : [];
  const addBtn = document.getElementById('animePopAddBtn');
  const inVault = vault.some(v => v.term === term && v.source === 'anime');
  if (addBtn) { addBtn.textContent = inVault ? '✓ Added' : '+ Add to Study'; addBtn.className = inVault ? 'btn btn-secondary' : 'btn btn-primary'; }

  const qsBtn  = document.getElementById('animePopQSBtn');
  const inQS   = _AN.qsQueue.some(q => q.term === term);
  if (qsBtn) { qsBtn.textContent = inQS ? '✓ In Queue' : '+ Quick Study'; qsBtn.className = inQS ? 'btn btn-primary' : 'btn btn-secondary'; }

  document.getElementById('animeWordPopup').style.display = '';
  document.getElementById('animePopupBg').style.display   = '';
}

function animeClosePopup() {
  document.getElementById('animeWordPopup').style.display = 'none';
  document.getElementById('animePopupBg').style.display   = 'none';
  _AN.popEntry = null;
}

function animePopAdd() {
  if (!_AN.popEntry) return;
  _anAddVault(_AN.popEntry);
  const btn = document.getElementById('animePopAddBtn');
  if (btn) { btn.textContent = '✓ Added'; btn.className = 'btn btn-secondary'; }
  _anRefreshLine();
}

function animePopQueueQS() {
  if (!_AN.popEntry) return;
  const term = _AN.popEntry.term;
  const inQ  = _AN.qsQueue.some(q => q.term === term);
  if (inQ) { _AN.qsQueue = _AN.qsQueue.filter(q => q.term !== term); }
  else      { _AN.qsQueue.push({ ..._AN.popEntry }); }
  const qsBtn = document.getElementById('animePopQSBtn');
  const nowIn = _AN.qsQueue.some(q => q.term === term);
  if (qsBtn) { qsBtn.textContent = nowIn ? '✓ In Queue' : '+ Quick Study'; qsBtn.className = nowIn ? 'btn btn-primary' : 'btn btn-secondary'; }
  _anRenderQSChips();
  _anRefreshLine();
}

function _anRefreshLine() {
  // re-render the currently active subtitle line
  if (_AN.segIdx < 0) return;
  const seg = _AN.segments[_AN.segIdx];
  const el  = document.getElementById('anline_' + _AN.segIdx);
  if (!seg || !el) return;
  const vault   = typeof lvLoad === 'function' ? lvLoad() : [];
  const watched = _anWatched()[_AN.animeId] || [];
  el.innerHTML  = _anBuildLine(seg, vault, watched);
}

// ── QS chips ──────────────────────────────────────────────────────
function _anRenderQSChips() {
  const chips    = document.getElementById('animeQSChips');
  const hint     = document.getElementById('animeQSHint');
  const startBtn = document.getElementById('animePanelQSStartBtn');
  const panelWds = document.getElementById('animePanelQSWords');
  if (!chips) return;

  chips.innerHTML = '';
  _AN.qsQueue.forEach(entry => {
    const chip = document.createElement('span');
    chip.className = 'anime-qs-chip';
    chip.innerHTML = `${_anEsc(entry.term)}<button class="anime-qs-chip-x" onclick="anQSRemoveChip('${entry.term.replace(/'/g,"\\'")}')">✕</button>`;
    chips.appendChild(chip);
  });

  const has = _AN.qsQueue.length > 0;
  if (hint)     hint.style.display     = has ? 'none' : '';
  if (startBtn) startBtn.style.display = has ? '' : 'none';

  // panel word list
  if (panelWds) {
    panelWds.innerHTML = '';
    _AN.qsQueue.forEach(e => {
      const d = document.createElement('div');
      d.style.cssText = 'display:flex;align-items:center;gap:6px;font-size:12px;color:var(--text)';
      d.innerHTML = `<span style="font-family:\'Noto Sans JP\',sans-serif">${_anEsc(e.term)}</span><span style="color:var(--muted);font-size:11px;flex:1">${_anEsc(e.reading||'')}</span><button onclick="anQSRemoveChip('${e.term.replace(/'/g,"\\'")}\')" style="background:none;border:none;color:var(--muted);cursor:pointer;font-size:10px">✕</button>`;
      panelWds.appendChild(d);
    });
  }
}

function anQSRemoveChip(term) {
  _AN.qsQueue = _AN.qsQueue.filter(q => q.term !== term);
  _anRenderQSChips();
  _anRefreshLine();
}

// ── QS session ────────────────────────────────────────────────────
function animeOpenQS() {
  if (!_AN.qsQueue.length) return;
  const sess = document.getElementById('animeQSSession');
  if (!sess) return;
  sess.style.display = '';
  document.getElementById('animeQSPicker').style.display = 'flex';
  document.getElementById('animeQSCard').style.display   = 'none';
  document.getElementById('animeQSSummary').style.display= 'none';
  // pause video
  const v = document.getElementById('animeVideo');
  if (v && !v.paused) v.pause();
}

function animeQSSetMode(mode, btn) {
  _AN.qsMode = mode;
  document.querySelectorAll('#animeQSPicker .unit-btn').forEach(b => b.classList.remove('active'));
  if (btn) btn.classList.add('active');
}

function animeQSBegin() {
  _AN.qsDeck = [..._AN.qsQueue];
  // shuffle
  for (let i = _AN.qsDeck.length-1; i>0; i--) { const j=Math.floor(Math.random()*(i+1)); [_AN.qsDeck[i],_AN.qsDeck[j]]=[_AN.qsDeck[j],_AN.qsDeck[i]]; }
  _AN.qsIdx = 0; _AN.qsWrong = []; _AN.qsScore = 0;
  document.getElementById('animeQSPicker').style.display = 'none';
  document.getElementById('animeQSCard').style.display   = '';
  _anQSRender();
}

function _anQSRender() {
  const word = _AN.qsDeck[_AN.qsIdx];
  if (!word) { _anQSSummary(); return; }
  document.getElementById('animeQSProg').textContent = `${_AN.qsIdx+1} / ${_AN.qsDeck.length}`;
  document.getElementById('animeQSTerm').textContent = word.term;

  // dots
  const dotsEl = document.getElementById('animeQSDots');
  if (dotsEl) {
    dotsEl.innerHTML = '';
    _AN.qsDeck.forEach((_,i) => {
      const d = document.createElement('span');
      d.className = 'dot' + (i===_AN.qsIdx?' current':'') + (i<_AN.qsIdx?' correct':'');
      dotsEl.appendChild(d);
    });
  }

  // hide all panels
  document.getElementById('animeQSFlashArea').style.display  = 'none';
  document.getElementById('animeQSMCQArea').style.display    = 'none';
  document.getElementById('animeQSWrittenArea').style.display= 'none';

  if (_AN.qsMode === 'flash') {
    document.getElementById('animeQSFlashArea').style.display = '';
    document.getElementById('animeQSReveal').style.display    = 'none';
    document.getElementById('animeQSFlipBtn').style.display   = '';
    document.getElementById('animeQSGradeRow').style.display  = 'none';
  } else if (_AN.qsMode === 'mcq') {
    document.getElementById('animeQSMCQArea').style.display = '';
    _anQSBuildMCQ(word);
  } else {
    document.getElementById('animeQSWrittenArea').style.display = '';
    const inp = document.getElementById('animeQSInput');
    if (inp) inp.value = '';
    document.getElementById('animeQSWriteFb').style.display = 'none';
    setTimeout(() => { const inp = document.getElementById('animeQSInput'); if(inp) inp.focus(); }, 40);
  }
}

function animeQSFlip() {
  const word = _AN.qsDeck[_AN.qsIdx];
  document.getElementById('animeQSReading').textContent = word.reading || '';
  document.getElementById('animeQSMeaning').textContent = word.meaning || '';
  document.getElementById('animeQSReveal').style.display   = '';
  document.getElementById('animeQSFlipBtn').style.display  = 'none';
  document.getElementById('animeQSGradeRow').style.display = '';
  animeSpeak(word.reading || word.term);
}

function _anQSBuildMCQ(word) {
  const opts = document.getElementById('animeQSMCQOpts');
  if (!opts) return;
  opts.innerHTML = '';

  const pool = [];
  _AN.qsDeck.forEach(w => { if (w.term!==word.term && w.meaning) pool.push(w.meaning); });
  if (typeof lvLoad === 'function') lvLoad().forEach(v => { if (v.term!==word.term && v.meaning && !pool.includes(v.meaning)) pool.push(v.meaning); });
  for (let i=pool.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[pool[i],pool[j]]=[pool[j],pool[i]];}
  const choices = [word.meaning, ...pool.slice(0,3)];
  ...
  choices.forEach((c, ci) => {
    ...
    btn.innerHTML = `<span class="opt-letter">${String.fromCharCode(65+ci)}</span>${_anEsc(c)}`;
    btn.onclick = () => {
      if (btn.dataset.done) return;
      opts.querySelectorAll('.option').forEach(b => b.dataset.done = '1');
      opts.querySelectorAll('.option').forEach(b => { if (b.textContent.slice(1) === word.meaning) b.classList.add('correct'); });
      if (c !== word.meaning) btn.classList.add('wrong');
      setTimeout(() => animeQSGrade(c===word.meaning?'know':'again'), 900);
    };
    opts.appendChild(btn);
  });
}

function animeQSWriteCheck() {
  document.getElementById('animeQSWriteFb').style.display = '';
  const word = _AN.qsDeck[_AN.qsIdx];
  document.getElementById('animeQSWriteAns').innerHTML = `<strong style="font-size:11px;font-family:\'Space Mono\',monospace;color:var(--accent);display:block;margin-bottom:4px">ANSWER</strong>${_anEsc(word.meaning||'')}`;
}

function animeQSGrade(result) {
  if (result==='know'||result==='correct') { _AN.qsScore++; if (typeof rewardStudy==='function') rewardStudy(2); }
  else _AN.qsWrong.push(_AN.qsDeck[_AN.qsIdx]);
  _AN.qsIdx++;
  if (_AN.qsIdx < _AN.qsDeck.length) _anQSRender();
  else _anQSSummary();
}

function _anQSSummary() {
  document.getElementById('animeQSCard').style.display    = 'none';
  document.getElementById('animeQSSummary').style.display = '';
  document.getElementById('animeQSSumScore').textContent  = _AN.qsScore + ' / ' + _AN.qsDeck.length;
  document.getElementById('animeQSSumDetail').textContent = _AN.qsWrong.length + ' wrong · ' + (_AN.qsDeck.length-_AN.qsWrong.length) + ' correct';
  const rb = document.getElementById('animeQSRetryBtn');
  if (rb) rb.style.display = _AN.qsWrong.length ? '' : 'none';
}

function animeQSRetry() {
  _AN.qsDeck = [..._AN.qsWrong]; _AN.qsIdx=0; _AN.qsWrong=[]; _AN.qsScore=0;
  document.getElementById('animeQSSummary').style.display = 'none';
  document.getElementById('animeQSCard').style.display    = '';
  _anQSRender();
}

function animeQSClose() {
  document.getElementById('animeQSSession').style.display = 'none';
  const sb = document.getElementById('animePanelQSStartBtn');
  if (sb && _AN.qsQueue.length) sb.style.display = '';
}

// ── Utilities ──────────────────────────────────────────────────────
function _anFmtTime(s) {
  s=Math.floor(s||0); return Math.floor(s/60)+':'+String(s%60).padStart(2,'0');
}
function _anFmtDur(s) {
  s=Math.floor(s||0); return Math.floor(s/60)+'m'+(s%60?(' '+s%60+'s'):'');
}

// ── Init ───────────────────────────────────────────────────────────
function initAnimeTab() {
  _anInitSynth();
  // always land on library view
  _anShowView('Library');
  _anRenderPanelList();
  _anRenderLibrary();
}
...
(function() {
  ...
  window.switchMode = function(mode, event) {
    if (_orig) _orig(mode, event);
    if (mode === 'anime') setTimeout(initAnimeTab, 40);
  };
})();

// ── Collapsible panel sections ────────────────────────────────────
function anToggleSection(id) {
...
// ════════════════════════════════════════════════════════════════
//  ANIME TAB — PANEL COLLAPSE + EPISODE SOURCE FILTERS + DEEP STUDY SENTENCE MODE
// ════════════════════════════════════════════════════════════════

// ── Panel collapse ────────────────────────────────────────────────
let _anPanelCollapsed = false;

function anTogglePanel() {
  _anPanelCollapsed = !_anPanelCollapsed;
  const panel  = document.getElementById('animeSidePanel');
  const main   = document.getElementById('animeMainArea');
  const btn    = document.getElementById('animePanelCollapseBtn');
  const layout = panel?.closest('.tab-layout');

  if (_anPanelCollapsed) {
    if (panel)  { panel.style.width = '0'; panel.style.minWidth = '0'; panel.style.overflow = 'hidden'; panel.style.opacity = '0'; panel.style.padding = '0'; }
    if (main)   main.style.maxWidth = '100%';
    if (layout) layout.style.maxWidth = '100%';
    if (btn)    btn.textContent = '›';
    // sync subtitle col height after expansion
    setTimeout(_anSyncSubHeight, 320);
  } else {
    if (panel)  { panel.style.width = ''; panel.style.minWidth = ''; panel.style.overflow = ''; panel.style.opacity = ''; panel.style.padding = ''; }
...
// ── MCQ anime source filter ───────────────────────────────────────
let _mcqAnimeSource = 'all';

function setMcqAnimeSource(src, btn) {
  _mcqAnimeSource = src;
  document.querySelectorAll('[id^="mcqAnimeSource"]').forEach(b => b.classList.remove('active'));
  if (btn) btn.classList.add('active');

  if (src === 'all') {
    // restore normal MCQ pool
    initMCQ();
    return;
  }

  // Build MCQ questions from vault words for this episode
  const vault   = typeof lvLoad === 'function' ? lvLoad() : [];
  const epWords = vault.filter(v => v.source === 'anime' && v.episodeId === src && v.term && v.meaning);

  if (!epWords.length) {
    alert('No words saved from this episode yet. Watch the episode and add words first!');
    return;
  }

  // Build MCQ-compatible questions: JP term → EN meaning
  const questions = epWords.map(v => ({
    q: v.term + (v.reading && v.reading !== v.term ? '\n' + v.reading : ''),
    a: v.meaning,
    opts: [],         // will be generated dynamically
    ans: 0,
    unit: 'anime',
    diff: 'medium',
    source: 'anime',
  ...
  // Shuffle and inject as MCQ pool
  ...
  // Build multi-choice for each question using other words as distractors
  shuffled.forEach((q, qi) => {
    const distractors = shuffled
      .filter((_, di) => di !== qi && shuffled[di].a !== q.a)
      .map(d => d.a)
      .slice(0, 3);
    while (distractors.length < 3) distractors.push('—');
    const opts = [q.a, ...distractors];
    for (let i = opts.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [opts[i], opts[j]] = [opts[j], opts[i]]; }
    q.opts = opts;
    q.ans  = opts.indexOf(q.a);
  });
  ...
  mcqAnswered = new Array(mcqPool.length).fill(null);
  if (typeof renderMCQ === 'function') renderMCQ();
  else if (typeof initMCQ === 'function') {
    // Force re-render by calling renderMCQCard equivalent
    const card = document.getElementById('mcqCardWrap') || document.getElementById('mcqCard');
    if (card && mcqPool.length) {
      // Let renderMCQ handle it
      window.mcqPool = mcqPool;
      window.mcqIdx  = 0;
      if (typeof window._mcqRender === 'function') window._mcqRender();
    }
  }
}

// ── Anime Deep Study — Sentence Translation Mode ──────────────────
// Exposed as window.animeDeepStudy so it can be launched from anywhere

window.animeDeepStudy = {
  pool:    [],
  idx:     0,
  results: [],
  videoEl: null,
...
function animeDeepStudyOpen(epId) {
  epId = epId || _AN.epId || 'yofukashi_ep1';

  // Build pool from bundled segments that have words
  const bundled = window.ANIME_SUBS?.[epId];
  if (!bundled || !bundled.segments?.length) {
    alert('Load the episode first to use Anime Sentence Study.');
    return;
  }

  const segs = bundled.segments.filter(s => s.words?.length > 2 && s.text.length > 3);
  if (!segs.length) { alert('No subtitle sentences available.'); return; }

  // Shuffle
  const pool = [...segs];
  for (let i = pool.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [pool[i], pool[j]] = [pool[j], pool[i]]; }

  window.animeDeepStudy.pool    = pool.slice(0, 20); // cap at 20 per session
  window.animeDeepStudy.idx     = 0;
  window.animeDeepStudy.results = [];

  _anDSRenderModal();
}

function _anDSRenderModal() {
  // Remove existing
  const old = document.getElementById('animeDeepStudyModal');
  if (old) old.remove();

  const ds   = window.animeDeepStudy;
  const seg  = ds.pool[ds.idx];
  if (!seg) { _anDSShowSummary(); return; }

  const modal = document.createElement('div');
  modal.id = 'animeDeepStudyModal';
  modal.style.cssText = 'position:fixed;inset:0;z-index:2000;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.75);padding:20px;box-sizing:border-box';

  const box = document.createElement('div');
  box.style.cssText = 'background:var(--surface);border:1px solid var(--border);border-radius:16px;padding:24px;max-width:560px;width:100%;max-height:90vh;overflow-y:auto;animation:slideIn .25s ease;position:relative';

  // header
  box.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
      <div>
        <div style="font-family:'Space Mono',monospace;font-size:9px;letter-spacing:2px;color:var(--accent3);text-transform:uppercase;margin-bottom:2px">ANIME SENTENCE STUDY</div>
        <div style="font-family:'Space Mono',monospace;font-size:10px;color:var(--muted)">${ds.idx + 1} / ${ds.pool.length}</div>
      </div>
      <button onclick="document.getElementById('animeDeepStudyModal').remove()" style="background:none;border:none;color:var(--muted);font-size:18px;cursor:pointer;padding:4px">✕</button>
    </div>

    <!-- dot progress -->
    <div class="dot-row" style="margin-bottom:18px">
      ${ds.pool.map((_,i) => `<div class="dot${i===ds.idx?' current':i<ds.idx?' correct':''}"></div>`).join('')}
    </div>

    <!-- JP sentence -->
    <div style="background:var(--surface2);border:1px solid var(--border);border-radius:12px;padding:16px 18px;margin-bottom:14px">
      <div style="font-family:'Space Mono',monospace;font-size:8px;letter-spacing:2px;color:var(--muted);text-transform:uppercase;margin-bottom:8px">Japanese</div>
      <div id="anDSSentenceJP" style="font-family:'Noto Sans JP',sans-serif;font-size:20px;font-weight:600;color:var(--text);line-height:1.7;cursor:pointer"
        onclick="anDSPlaySegment(${seg.start}, ${seg.end})">${seg.words.map(w => `<span class="anime-word" onclick="anWordPopupFromDS(event,'${w.word.replace(/'/g,"\\'")}');event.stopPropagation()">${w.word}</span>`).join('')}</div>
      <button onclick="anDSPlaySegment(${seg.start}, ${seg.end})" style="margin-top:10px;background:none;border:1px solid var(--border);border-radius:6px;color:var(--muted);font-size:11px;padding:4px 10px;cursor:pointer">🔊 Play clip</button>
    </div>

    <!-- write-out area -->
    <div style="margin-bottom:12px">
      <div style="font-family:'Space Mono',monospace;font-size:8px;letter-spacing:2px;color:var(--muted);text-transform:uppercase;margin-bottom:8px">Your Translation (English)</div>
      <textarea id="anDSUserAnswer" rows="3"
        style="width:100%;background:var(--bg);border:1px solid var(--border);border-radius:8px;color:var(--text);font-family:'Syne',sans-serif;font-size:14px;padding:10px 12px;resize:vertical;min-height:72px;box-sizing:border-box;outline:none;transition:border-color .2s"
        onfocus="this.style.borderColor='var(--accent3)'" onblur="this.style.borderColor=''"
        placeholder="What does this sentence mean in English?"></textarea>
    </div>

    <div class="btn-row" style="margin-top:0">
      <button class="btn btn-primary" style="flex:1" onclick="anDSReveal()">Check →</button>
      <button class="btn btn-secondary" style="padding:10px 14px" onclick="anDSSkip()">Skip</button>
    </div>

    <!-- answer reveal area (hidden until Check) -->
    <div id="anDSRevealArea" style="display:none;margin-top:16px">
      <div style="background:rgba(0,255,157,0.06);border:1px solid rgba(0,255,157,0.2);border-radius:12px;padding:14px 16px;margin-bottom:12px">
        <div style="font-family:'Space Mono',monospace;font-size:8px;letter-spacing:2px;color:var(--accent);text-transform:uppercase;margin-bottom:8px">WORD BREAKDOWN</div>
        <div id="anDSBreakdown" style="display:flex;flex-direction:column;gap:6px"></div>
      </div>
      <div style="font-family:'Space Mono',monospace;font-size:8px;letter-spacing:2px;color:var(--muted);text-transform:uppercase;margin-bottom:6px">SELF GRADE</div>
      <div class="self-rate show" style="margin-top:0">
        <button class="rate-btn missed" style="flex:1" onclick="anDSGrade('wrong')">✗ Wrong</button>
        <button class="rate-btn" style="flex:1;border-color:var(--warn);color:var(--warn);background:rgba(255,211,42,0.08)" onclick="anDSGrade('close')">~ Close</button>
        <button class="rate-btn got-it" style="flex:1" onclick="anDSGrade('correct')">✓ Correct</button>
      </div>
    </div>
  `;
  ...
  // Focus textarea
  setTimeout(() => { const ta = document.getElementById('anDSUserAnswer'); if (ta) ta.focus(); }, 100);
}

async function anDSReveal() {
  const ds  = window.animeDeepStudy;
  const seg = ds.pool[ds.idx];
  if (!seg) return;

  const revealArea = document.getElementById('anDSRevealArea');
  if (!revealArea) return;
  revealArea.style.display = '';

  const breakdown = document.getElementById('anDSBreakdown');
  if (!breakdown) return;

  // Look up each word in segment
  breakdown.innerHTML = '<div style="color:var(--muted);font-size:12px">Loading…</div>';

  const uniqueWords = [...new Set((seg.words || []).map(w => w.word.trim()).filter(w => /[\u3040-\u9faf]/.test(w)))];
  const lookups = await Promise.all(uniqueWords.map(w => _anLookup(w)));

  breakdown.innerHTML = '';
  lookups.forEach((entry, i) => {
    if (!entry || (!entry.reading && !entry.meaning)) return;
    const row = document.createElement('div');
    row.style.cssText = 'display:flex;align-items:baseline;gap:10px;padding:5px 0;border-bottom:1px solid var(--border)';
    row.innerHTML = `
      <span style="font-family:'Noto Sans JP',sans-serif;font-size:16px;font-weight:700;color:var(--text);min-width:36px">${entry.term}</span>
      <span style="font-family:'Noto Sans JP',sans-serif;font-size:12px;color:var(--muted);min-width:60px">${entry.reading !== entry.term ? entry.reading : ''}</span>
      <span style="font-size:13px;color:var(--text);flex:1">${entry.meaning || '—'}</span>
      ${entry.jlpt ? `<span style="font-family:'Space Mono',monospace;font-size:9px;color:var(--accent3)">${entry.jlpt.toUpperCase()}</span>` : ''}
      <button onclick="animeSpeak('${entry.reading.replace(/'/g,"\\'")}\')" style="background:none;border:none;color:var(--muted);cursor:pointer;font-size:13px;padding:0">🔊</button>
    `;
    breakdown.appendChild(row);
  });

  if (!breakdown.children.length) {
    breakdown.innerHTML = '<div style="color:var(--muted);font-size:12px">Click individual words above to look them up.</div>';
  }

  // Disable textarea
  const ta = document.getElementById('anDSUserAnswer');
  if (ta) ta.disabled = true;
  // Hide check btn
  const checkBtn = document.querySelector('#animeDeepStudyModal .btn-primary');
  if (checkBtn) checkBtn.style.display = 'none';
}

function anDSGrade(result) {
  const ds = window.animeDeepStudy;
  const ta = document.getElementById('anDSUserAnswer');
  ds.results.push({ seg: ds.pool[ds.idx], answer: ta?.value || '', grade: result });
  ds.idx++;

  if (ds.idx < ds.pool.length) {
    _anDSRenderModal();
  } else {
    _anDSShowSummary();
  }
...
function anDSSkip() {
  const ds = window.animeDeepStudy;
  ds.results.push({ seg: ds.pool[ds.idx], answer: '', grade: 'skip' });
  ds.idx++;
  if (ds.idx < ds.pool.length) _anDSRenderModal();
  else _anDSShowSummary();
}

function _anDSShowSummary() {
  const old = document.getElementById('animeDeepStudyModal');
  if (old) old.remove();

  const ds = window.animeDeepStudy;
  const correct = ds.results.filter(r => r.grade === 'correct').length;
  const close   = ds.results.filter(r => r.grade === 'close').length;
  const wrong   = ds.results.filter(r => r.grade === 'wrong').length;
  const skipped = ds.results.filter(r => r.grade === 'skip').length;
  ...
  modal.innerHTML = `
    <div style="background:var(--surface);border:1px solid var(--border);border-radius:16px;padding:28px;max-width:400px;width:100%;text-align:center;animation:slideIn .25s ease">
      <div style="font-size:36px;margin-bottom:12px">🎌</div>
      <div style="font-family:'Syne',sans-serif;font-size:20px;font-weight:800;color:var(--text);margin-bottom:6px">Session Complete</div>
      <div style="font-family:'Space Mono',monospace;font-size:10px;color:var(--muted);margin-bottom:20px">ANIME SENTENCE STUDY</div>
      <div class="stats-bar" style="justify-content:center;margin-bottom:20px">
        <div class="stat"><span class="stat-val green">${correct}</span><span class="stat-label">Correct</span></div>
        <div class="stat"><span class="stat-val" style="color:var(--warn)">${close}</span><span class="stat-label">Close</span></div>
        <div class="stat"><span class="stat-val red">${wrong}</span><span class="stat-label">Wrong</span></div>
        <div class="stat"><span class="stat-val" style="color:var(--muted)">${skipped}</span><span class="stat-label">Skipped</span></div>
      </div>
      <div class="btn-row" style="justify-content:center">
        <button class="btn btn-primary" onclick="animeDeepStudyOpen();document.getElementById('animeDeepStudyModal').remove()">Play Again</button>
        <button class="btn btn-secondary" onclick="document.getElementById('animeDeepStudyModal').remove()">Done</button>
      </div>
    </div>
  `;
  document.body.appendChild(modal);
...
function anDSPlaySegment(start, end) {
  // If video is open on player view, seek to segment and play
  const video = document.getElementById('animeVideo');
...
async function anWordPopupFromDS(evt, term) {
  evt.stopPropagation();
  const entry = await _anLookup(term);
  // show a mini tooltip near cursor
  let tip = document.getElementById('anDSWordTip');
  if (!tip) { tip = document.createElement('div'); tip.id = 'anDSWordTip'; tip.style.cssText = 'position:fixed;z-index:3000;background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:10px 13px;max-width:240px;box-shadow:0 4px 20px rgba(0,0,0,.5);font-size:12px;animation:slideIn .15s ease'; document.body.appendChild(tip); }
  tip.innerHTML = `<div style="font-family:'Noto Sans JP',sans-serif;font-size:20px;font-weight:700;color:var(--text);margin-bottom:3px">${entry.term} <span style="font-size:13px;color:var(--muted)">${entry.reading!==entry.term?entry.reading:''}</span></div><div style="color:var(--text);margin-bottom:6px">${entry.meaning||'—'}</div>${entry.jlpt?`<span style="font-family:'Space Mono',monospace;font-size:9px;color:var(--accent3)">${entry.jlpt.toUpperCase()}</span>`:''}`;
  tip.style.left = Math.min(evt.clientX + 12, window.innerWidth - 260) + 'px';
  tip.style.top  = (evt.clientY - 60) + 'px';
  const remove = (e) => { if (!tip.contains(e.target)) { tip.remove(); document.removeEventListener('click', remove); } };
  setTimeout(() => document.addEventListener('click', remove), 100);
}

// Expose launch for Deep Study panel
window.animeDeepStudyOpen = animeDeepStudyOpen;
