...
function kanaBuildPool() {
  ...
  const seen = new Set();
  return src.filter(k => { if (seen.has(k.kana)) return false; seen.add(k.kana); return true; });
}
...
function kanaRenderHiddenGrid(pool, correctRomaji) {
  ...
  const display = [...pool].sort(() => Math.random() - 0.5);
  grid.innerHTML = display.map(k =>
    `<button class="kana-cell kana-cell-hidden" data-romaji="${k.romaji}" data-kana="${k.kana}"
